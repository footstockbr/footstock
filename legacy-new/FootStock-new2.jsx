...
/* ── TOKENS — Black & Gold Premium Fintech Palette ── */
const BG="#080808",SURFACE="#0f0f0f",CARD="#141414",BORDER="#2a2218";
const ACCENT="#C9A84C",ACCENT2="#E8C96A",RED="#e05555",GOLD="#C9A84C";
const ACCENT_SOFT="#D4B466",ACCENT_DIM="rgba(201,168,76,0.10)";
const TEXT="#F0EAD6",MUTED="#7a7060";
const SANS="'Playfair Display','Georgia',serif",MONO="'JetBrains Mono','Fira Mono',monospace";
const SANS_BODY="'Inter','Helvetica Neue',sans-serif";
    ...
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none"
      ...
      <g clipPath="url(#fsclip)">
        ...
        <circle cx="55" cy="78" r="10"
          fill="#0d0c09" stroke="#5dfc00" strokeWidth="2.5"
          filter="url(#fsglow)"/>
        ...
        <circle cx="82" cy="68" r="5.5"
          fill="#0d0c09" stroke="#5dfc00" strokeWidth="2"
...
const SESSIONS=[
  ...
  {id:"main",    label:"Negociação",         short:"ABERTO", color:"#C9A84C", bg:"rgba(201,168,76,.08)",  border:"rgba(201,168,76,.3)",    start:11*60+0,  end:24*60+45},
  {id:"closing", label:"Call de Fechamento", short:"LEILÃO", color:"#D4B466", bg:"rgba(201,168,76,.07)", border:"rgba(201,168,76,.25)",   start:24*60+45, end:25*60+0},
  {id:"after",   label:"After-Market",       short:"AFTER",  color:"#C9A84C", bg:"rgba(201,168,76,.08)", border:"rgba(201,168,76,.25)",   start:25*60+0,  end:25*60+30},
];
...
function useMarket(){
  ...
  /* ── Camada: Minimum Activity Nudge ────────────────────────────────
     Garante que nenhum ativo fique mais de 5 minutos sem variar ao menos
     FS$0,01. A cada tick verifica quanto tempo o preco está estático;
     após 150 ticks consecutivos sem mudança (= 300s = 5min) injeta um
     micro-choque de ±0,01 na direção do FV (se FV > P → +0.01, senão −0.01).
     Não conta durante Circuit Breaker (preco intencionalmente congelado). */
  const lastMovedAt=useRef({}); // {ticker: tickCount} — ticks desde ultima variacao
  const NUDGE_TICKS=150; // 150 × 2s = 300s = 5 minutos
  ...
  useEffect(()=>{
    const t=setInterval(()=>{
      ...
      CLUBS.forEach(c=>{
        ...
        /* ── Minimum Activity Nudge ──────────────────────────────────
           Conta ticks consecutivos sem variacao. Se >= NUDGE_TICKS e
           preco nao se moveu, injeta ±0.01 na direcao do Fair Value.
           So age fora de Circuit Breaker e durante sessao ativa.      */
        let nudgedNp = np;
        if(!rCb.current[c.ticker]){
          if(np===P){
            // Preco nao variou neste tick — incrementa contador
            lastMovedAt.current[c.ticker]=(lastMovedAt.current[c.ticker]||0)+1;
            if(lastMovedAt.current[c.ticker]>=NUDGE_TICKS){
              // 5 minutos sem movimento: aplica micro-nudge na direcao do FV
              const nudgeDir=fairV>=P?1:-1;
              nudgedNp=Math.max(0.01, +(P+nudgeDir*0.01).toFixed(2));
              lastMovedAt.current[c.ticker]=0;
            }
          } else {
            // Preco se moveu — reset do contador
            lastMovedAt.current[c.ticker]=0;
          }
        }
        const finalNp=nudgedNp;
        const finalR=(finalNp-P)/P;
        newPrices[c.ticker]=finalNp;
        newV    [c.ticker]=garch(prevV,finalR);
        newBook [c.ticker]=bidAsk(finalNp,p,imbV);
        // M7: atualiza histórico de preços (circular, tamanho HISTORY_LEN)
        if(!priceHistory.current[c.ticker]){
          priceHistory.current[c.ticker]=Array(HISTORY_LEN).fill(finalNp);
        } else {
          priceHistory.current[c.ticker].push(finalNp);
...
function CandleChart({club, indicators=[], chartMode="candle", compareClubs=[], currentPrice=null, prevCloses={}, period="1D"}) {
  ...
  const FALLBACK_COLORS=["#D4B466","#34d399","#fb923c","#f472b6","#60a5fa"];
  ...
  return(
    <svg width="100%" viewBox={`0 0 ${W} ${TOTAL_H}`} style={{overflow:'visible'}}>
      ...
      {bollinger&&<>
        <path d={[bollinger.map((b,i)=>b?`${i===0||!bollinger[i-1]?'M':'L'}${cx(i).toFixed(1)},${toY(b.upper).toFixed(1)}`:'').filter(Boolean).join(' '),[...bollinger].reverse().map((b,i,arr)=>{const oi=arr.length-1-i;return b?`L${cx(oi).toFixed(1)},${toY(b.lower).toFixed(1)}`:''}).filter(Boolean).join(' '),'Z'].join(' ')} fill="rgba(201,168,76,.05)" stroke="none"/>
        <path d={bbPath('upper')} fill="none" stroke="rgba(201,168,76,.5)" strokeWidth="1" strokeDasharray="4 2"/>
        <path d={bbPath('mid')}   fill="none" stroke="rgba(245,166,35,.7)"  strokeWidth="1.2"/>
        <path d={bbPath('lower')} fill="none" stroke="rgba(201,168,76,.5)" strokeWidth="1" strokeDasharray="4 2"/>
      </>}
      {isLine&&!isCompare&&<>
      ...
      {mm21&&<path d={mmPath(mm21)} fill="none" stroke="#D4B466" strokeWidth="1.4" strokeLinejoin="round" strokeLinecap="round" opacity=".85"/>}
      ...
      {(bollinger||mm9||mm21)&&!isCompare&&<g>
        {bollinger&&<>
          ...
          <line x1={PX+3} x2={PX+10} y1={TOTAL_H-10} y2={TOTAL_H-10} stroke="rgba(201,168,76,.6)" strokeWidth="1" strokeDasharray="3 1"/>
          <text x={PX+13} y={TOTAL_H-7} fontSize="6" fill="rgba(201,168,76,.85)" fontFamily="monospace">Bollinger</text>
        ...
        {mm21&&<>
          <line x1={bollinger?(mm9?PX+126:PX+96):(mm9?PX+30:PX)} x2={bollinger?(mm9?PX+133:PX+103):(mm9?PX+37:PX+7)} y1={TOTAL_H-10} y2={TOTAL_H-10} stroke="#D4B466" strokeWidth="1.4"/>
          <text x={bollinger?(mm9?PX+136:PX+106):(mm9?PX+40:PX+10)} y={TOTAL_H-7} fontSize="6" fill="#D4B466" fontFamily="monospace" fontWeight="bold">MM21</text>
        </>}
      </g>}
...
function StatusBar({time}){
  ...
  return <div style={{height:44,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",flexShrink:0}}>
    <span style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{h}:{m}</span>
...
function PhoneShell({children,time}){
  return <div style={{minHeight:"100vh",background:"#050505",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px 0"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600;700;800;900&family=Inter:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;700&display=swap');@keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}@keyframes slideUp{from{opacity:0;transform:translateY(50px)}to{opacity:1;transform:translateY(0)}}@keyframes pls{0%,100%{transform:scale(1)}50%{transform:scale(1.06)}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes shimmer{0%{left:-100%}100%{left:200%}}@keyframes goldPulse{0%,100%{box-shadow:0 0 0 0 rgba(201,168,76,0.25)}50%{box-shadow:0 0 0 8px rgba(201,168,76,0)}}_::-webkit-scrollbar{width:0}*{-webkit-font-smoothing:antialiased}`}</style>
    <div style={{width:390,height:844,borderRadius:50,background:"#080808",border:"10px solid #1a1510",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 0 0 1px rgba(201,168,76,.18),0 40px 100px rgba(0,0,0,.98),0 0 60px rgba(201,168,76,.05),inset 0 0 0 1px rgba(201,168,76,.06)"}}>
      <StatusBar time={time}/>
      <InfoProvider>
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>{children}</div>
      </InfoProvider>
      <div style={{height:30,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><div style={{width:120,height:4,background:"rgba(201,168,76,.25)",borderRadius:3}}/></div>
    </div>
  </div>;
}

function ExitBtn({onExit}){return <button onClick={onExit} style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",color:"rgba(201,168,76,.5)",borderRadius:8,padding:"5px 11px",fontSize:10,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:600,letterSpacing:"0.2px",transition:"all .15s"}}>✕</button>;}

function Avatar({user,size=40,fontSize=15}){
  const ini=user.name.split(" ").slice(0,2).map(n=>n[0]).join("");
  return <div style={{width:size,height:size,borderRadius:"50%",background:"linear-gradient(135deg,#C9A84C,#8a6820)",display:"flex",alignItems:"center",justifyContent:"center",fontSize,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,flexShrink:0,boxShadow:"0 0 0 2px rgba(201,168,76,.25)"}}>{ini}</div>;
}

/* ── SPLASH ── */
function SplashScreen({onNext}){
  useEffect(()=>{const t=setTimeout(onNext,3000);return()=>clearTimeout(t);},[]);
  return <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:BG,position:"relative",overflow:"hidden"}}>
    ...
    <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 60% 45% at 50% 35%,rgba(201,168,76,.08) 0%,transparent 70%)",pointerEvents:"none"}}/>
    {/* Bottom glow */}
    <div style={{position:"absolute",bottom:"-25%",left:"50%",transform:"translateX(-50%)",width:360,height:360,background:"radial-gradient(circle,rgba(201,168,76,.04),transparent 65%)",borderRadius:"50%",pointerEvents:"none"}}/>
    {/* Decorative top line */}
    <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:1,height:"35%",background:"linear-gradient(to bottom,transparent,rgba(201,168,76,.3),transparent)",pointerEvents:"none"}}/>
    <div style={{width:90,height:90,borderRadius:26,background:"linear-gradient(145deg,#1a1410,#0e0c09)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:28,animation:"pls 2.4s ease-in-out infinite",boxShadow:"0 0 0 1px rgba(201,168,76,.3),0 20px 60px rgba(201,168,76,.18),0 0 80px rgba(201,168,76,.06)"}}>
      <FootStockLogo size={66} rounded={false}/>
    </div>
    <div style={{fontSize:32,fontWeight:800,color:"#F0EAD6",letterSpacing:"-0.5px",fontFamily:SANS,display:"flex",alignItems:"baseline",gap:9}}>
      FootStock
      <span style={{fontSize:9,fontWeight:700,color:GOLD,letterSpacing:"3px",fontFamily:SANS_BODY,opacity:.8}}>BETA</span>
    </div>
    <div style={{fontSize:9,color:"rgba(201,168,76,.7)",fontWeight:600,letterSpacing:"5px",marginTop:10,fontFamily:SANS_BODY,textTransform:"uppercase"}}>A Bolsa do Futebol</div>
    {/* Divider line */}
    <div style={{width:48,height:1,background:"linear-gradient(90deg,transparent,rgba(201,168,76,.5),transparent)",margin:"28px 0 24px"}}/>
    <div style={{display:"flex",gap:6}}>
      {[0,1,2].map(i=><div key={i} style={{width:i===0?22:6,height:4,borderRadius:2,background:i===0?"rgba(201,168,76,.8)":"rgba(255,255,255,.12)",transition:"all .3s"}}/>)}
    </div>
  </div>;
}
...
function OnboardingScreen({onSelect,onExit}){
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <div style={{padding:"8px 16px 0",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      <div style={{width:30,height:30,borderRadius:9,background:"linear-gradient(135deg,#1a1510,#0e0c09)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 12px rgba(201,168,76,.2)"}}><FootStockLogo size={24} rounded={false}/></div>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"0 16px 80px"}}>
      ...
      <div style={{fontSize:12,color:MUTED,fontFamily:SANS_BODY,marginBottom:22}}>Vamos personalizar sua experiência.</div>
      {opts.map(o=><div key={o.id} onClick={()=>setSel(o.id)} style={{background:sel===o.id?"rgba(201,168,76,.08)":CARD,border:`1.5px solid ${sel===o.id?"rgba(201,168,76,.6)":BORDER}`,borderRadius:16,padding:"16px",marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:14,transition:"all .2s"}}>
        <div style={{fontSize:26}}>{o.ico}</div>
        <div><div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{o.t}</div><div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY,marginTop:2}}>{o.d}</div></div>
        <div style={{marginLeft:"auto",width:20,height:20,borderRadius:"50%",border:`2px solid ${sel===o.id?"#C9A84C":BORDER}`,background:sel===o.id?"#C9A84C":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>{sel===o.id&&<div style={{width:8,height:8,borderRadius:"50%",background:BG}}/>}</div>
      </div>)}
    </div>
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"12px 16px 16px",background:`linear-gradient(to top,${BG} 60%,transparent)`}}>
      <button onClick={()=>sel&&onSelect()} style={{width:"100%",padding:"14px",borderRadius:14,border:"none",cursor:sel?"pointer":"not-allowed",background:sel?"linear-gradient(135deg,#C9A84C,#8a6820)":"rgba(255,255,255,.06)",color:sel?"#fff":"rgba(255,255,255,.3)",fontSize:14,fontWeight:700,fontFamily:SANS_BODY}}>Continuar →</button>
    </div>
  </div>;
}
...
function LoginScreen({onLogin,onRegister,onExit}){
  ...
  const iS=(err,extra={})=>({width:"100%",background:"rgba(201,168,76,.04)",border:`1.5px solid ${err?"rgba(244,63,94,.5)":"rgba(201,168,76,.12)"}`,borderRadius:12,padding:"13px 16px",color:"#fff",fontSize:13,fontFamily:SANS_BODY,outline:"none",boxSizing:"border-box",caretColor:"#C9A84C",...extra});
  const lbl=(t)=><div style={{fontSize:9,color:"rgba(255,255,255,.45)",fontWeight:600,letterSpacing:"0.8px",fontFamily:SANS_BODY,marginBottom:5,textTransform:"uppercase"}}>{t}</div>;
  ...
  const Checkbox=({checked,onChange,children,err})=>(
    <div onClick={onChange} style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:14,cursor:"pointer"}}>
      ...
      <div style={{fontSize:11,color:err?"rgba(244,63,94,.9)":"rgba(255,255,255,.5)",fontFamily:SANS_BODY,lineHeight:1.6,paddingTop:3}}>{children}</div>
    </div>
  );

  if(mode==="login") return <div style={{flex:1,display:"flex",flexDirection:"column",background:"#080808",overflow:"hidden"}}>
    {/* ── HERO HEADER ── */}
    <div style={{position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#161008 0%,#0f0d08 55%,#080808 100%)",padding:"28px 20px 26px",flexShrink:0}}>
      <div style={{position:"absolute",top:-50,right:-40,width:200,height:200,background:"radial-gradient(circle,rgba(201,168,76,.18) 0%,transparent 65%)",borderRadius:"50%",pointerEvents:"none"}}/>
      <div style={{position:"absolute",bottom:-30,left:-20,width:150,height:150,background:"radial-gradient(circle,rgba(212,180,102,.1) 0%,transparent 65%)",borderRadius:"50%",pointerEvents:"none"}}/>
      <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",opacity:.05,pointerEvents:"none"}} viewBox="0 0 360 190" preserveAspectRatio="none">
        {[32,64,96,128,160].map(y=><line key={y} x1="0" y1={y} x2="360" y2={y} stroke="#C9A84C" strokeWidth="0.6"/>)}
        {[60,120,180,240,300].map(x=><line key={x} x1={x} y1="0" x2={x} y2="190" stroke="#C9A84C" strokeWidth="0.6"/>)}
      </svg>
      ...
      <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:24}}>
        <div style={{width:46,height:46,borderRadius:15,background:"linear-gradient(135deg,rgba(201,168,76,.18),rgba(212,180,102,.12))",border:"1px solid rgba(201,168,76,.3)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 28px rgba(201,168,76,.2)"}}>
          <FootStockLogo size={30} rounded={false}/>
        </div>
        <div>
          <div style={{fontSize:19,fontWeight:700,color:"#fff",letterSpacing:"-0.5px",fontFamily:SANS_BODY,lineHeight:1,display:"flex",alignItems:"baseline",gap:5}}>FootStock<span style={{fontSize:9,fontWeight:900,color:GOLD,letterSpacing:"2px",fontFamily:SANS_BODY}}>BETA</span></div>
          <div style={{fontSize:8,color:ACCENT,fontWeight:600,letterSpacing:"3px",marginTop:3,fontFamily:SANS_BODY,opacity:.9}}>A BOLSA DO FUTEBOL</div>
        </div>
      </div>
      {/* Headline + chart deco */}
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:11,color:"rgba(255,255,255,.38)",fontFamily:SANS_BODY,marginBottom:5,letterSpacing:"0.3px"}}>Bem-vindo de volta</div>
          <div style={{fontSize:28,fontWeight:900,color:"#fff",letterSpacing:"-1.2px",fontFamily:SANS,lineHeight:1.05}}>Acesse sua<br/><span style={{background:"linear-gradient(90deg,#C9A84C,#D4B466)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>conta</span></div>
        </div>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"22px 20px 100px"}}>
      ...
      {error&&<div style={{fontSize:10,color:"#f87171",fontFamily:SANS_BODY,marginBottom:16,display:"flex",alignItems:"center",gap:8,background:"rgba(244,63,94,.09)",border:"1px solid rgba(244,63,94,.22)",borderRadius:11,padding:"8px 13px"}}>⚠ {error}</div>}

      <div style={{textAlign:"center",fontSize:12,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY}}>
        Não tem conta?{" "}
        <span onClick={()=>{setMode("register");setError("");}} style={{color:ACCENT,cursor:"pointer",fontWeight:600,borderBottom:`1px solid rgba(201,168,76,.3)`,paddingBottom:1}}>Criar conta grátis</span>
      </div>
      {/* Market Hours */}
      {(()=>{
        const sess=getMarketSession();
        const sessions=[
          ...
          {id:"main",    label:"Negociação",   color:"#C9A84C", start:"11:00", end:"00:45"},
          {id:"closing", label:"Leilão",       color:"#D4B466", start:"00:45", end:"01:00"},
          {id:"after",   label:"After-Market", color:"#C9A84C", start:"01:00", end:"01:30"},
        ];
        return(
          <div style={{background:"rgba(255,255,255,.03)",border:`1px solid ${sess.id==="closed"?"rgba(244,63,94,.2)":"rgba(201,168,76,.18)"}`,borderRadius:13,padding:"11px 14px",marginTop:22}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}>
              <div style={{fontSize:8,fontWeight:700,color:"rgba(255,255,255,.35)",letterSpacing:"1.5px",fontFamily:SANS_BODY}}>HORÁRIO DO MERCADO · BRASÍLIA</div>
              <div style={{display:"flex",alignItems:"center",gap:5}}>
                <div style={{width:6,height:6,borderRadius:"50%",background:sess.color,boxShadow:`0 0 7px ${sess.color}`}}/>
                <span style={{fontSize:8,fontWeight:800,color:sess.color,fontFamily:SANS_BODY,letterSpacing:"1px"}}>{sess.short}</span>
              </div>
            </div>
              ...
              {sessions.map(s=>{
                ...
                return(
                  <div key={s.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",borderRadius:8,background:active?"rgba(255,255,255,.06)":"transparent",border:`1px solid ${active?"rgba(255,255,255,.1)":"transparent"}`}}>
                    ...
                    <span style={{fontSize:10,fontWeight:active?700:400,color:active?"#fff":"rgba(255,255,255,.3)",fontFamily:SANS_BODY,flex:1}}>{s.label}</span>
            ...
            {sess.id==="closed"&&<div style={{marginTop:8,fontSize:9,color:"rgba(244,63,94,.55)",fontFamily:SANS_BODY,textAlign:"center",fontStyle:"italic"}}>Mercado fechado · Reabre às 10:45</div>}
          </div>
        );
      })()}
    </div>
    ...
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"12px 20px 22px",background:"linear-gradient(to top,#07090f 70%,transparent)"}}>
      <button onClick={handleLogin} style={{width:"100%",padding:"17px",borderRadius:20,border:"none",cursor:"pointer",background:"linear-gradient(135deg,#C9A84C 0%,#8a6820 100%)",color:"#fff",fontSize:15,fontWeight:700,fontFamily:SANS_BODY,letterSpacing:"0.3px",boxShadow:"0 8px 32px rgba(201,168,76,.3)"}}>
        Acessar →
      </button>
    </div>
  </div>;
  ...
  // ── REGISTER ──
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:"#080808",overflow:"hidden"}}>
    <div style={{position:"relative",background:"linear-gradient(160deg,#161008,#0d0b08)",padding:"18px 20px 16px",flexShrink:0,overflow:"hidden"}}>
      <div style={{position:"absolute",top:-30,right:-30,width:130,height:130,background:"radial-gradient(circle,rgba(201,168,76,.10),transparent 70%)",borderRadius:"50%",pointerEvents:"none"}}/>
      <div style={{position:"absolute",top:12,right:14}}><ExitBtn onExit={onExit}/></div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        <div style={{width:38,height:38,borderRadius:12,background:"rgba(201,168,76,.1)",border:"1px solid rgba(201,168,76,.15)",display:"flex",alignItems:"center",justifyContent:"center"}}><FootStockLogo size={24} rounded={false}/></div>
        <div>
          <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Criar conta</div>
          <div style={{fontSize:8,color:ACCENT,fontWeight:700,letterSpacing:"1.8px",fontFamily:SANS_BODY}}>FOOTSTOCK · GRATUITO</div>
        </div>
      </div>
      <div style={{display:"flex",gap:4}}>
        {["Dados Pessoais","Acesso","Time","Aceite"].map((s,i)=>(
          <div key={s} style={{flex:1,height:3,borderRadius:2,background:i===0?"rgba(201,168,76,.75)":i===1?(rEmail&&rPass&&rPassConf?"rgba(201,168,76,.4)":"rgba(255,255,255,.1)"):i===2?(rTime?"rgba(201,168,76,.4)":"rgba(255,255,255,.1)"):(rIdade&&rPrivacy?"rgba(201,168,76,.4)":"rgba(255,255,255,.1)")}}/>
        ))}
      </div>
    </div>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"18px 20px 100px"}}>
      ...
      <div style={{marginBottom:14}}>
        ...
        {rErrors.passConf&&<div style={{fontSize:10,color:"#f87171",marginTop:4,fontFamily:SANS_BODY}}>⚠ As senhas não coincidem</div>}
        {rPassConf&&rPass&&rPassConf===rPass&&!rErrors.passConf&&<div style={{fontSize:10,color:ACCENT,marginTop:4,fontFamily:SANS_BODY}}>✓ Senhas conferem</div>}
      </div>
      ...
      <div style={{marginBottom:20}}>
        ...
        {rTime&&(()=>{const cl=CLUBS.find(c=>c.ticker===rTime);return cl?<div style={{marginTop:8,display:"flex",alignItems:"center",gap:9,padding:"7px 11px",background:`${cl.color}15`,border:`1px solid ${cl.color}38`,borderRadius:11}}>
          ...
          <span style={{fontSize:11,color:"#fff",fontFamily:SANS_BODY,fontWeight:700}}>{cl.realName||cl.name}</span>
      ...
      {Object.values(rErrors).some(Boolean)&&<div style={{fontSize:10,color:"#f87171",fontFamily:SANS_BODY,marginBottom:10,background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.2)",borderRadius:11,padding:"8px 13px"}}>⚠ Preencha todos os campos obrigatórios</div>}
      <div style={{textAlign:"center",fontSize:12,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY}}>
        Já tem conta?{" "}
        <span onClick={()=>{setMode("login");setRErrors({});}} style={{color:ACCENT,cursor:"pointer",fontWeight:600,borderBottom:`1px solid rgba(201,168,76,.3)`,paddingBottom:1}}>Entrar</span>
      </div>
    </div>
    ...
    {showPrivacy&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.8)",zIndex:50,display:"flex",alignItems:"flex-end",backdropFilter:"blur(4px)"}}>
      <div style={{background:"#0f1923",borderRadius:"22px 22px 0 0",padding:"20px 20px 28px",width:"100%",maxHeight:"72%",overflowY:"auto",boxSizing:"border-box"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div style={{fontSize:15,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Política de Privacidade</div>
        ...
        <div style={{fontSize:11,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,lineHeight:1.9}}>
        ...
        <button onClick={()=>{setShowPrivacy(false);setRPrivacy(true);setRErrors(p=>({...p,privacy:false}));}}
          style={{width:"100%",marginTop:18,padding:"15px",borderRadius:18,border:"none",background:"linear-gradient(135deg,#C9A84C,#8a6820)",color:"#fff",fontSize:14,fontWeight:700,fontFamily:SANS_BODY,cursor:"pointer",boxShadow:"0 6px 24px rgba(201,168,76,.3)"}}>
          Entendi e Aceito ✓
        </button>
      </div>
    </div>}
    ...
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"12px 20px 22px",background:"linear-gradient(to top,#07090f 70%,transparent)"}}>
      <button onClick={handleRegister} style={{width:"100%",padding:"17px",borderRadius:20,border:"none",cursor:"pointer",background:"linear-gradient(135deg,#C9A84C 0%,#8a6820 100%)",color:"#fff",fontSize:15,fontWeight:700,fontFamily:SANS_BODY,boxShadow:"0 8px 32px rgba(201,168,76,.3)"}}>
        Criar conta grátis →
      </button>
    </div>
  </div>;
...
function PlansScreen({onSelect,onExit}){
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <div style={{padding:"8px 16px 0",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
      <div><div style={{fontSize:15,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Escolha seu Plano</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>Modalidade de pagamento na próxima etapa</div></div>
      <ExitBtn onExit={onExit}/>
    </div>
    {/* aviso de desconto à vista */}
    <div style={{margin:"4px 16px 0",background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",borderRadius:10,padding:"7px 12px",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
      <span style={{fontSize:14}}>💰</span>
      <span style={{fontSize:10,color:"rgba(255,255,255,.55)",fontFamily:SANS_BODY}}>Pague <strong style={{color:ACCENT}}>à vista anual</strong> e economize <strong style={{color:ACCENT}}>25%</strong> — escolha na tela de pagamento.</span>
    </div>
    <div style={{flex:1,overflowY:"auto",padding:"10px 14px 80px"}}>
      {PLANS.map(p=><div key={p.id} onClick={()=>setSel(p.id)} style={{background:sel===p.id?`${p.color}09`:CARD,border:`1.5px solid ${sel===p.id?p.color:BORDER}`,borderRadius:18,padding:"14px",marginBottom:10,cursor:"pointer",transition:"all .2s",position:"relative"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:15,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{p.name}</div>
            {p.price===0
              ? <div style={{fontSize:15,fontWeight:700,color:p.color,fontFamily:SANS_BODY}}>Grátis</div>
              : <div style={{display:"flex",alignItems:"baseline",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:15,fontWeight:700,color:p.color,fontFamily:SANS_BODY}}>R${p.price.toFixed(2).replace(".",",")}/mês</span>
                  <span style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>ou R${p.priceA.toFixed(2).replace(".",",")} à vista</span>
                </div>
            }
          </div>
        ...
        {p.features.map(f=><div key={f} style={{fontSize:10.5,color:"rgba(255,255,255,.8)",fontFamily:SANS_BODY,padding:"3.5px 0",display:"flex",alignItems:"center",gap:7}}><span style={{color:p.color}}>✓</span>{f}</div>)}
        {p.locked.map(f=><div key={f} style={{fontSize:10.5,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY,padding:"3.5px 0",display:"flex",alignItems:"center",gap:7}}><span>🔒</span>{f}</div>)}
      </div>)}
    </div>
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"12px 16px 16px",background:`linear-gradient(to top,${BG} 65%,transparent)`}}>
      <button onClick={()=>onSelect(sel)} style={{width:"100%",padding:"14px",borderRadius:14,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${chosen.color},${chosen.id==="lenda"?"#d97706":chosen.id==="craque"?"#D4B466":"#7a8ba8"})`,color:chosen.id==="jogador"?TEXT:BG,fontSize:14,fontWeight:800,fontFamily:SANS_BODY}}>
        {chosen.cta}
      </button>
    </div>
  </div>;
...
function PaymentScreen({plan,onSuccess,onBack,onExit,isUpgrade=false,previousPlan=null,billing="monthly"}){
  ...
  if(step==="done"){
    ...
    return <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:BG,padding:"0 24px",textAlign:"center"}}>
      <div style={{width:80,height:80,borderRadius:"50%",background:`linear-gradient(135deg,${pc},${plan==="lenda"?"#d97706":"#D4B466"})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,marginBottom:24,boxShadow:`0 0 50px ${pc}50`,animation:"pls 1.5s ease-in-out 1"}}>✓</div>
      <div style={{fontSize:10,color:pc,fontWeight:700,letterSpacing:"2px",fontFamily:SANS_BODY,marginBottom:8}}>PAGAMENTO CONFIRMADO</div>
      <div style={{fontSize:22,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,letterSpacing:"-0.5px",marginBottom:6}}>{isUpgrade?"Upgrade realizado!":"Bem-vindo ao Plano"} {pLabel}! <PlanIcon icon={pIconKey} size={22} color={pc}/></div>
      {isUpgrade&&previousPlan&&<div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY,marginBottom:8}}>{previousPlan} → <span style={{color:pc,fontWeight:700}}>{pLabel}</span></div>}
      <div style={{background:`${pc}12`,border:`1px solid ${pc}30`,borderRadius:14,padding:"12px 16px",marginBottom:20,width:"100%",boxSizing:"border-box",textAlign:"left"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>Modalidade</span>
          <span style={{fontSize:10,fontWeight:700,color:pc,fontFamily:MONO}}>{modality==="vista"?"À vista (anual)":"Parcelado (mensal)"}</span>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>Gateway</span>
          <span style={{fontSize:10,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{GATEWAYS.find(g=>g.id===gateway)?.label||"—"}</span>
        </div>
        {isUpgrade&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
          <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>Ativação imediata</span>
          <span style={{fontSize:10,fontWeight:700,color:pc,fontFamily:MONO}}>{upgradeDate}</span>
        </div>}
        <div style={{display:"flex",justifyContent:"space-between"}}>
          <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>{confirmedBilling==="annual"?"Renovação anual em":"Próxima cobrança em"}</span>
          <span style={{fontSize:10,fontWeight:700,color:"#fff",fontFamily:MONO}}>{nextBilling}</span>
        </div>
      </div>
      <button onClick={()=>onSuccess(plan,confirmedBilling)} style={{width:"100%",padding:"15px",borderRadius:14,border:"none",cursor:"pointer",fontFamily:SANS_BODY,fontWeight:900,fontSize:14,background:`linear-gradient(135deg,${pc},${plan==="lenda"?"#e8830a":ACCENT2})`,color:BG}}>{isUpgrade?"Continuar no FootStock →":"Acessar o FootStock →"}</button>
    </div>;
  }
  ...
  if(step==="waiting"){
    ...
    return <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:BG,padding:"0 28px",textAlign:"center",gap:0}}>
      ...
      <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,marginBottom:8}}>Aguardando confirmação</div>
      <div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.7,marginBottom:24}}>
  ...
  if(step==="redirecting"){
    ...
    return <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:BG,padding:"0 28px",textAlign:"center",gap:16}}>
      <div style={{width:64,height:64,borderRadius:18,background:gw?.color||pc,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:900,color:"#fff",fontFamily:SANS_BODY,boxShadow:`0 0 32px ${gw?.color||pc}60`}}>{gw?.logo}</div>
      <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Redirecionando para {gw?.label}</div>
      <div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>Você será devolvido ao FootStock<br/>após concluir o pagamento.</div>
    </div>;
  }
  ...
  if(step==="gateway"){
    ...
    return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
      <div style={{padding:"8px 16px 0",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        <button onClick={()=>setStep("modality")} style={{background:"none",border:"none",color:MUTED,fontSize:11,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:700,padding:0}}>← Voltar</button>
        <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Escolher Gateway</div>
        <ExitBtn onExit={onExit}/>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"0 16px 100px"}}>
        ...
        <div style={{background:`${pc}10`,border:`1px solid ${pc}30`,borderRadius:14,padding:"12px 14px",marginBottom:20,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Plano {pLabel} · {isVista?"À vista":"Parcelado"}</div>
            <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{isVista?`R$${total.toFixed(2).replace(".",",")} /ano (12× R$${price.toFixed(2).replace(".",",")})`:`R$${price.toFixed(2).replace(".",",")} /mês`}</div>
          </div>
          <div style={{fontSize:15,fontWeight:900,color:pc,fontFamily:SANS_BODY}}>R${price.toFixed(2).replace(".",",")}</div>
        </div>
        <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginBottom:12,letterSpacing:"1px",fontWeight:700}}>SELECIONE O GATEWAY DE PAGAMENTO</div>
        {GATEWAYS.map(gw=>(
          <div key={gw.id} onClick={()=>setGateway(gw.id)}
            ...
            <div style={{width:42,height:42,borderRadius:12,background:gw.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:900,color:"#fff",fontFamily:SANS_BODY,flexShrink:0}}>{gw.logo}</div>
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{gw.label}</div>
              <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>{gw.sub}</div>
            </div>
        ...
        <div style={{fontSize:9,color:"rgba(255,255,255,.2)",fontFamily:SANS_BODY,textAlign:"center",marginTop:8,lineHeight:1.6}}>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"10px 16px 16px",background:`linear-gradient(to top,${BG} 65%,transparent)`}}>
        <button onClick={()=>{if(!gateway)return;redirectToGateway();}}
          style={{width:"100%",padding:"15px",borderRadius:14,border:"none",cursor:gateway?"pointer":"not-allowed",background:gateway?`linear-gradient(135deg,${pc},${plan==="lenda"?"#e8830a":ACCENT2})`:SURFACE,color:gateway?BG:MUTED,fontSize:13,fontWeight:800,fontFamily:SANS_BODY,transition:"all .25s"}}>
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <div style={{padding:"8px 16px 0",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:MUTED,fontSize:11,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:700,padding:0}}>← Planos</button>
      <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Pagamento</div>
      <ExitBtn onExit={onExit}/>
    </div>
    <div style={{flex:1,overflowY:"auto",padding:"0 16px 100px"}}>
      {/* card do plano */}
      <div style={{background:`${pc}10`,border:`1px solid ${pc}35`,borderRadius:18,padding:"14px",marginBottom:22,display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{flex:1}}>
          <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,letterSpacing:"0.5px"}}>ASSINATURA SELECIONADA</div>
          <div style={{fontSize:16,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Plano {pLabel}</div>
        </div>
      </div>
      {/* modalidades */}
      <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginBottom:10,letterSpacing:"1px",fontWeight:700}}>ESCOLHA A MODALIDADE</div>
      {/* À VISTA */}
      <div onClick={()=>setModality("vista")}
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          ...
          <div style={{fontSize:13,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>À vista · Anual</div>
        </div>
        <div style={{paddingLeft:30}}>
          <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:4}}>
            <span style={{fontSize:22,fontWeight:900,color:pc,fontFamily:SANS_BODY}}>R${pr.vista.toFixed(2).replace(".",",")}</span>
            <span style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>/mês</span>
            <span style={{fontSize:10,color:"rgba(255,255,255,.3)",textDecoration:"line-through",fontFamily:SANS_BODY}}>R${pr.full.toFixed(2).replace(".",",")}</span>
          </div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>
            Cobrado à vista: <span style={{color:"#fff",fontWeight:700}}>R${(pr.vista*12).toFixed(2).replace(".",",")}/ano</span>
          </div>
          <div style={{fontSize:9,color:"rgba(255,255,255,.3)",fontFamily:SANS_BODY,marginTop:3}}>Renovação automática em 12 meses</div>
        </div>
      </div>
      {/* PARCELADO */}
      <div onClick={()=>setModality("parcelado")}
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          ...
          <div style={{fontSize:13,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Parcelado · Mensal</div>
        </div>
        <div style={{paddingLeft:30}}>
          <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:4}}>
            <span style={{fontSize:22,fontWeight:900,color:modality==="parcelado"?pc:MUTED,fontFamily:SANS_BODY}}>R${pr.full.toFixed(2).replace(".",",")}</span>
            <span style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>/mês</span>
          </div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>
            Sem desconto · <span style={{color:"#fff",fontWeight:700}}>sem fidelidade</span>
          </div>
          <div style={{fontSize:9,color:"rgba(255,255,255,.3)",fontFamily:SANS_BODY,marginTop:3}}>Cancele quando quiser</div>
        </div>
      </div>
      {/* aviso transparência */}
      <div style={{background:"rgba(255,255,255,.03)",border:`1px solid ${BORDER}`,borderRadius:12,padding:"10px 13px",display:"flex",gap:9,alignItems:"flex-start"}}>
        ...
        <div style={{fontSize:9,color:"rgba(255,255,255,.35)",fontFamily:SANS_BODY,lineHeight:1.7}}>
    ...
    <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"10px 16px 16px",background:`linear-gradient(to top,${BG} 65%,transparent)`}}>
      <button onClick={()=>{if(!modality)return;setStep("gateway");}}
        style={{width:"100%",padding:"15px",borderRadius:14,border:"none",cursor:modality?"pointer":"not-allowed",background:modality?`linear-gradient(135deg,${pc},${plan==="lenda"?"#e8830a":ACCENT2})`:SURFACE,color:modality?BG:MUTED,fontSize:13,fontWeight:800,fontFamily:SANS_BODY,transition:"all .25s"}}>
...
function AdminPanel({onBack,onExit,role}){
  ...
  const btn2=(c,fg="#fff")=>({background:c,border:"none",borderRadius:8,padding:"5px 10px",color:fg,fontSize:9,fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer"});
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <div style={{padding:"10px 14px 0",flexShrink:0,background:"linear-gradient(135deg,rgba(244,63,94,.06),rgba(201,168,76,.03))",borderBottom:`1px solid ${BORDER}`}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:MUTED,fontSize:11,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:700,padding:0}}>← App</button>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          ...
          <div><div style={{fontSize:12,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Painel Admin</div>
          <div style={{fontSize:7,color:RED,fontWeight:800,letterSpacing:"1.5px",fontFamily:SANS_BODY}}>{isAdmin?"ACESSO IRRESTRITO":"ACESSO RESTRITO"}</div></div>
        </div>
        <ExitBtn onExit={onExit}/>
      </div>
      {!isAdmin&&<div style={{background:"rgba(245,166,35,.08)",border:"1px solid rgba(245,166,35,.25)",borderRadius:10,padding:"7px 11px",marginBottom:10,display:"flex",alignItems:"center",gap:7}}>
        <span>🔒</span><span style={{fontSize:9,color:"#fde68a",fontFamily:SANS_BODY}}>Acesso restrito — apenas gestão de Usuários disponível.</span>
      </div>}
      <div style={{display:"flex",gap:4,overflowX:"auto",paddingBottom:10}}>
        {SECTIONS.map(s=><button key={s} onClick={()=>setTab(s)} style={{background:tab===s?`${RED}22`:SURFACE,border:`1px solid ${tab===s?RED:BORDER}`,borderRadius:20,padding:"5px 12px",color:tab===s?RED:MUTED,fontSize:9,fontWeight:tab===s?800:600,fontFamily:SANS_BODY,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{s}</button>)}
      </div>
    </div>
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,padding:24}}>
      <div style={{fontSize:40}}>🛡️</div>
      <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,textAlign:"center"}}>{tab}</div>
      <div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY,textAlign:"center",lineHeight:1.6}}>Esta seção está conectada ao banco de dados PostgreSQL e será carregada dinamicamente no ambiente de produção.</div>
      <div style={{background:"rgba(201,168,76,.05)",border:"1px solid rgba(201,168,76,.2)",borderRadius:12,padding:"12px 16px",width:"100%",maxWidth:280}}>
        <div style={{fontSize:8,color:ACCENT,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:6}}>ACESSO CONCEDIDO</div>
        <div style={{fontSize:10,color:TEXT,fontFamily:SANS_BODY}}>Usuário: <span style={{color:isAdmin?RED:ACCENT2,fontWeight:700}}>{isAdmin?"Admin (Irrestrito)":"Rodrigo (Usuários)"}</span></div>
      </div>
    </div>
  </div>;
}
...
/* ── LOGO DROPDOWN ── */
function LogoDropdown({user,onProfile,onExit,onAdmin,onClose,onGlossary}){
  ...
  const items=[
    ...
    {ico:"📖",label:"Glossário",    sub:"Dicionário de termos do mercado",  action:()=>{onGlossary();onClose();}, danger:false, admin:false, gold:true},
  ...
  return <>
    ...
    <div style={{position:"absolute",top:52,left:12,zIndex:50,width:230,background:CARD,border:`1px solid ${BORDER}`,borderRadius:18,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,.7)",animation:"fadeIn .18s ease"}}>
      <div style={{padding:"14px 14px 12px",background:"linear-gradient(135deg,rgba(201,168,76,.06),rgba(201,168,76,.03))",borderBottom:`1px solid ${BORDER}`,display:"flex",alignItems:"center",gap:10}}>
        <Avatar user={user} size={44} fontSize={16}/>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.name}</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.email}</div>
          <div style={{display:"flex",alignItems:"center",gap:5,marginTop:4,flexWrap:"wrap"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:4,background:`${user.planColor}20`,border:`1px solid ${user.planColor}50`,borderRadius:20,padding:"2px 7px"}}>
              ...
              <span style={{fontSize:9,fontWeight:800,color:user.planColor,fontFamily:SANS_BODY}}>PLANO {user.plan.toUpperCase()}</span>
            </div>
            {canAdmin&&<div style={{display:"inline-flex",alignItems:"center",gap:3,background:"rgba(244,63,94,.12)",border:"1px solid rgba(244,63,94,.25)",borderRadius:20,padding:"2px 7px"}}>
              <span style={{fontSize:9,fontWeight:800,color:RED,fontFamily:SANS_BODY}}>{user.role==="admin"?"🛡️ ADMIN":"👤 OPER"}</span>
            </div>}
          </div>
        </div>
      </div>
      <div style={{padding:"6px 0"}}>
        {items.map((item,i)=><button key={i} onClick={item.action} style={{width:"100%",background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:10,padding:"11px 14px",transition:"background .15s"}} onMouseEnter={e=>e.currentTarget.style.background=item.danger?"rgba(244,63,94,.07)":item.admin?"rgba(244,63,94,.05)":item.gold?"rgba(201,168,76,.07)":"rgba(255,255,255,.04)"} onMouseLeave={e=>e.currentTarget.style.background="none"}>
          <div style={{width:30,height:30,borderRadius:9,background:item.danger?"rgba(244,63,94,.1)":item.admin?"rgba(244,63,94,.08)":item.gold?"rgba(201,168,76,.1)":SURFACE,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,border:`1px solid ${item.danger?"rgba(244,63,94,.2)":item.admin?"rgba(244,63,94,.15)":item.gold?"rgba(201,168,76,.25)":BORDER}`,flexShrink:0}}>{item.ico}</div>
          <div style={{textAlign:"left"}}>
            <div style={{fontSize:12,fontWeight:700,color:item.danger?RED:item.admin?"#fda4af":item.gold?ACCENT:"#e2e8f0",fontFamily:SANS_BODY}}>{item.label}</div>
            <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>{item.sub}</div>
          </div>
...
/* ══════════════════════════════════════════════════════════════
   INFO TOOLTIP — Ícone ⓘ clicável que abre modal com definição
   do glossário. Uso: <InfoTip term="Preço IPO" />
   O modal é renderizado num portal no nível do PhoneShell via
   um contexto React leve (InfoContext).
══════════════════════════════════════════════════════════════ */
const InfoContext=createContext({show:()=>{}});

function InfoProvider({children}){
  const [modal,setModal]=useState(null); // {term, def, cat, emoji}
  const show=(termKey)=>{
    const found=GLOSSARY_TERMS.find(t=>
      t.term.toLowerCase()===termKey.toLowerCase()||
      t.term.toLowerCase().includes(termKey.toLowerCase())
    );
    if(found) setModal(found);
  };
  return <InfoContext.Provider value={{show}}>
    {children}
    {modal&&<div
      onClick={()=>setModal(null)}
      style={{position:"absolute",inset:0,zIndex:9999,display:"flex",alignItems:"flex-end",justifyContent:"center",background:"rgba(0,0,0,.65)",backdropFilter:"blur(4px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"linear-gradient(160deg,#161208,#0d0b08)",borderRadius:"22px 22px 0 0",padding:"6px 20px 30px",border:"1px solid rgba(201,168,76,.22)",borderBottom:"none",boxShadow:"0 -8px 40px rgba(0,0,0,.8)",animation:"slideUp .2s ease"}}>
        {/* Drag handle */}
        <div style={{width:36,height:4,borderRadius:2,background:"rgba(201,168,76,.25)",margin:"0 auto 16px"}}/>
        {/* Header */}
        <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:14}}>
          <div style={{width:42,height:42,borderRadius:13,background:"rgba(201,168,76,.1)",border:"1px solid rgba(201,168,76,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{modal.emoji}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,lineHeight:1.2,marginBottom:4}}>{modal.term}</div>
            <div style={{display:"inline-flex",alignItems:"center",gap:4,background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.2)",borderRadius:20,padding:"2px 8px"}}>
              <span style={{fontSize:8,fontWeight:700,color:ACCENT,fontFamily:SANS_BODY,letterSpacing:"0.5px"}}>{modal.cat.toUpperCase()}</span>
            </div>
          </div>
          <button onClick={()=>setModal(null)} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",color:"rgba(255,255,255,.5)",borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>✕</button>
        </div>
        {/* Divider */}
        <div style={{height:1,background:"linear-gradient(90deg,transparent,rgba(201,168,76,.3),transparent)",marginBottom:14}}/>
        {/* Definition */}
        <div style={{fontSize:12,color:"rgba(255,255,255,.78)",fontFamily:SANS_BODY,lineHeight:1.75}}>{modal.def}</div>
        {/* Footer hint */}
        <div style={{marginTop:16,display:"flex",alignItems:"center",gap:6,opacity:.55}}>
          <span style={{fontSize:10}}>📖</span>
          <span style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Ver todos os termos no Glossário</span>
        </div>
      </div>
    </div>}
...
/* Botão ⓘ inline — coloque ao lado de qualquer label */
function InfoTip({term,size=11}){
  const {show}=useContext(InfoContext);
  return <span
    onClick={e=>{e.stopPropagation();show(term);}}
    title={`Saiba mais: ${term}`}
    style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:size+3,height:size+3,borderRadius:"50%",background:"rgba(201,168,76,.15)",border:"1px solid rgba(201,168,76,.35)",color:ACCENT,fontSize:size-2,fontWeight:900,cursor:"pointer",flexShrink:0,lineHeight:1,fontFamily:SANS_BODY,userSelect:"none",transition:"background .15s",verticalAlign:"middle",marginLeft:4}}>i</span>;
}

/* ══════════════════════════════════════════════════════════════
   GLOSSÁRIO — Dicionário educativo de termos do mercado financeiro
   e do FootStock (valuation, pregão, ordens, indicadores, etc.)
══════════════════════════════════════════════════════════════ */
const GLOSSARY_TERMS=[
  /* ── VALUATION & FUNDAMENTOS ── */
  {cat:"Valuation & Fundamentos",emoji:"🏦",term:"IPO (Initial Public Offering)",def:"Oferta Pública Inicial. É a estreia de um ativo no mercado, quando as primeiras cotas são colocadas à venda ao público. No FootStock, o preço IPO é calculado com base na receita do clube, valor do plantel e valor de marca."},
  {cat:"Valuation & Fundamentos",emoji:"💰",term:"Preço IPO",def:"Preço de lançamento de cada ativo no FootStock. Calculado por uma fórmula multi-fator inspirada na metodologia Sports Value/KPMG: EV = (Receita × Múltiplo) + Plantel + Marca. Serve como âncora de referência histórica."},
  {cat:"Valuation & Fundamentos",emoji:"📊",term:"Equity Value",def:"Valor do patrimônio líquido do clube após deduzir a dívida operacional do EV bruto (Enterprise Value). Representa o que \"sobra\" para os acionistas se o clube fosse vendido hoje. Fórmula: Equity = EV − Dívida Líquida."},
  {cat:"Valuation & Fundamentos",emoji:"🌐",term:"Enterprise Value (EV)",def:"Valor total de uma empresa, considerando patrimônio + dívidas. No FootStock: EV = (Receita × Múltiplo EV/Receita) + Valor do Plantel + Valor de Marca. Representa o que custaria comprar o clube inteiro."},
  {cat:"Valuation & Fundamentos",emoji:"📈",term:"Market Cap (MKT Cap)",def:"Capitalização de mercado — valor total de todas as cotas em circulação multiplicado pelo preço atual. Reflete quanto o mercado \"vale\" o clube naquele momento: MKT Cap = Preço × Total de Cotas."},
  {cat:"Valuation & Fundamentos",emoji:"🔴",term:"MKT Cap Live",def:"Capitalização de mercado em tempo real, atualizada a cada tick do motor de preços. Oscila conforme o mercado negocia as cotas, diferentemente do valuation fundamental que muda apenas com notícias relevantes."},
  {cat:"Valuation & Fundamentos",emoji:"⚖️",term:"Fair Value (FV)",def:"Valor justo estimado para o ativo com base nos fundamentos do clube. O motor de preços tende a convergir para este valor ao longo do tempo (mean reversion). Notícias deslocam o FV temporariamente."},
  {cat:"Valuation & Fundamentos",emoji:"🏋️",term:"Múltiplo EV/Receita",def:"Quantas vezes a receita do clube vale em termos de Enterprise Value. Clubes maiores, com melhor governança e baixa dívida, têm múltiplos maiores (até 5×). Série B costuma ter múltiplos menores (0,8×–1,5×)."},
  {cat:"Valuation & Fundamentos",emoji:"👥",term:"Valor do Plantel",def:"Valor de mercado dos jogadores do clube (fonte: Transfermarkt). Entra diretamente no cálculo do EV. Um reforço caro eleva o EV; uma venda importante pode reduzi-lo."},
  {cat:"Valuation & Fundamentos",emoji:"🏷️",term:"Valor de Marca",def:"Quanto vale a marca do clube — torcida, história, visibilidade midiática (fonte: Sports Value). Flamengo e Palmeiras têm as maiores marcas do Brasil. Influencia diretamente o preço IPO e o Fair Value."},
  {cat:"Valuation & Fundamentos",emoji:"📉",term:"Dívida Operacional",def:"Dívidas do clube com fornecedores, bancos, Fisco e jogadores. Deduzida do EV para chegar ao Equity Value. Corinthians e Vasco são exemplos de clubes com alta dívida, o que comprime o preço dos ativos."},
  {cat:"Valuation & Fundamentos",emoji:"🔓",term:"Free Float (FF)",def:"Percentual do Equity disponível para negociação no mercado. No FootStock, varia de 15% (clubes com muita dívida) a 30% (boa governança e baixo endividamento). Determina quantas cotas existem no mercado."},
  /* ── MERCADO & PREGÃO ── */
  {cat:"Mercado & Pregão",emoji:"🟢",term:"Sessão de Negociação",def:"Período em que o mercado está aberto para compras e vendas. No FootStock (horário de Brasília): Pré-abertura 10h45–11h, Negociação 11h–00h45, Call de Fechamento 00h45–01h, After-Market 01h–01h30."},
  {cat:"Mercado & Pregão",emoji:"🔔",term:"Pré-abertura",def:"Fase anterior ao pregão principal, onde o mercado começa a se organizar. Ordens podem ser registradas mas não são executadas. Permite descoberta de preço antes da abertura oficial."},
  {cat:"Mercado & Pregão",emoji:"🔨",term:"Call de Fechamento",def:"Leilão de fechamento do pregão. As últimas ordens do dia são casadas num único preço de equilíbrio. Reduz volatilidade ao final do dia e produz o preço oficial de fechamento."},
  {cat:"Mercado & Pregão",emoji:"🌙",term:"After-Market",def:"Extensão do pregão após o fechamento oficial. Permite negociação com menor liquidez, geralmente para reagir a notícias publicadas após o fechamento. Volume tipicamente menor."},
  {cat:"Mercado & Pregão",emoji:"🔒",term:"Mercado Fechado",def:"Período sem negociações. As ordens ficam pendentes e o motor de preços não gera novos ticks. No FootStock, o mercado fecha às 01h30 e reabre às 10h45 do dia seguinte."},
  {cat:"Mercado & Pregão",emoji:"📋",term:"Book de Ofertas",def:"Lista de todas as ordens de compra (bids) e venda (asks) pendentes. A diferença entre o melhor bid e o melhor ask é o spread. O book revela a pressão compradora ou vendedora no momento."},
  {cat:"Mercado & Pregão",emoji:"💵",term:"Bid",def:"Melhor preço que um comprador está disposto a pagar por uma cota. É sempre menor que o Ask. Quando você vende no mercado, recebe o Bid."},
  {cat:"Mercado & Pregão",emoji:"💸",term:"Ask",def:"Melhor preço que um vendedor aceita para se desfazer de uma cota. É sempre maior que o Bid. Quando você compra no mercado, paga o Ask."},
  {cat:"Mercado & Pregão",emoji:"↔️",term:"Spread",def:"Diferença entre Ask e Bid. Quanto maior o spread, menor a liquidez do ativo. Ativos da Série B têm spreads maiores. No FootStock, varia de 0,3% (Flamengo/Palmeiras) a 1,4% (Série B ilíquidos)."},
  {cat:"Mercado & Pregão",emoji:"🔄",term:"Circuit Breaker",def:"Mecanismo de proteção que suspende negociações quando um ativo varia mais de 8% num único dia. Inspirado nas regras da B3. O ativo fica congelado por 5 minutos e retoma gradualmente."},
  {cat:"Mercado & Pregão",emoji:"💧",term:"Liquidez",def:"Facilidade de comprar ou vender um ativo sem impactar muito o preço. Ativos com alto float e muitos participantes têm alta liquidez. Série A top (Flamengo, Palmeiras) são os mais líquidos do FootStock."},
  {cat:"Mercado & Pregão",emoji:"📦",term:"Float (Oferta em Circulação)",def:"Quantidade total de cotas disponíveis para negociação no mercado. Diminui quando investidores compram e não vendem; aumenta quando vendem. Impacta diretamente a liquidez."},
  /* ── INDICADORES TÉCNICOS ── */
  {cat:"Indicadores Técnicos",emoji:"📉",term:"Variação Diária (%)",def:"Percentual de variação do preço desde a abertura do pregão (snap às 11h). Calculado como: (Preço Atual − Preço de Abertura) / Preço de Abertura × 100. É zerado a cada novo pregão."},
  {cat:"Indicadores Técnicos",emoji:"🕯️",term:"Candlestick (Vela)",def:"Representação gráfica de preço em um período. Cada vela tem: Abertura (open), Fechamento (close), Máxima (high) e Mínima (low). Verde = fechou acima da abertura; Vermelho = fechou abaixo."},
  {cat:"Indicadores Técnicos",emoji:"📏",term:"Média Móvel (MM)",def:"Média dos preços de fechamento nos últimos N períodos. Suaviza o ruído de curto prazo e revela tendências. No FootStock, disponíveis MM9 (curta) e MM21 (longa). Cruzamentos indicam mudança de tendência."},
  {cat:"Indicadores Técnicos",emoji:"📐",term:"MM9 (Média Móvel 9)",def:"Média dos últimos 9 fechamentos. Mais sensível, reage rápido a movimentos. Quando o preço cruza acima da MM9, sinal de alta de curto prazo. Exibida em amarelo nos gráficos do FootStock."},
  {cat:"Indicadores Técnicos",emoji:"📐",term:"MM21 (Média Móvel 21)",def:"Média dos últimos 21 fechamentos. Mais lenta e estável, indica tendência de médio prazo. Quando MM9 cruza acima da MM21 (Golden Cross), sinal clássico de alta. Exibida em lilás nos gráficos."},
  {cat:"Indicadores Técnicos",emoji:"🎸",term:"Bandas de Bollinger",def:"Indicador de volatilidade composto por 3 linhas: SMA20 (centro) e dois desvios-padrão acima e abaixo. Quando o preço toca a banda superior, pode estar sobrecomprado; banda inferior, sobrevendido."},
  {cat:"Indicadores Técnicos",emoji:"🔊",term:"Volume",def:"Quantidade de cotas negociadas num período. Alto volume confirma tendências e breakouts. Barras verdes = volume em alta; vermelhas = em queda. Visualizado abaixo do gráfico de preços."},
  {cat:"Indicadores Técnicos",emoji:"⚡",term:"Volatilidade",def:"Medida da variação de preço ao longo do tempo. Calculada via modelo GARCH no FootStock. Alta volatilidade = maior risco e oportunidade. Série B tem volatilidade estruturalmente mais alta que Série A."},
  {cat:"Indicadores Técnicos",emoji:"🌊",term:"GARCH (Volatilidade Condicional)",def:"Modelo estatístico que usa a volatilidade passada para estimar a volatilidade futura. Após movimentos bruscos, a volatilidade sobe. No FootStock, limita spikes extremos com um cap de 1,8×."},
  {cat:"Indicadores Técnicos",emoji:"🧲",term:"Mean Reversion",def:"Tendência dos preços de voltarem ao fair value após grandes desvios. Implementado via processo Ornstein-Uhlenbeck no motor. Evita que o preço derive indefinidamente longe dos fundamentos."},
  {cat:"Indicadores Técnicos",emoji:"🏊",term:"OFI (Order Flow Imbalance)",def:"Desequilíbrio entre ordens de compra e venda. OFI positivo = mais compradores, pressão de alta. OFI negativo = mais vendedores, pressão de baixa. Decai ao longo do tempo conforme o mercado absorve as ordens."},
  /* ── TIPOS DE ORDEM ── */
  {cat:"Tipos de Ordem",emoji:"⚡",term:"Ordem a Mercado (Fast)",def:"Execução imediata ao melhor preço disponível. No FootStock: compra ao Ask, venda ao Bid. Garante execução, mas não garante preço exato. Ideal quando velocidade importa mais que precisão."},
  {cat:"Tipos de Ordem",emoji:"🎯",term:"Ordem Limitada (Limit)",def:"Ordem com preço máximo de compra ou mínimo de venda pré-definido. Fica pendente no book até o mercado atingir o preço alvo. Você controla o preço, mas não a execução. Disponível no plano Craque."},
  {cat:"Tipos de Ordem",emoji:"🛡️",term:"OCO (One Cancels Other)",def:"Estratégia com dois gatilhos simultâneos: Stop Loss (proteção) e Take Profit (lucro). Quando um dispara, o outro é cancelado automaticamente. Ideal para gerenciar risco sem monitorar o mercado constantemente."},
  {cat:"Tipos de Ordem",emoji:"🛑",term:"Stop Loss (SL)",def:"Ordem de venda automática acionada quando o preço cai a um nível pré-definido. Limita o prejuízo máximo de uma posição. Ex: comprou a FS$20, Stop Loss em FS$17 → perde no máximo 15%."},
  {cat:"Tipos de Ordem",emoji:"🏹",term:"Take Profit (TP)",def:"Ordem de venda automática acionada quando o preço sobe a um nível pré-definido. Garante o lucro sem precisar acompanhar o mercado. Ex: comprou a FS$20, Take Profit em FS$25 → embolsa 25%."},
  {cat:"Tipos de Ordem",emoji:"📅",term:"Ordem Agendada",def:"Ordem programada para ser executada automaticamente em uma data e hora futuras. Útil para planejar entradas e saídas sem precisar estar presente no momento do pregão."},
  {cat:"Tipos de Ordem",emoji:"📉",term:"Short Selling (Venda a Descoberto)",def:"Operação em que você vende cotas que não possui, apostando na queda do preço. Para abrir: paga margem de 150% do valor. Para fechar: recompra as cotas. Lucro = diferença entre venda e recompra. Risco: ilimitado (preço pode subir indefinidamente)."},
  {cat:"Tipos de Ordem",emoji:"🔄",term:"Fechar Short (Short Close)",def:"Recompra das cotas vendidas a descoberto para encerrar a posição. Se o preço caiu desde a abertura, você lucra. Se subiu, você perde. A margem bloqueada é devolvida ao encerrar."},
  {cat:"Tipos de Ordem",emoji:"💲",term:"Margem (Short)",def:"Garantia financeira bloqueada ao abrir um short. No FootStock, equivale a 150% do valor da operação. Ex: short de FS$1.000 bloqueia FS$1.500 do saldo. Liberada ao fechar a posição."},
  /* ── CARTEIRA & P&L ── */
  {cat:"Carteira & Rentabilidade",emoji:"💼",term:"Carteira (Portfolio)",def:"Conjunto de todos os ativos que você possui. No FootStock, exibe cada clube com quantidade de cotas, preço médio de compra, valor atual e rentabilidade. Diversificar entre clubes e séries reduz o risco."},
  {cat:"Carteira & Rentabilidade",emoji:"💹",term:"P&L (Profit and Loss)",def:"Lucro ou Prejuízo de uma posição ou da carteira inteira. P&L = (Preço Atual − Preço Médio) × Quantidade. Positivo = lucro; negativo = prejuízo. Também chamado de resultado ou rentabilidade."},
  {cat:"Carteira & Rentabilidade",emoji:"💲",term:"Preço Médio",def:"Custo médio de aquisição das cotas de um ativo, ponderado por quantidade. Se você comprou 100 cotas a FS$10 e mais 100 a FS$12, seu preço médio é FS$11. Usado para calcular o P&L."},
  {cat:"Carteira & Rentabilidade",emoji:"📊",term:"Rentabilidade (%)",def:"Percentual de ganho ou perda em relação ao valor investido. Calculado como: (Valor Atual − Valor Investido) / Valor Investido × 100. É o termômetro da saúde da sua carteira."},
  {cat:"Carteira & Rentabilidade",emoji:"🏦",term:"Saldo Disponível",def:"Dinheiro em FootStockCoins (FS$) disponível para novas operações. Diminui ao comprar ou abrir short; aumenta ao vender ou fechar short com lucro. Não inclui o valor dos ativos na carteira."},
  {cat:"Carteira & Rentabilidade",emoji:"🔒",term:"Margem Bloqueada",def:"Valor em FS$ retido como garantia de posições short abertas. Não está disponível para outras operações enquanto o short estiver ativo. Devolvido ao fechar a posição."},
  {cat:"Carteira & Rentabilidade",emoji:"📈",term:"Patrimônio Total",def:"Soma do saldo disponível + valor atual da carteira + margem bloqueada. Representa seu patrimônio líquido total no FootStock. Exibido no Dashboard com gráfico de evolução histórica."},
  {cat:"Carteira & Rentabilidade",emoji:"💸",term:"Taxa Operacional",def:"Custo de cada transação executada. No FootStock, simulada como percentual do valor negociado, cobrada apenas na execução. Ordens Limite só pagam taxa quando efetivamente executadas."},
  /* ── SENTIMENTO & ANÁLISE ── */
  {cat:"Sentimento & Análise",emoji:"🧠",term:"Sentimento (Sentiment)",def:"Indicador de −1 a +1 que representa o humor do mercado em relação a um ativo. Valores positivos indicam otimismo (notícias boas, resultados recentes). Negativos indicam pessimismo. Afeta o drift do motor de preços."},
  {cat:"Sentimento & Análise",emoji:"📰",term:"Impacto de Notícias",def:"Cada notícia no FootStock pertence a uma categoria com impacto pré-calibrado: Financeira Crítica (±5%), Esportiva Maior (±3%), Mercado de Ativos (±2%), Integridade/Saúde (±1,5%), Institucional (±1%), Esportiva Menor (±0,5%)."},
  {cat:"Sentimento & Análise",emoji:"🌊",term:"Absorção de Notícia",def:"O impacto de uma notícia não é instantâneo. É distribuído ao longo de ~80 ticks (≈2 minutos), simulando como o mercado real processa informações gradualmente à medida que mais participantes reagem."},
  {cat:"Sentimento & Análise",emoji:"🔗",term:"Correlação entre Ativos",def:"Tendência de ativos se moverem juntos. No FootStock, clubes do mesmo cluster (A_TOP, A_MID, Série B) têm correlação via choque sistêmico. Clubes do mesmo estado (RJ, SP, MG, RS) têm correlação regional adicional."},
  {cat:"Sentimento & Análise",emoji:"🏗️",term:"Kyle's Lambda",def:"Modelo de impacto de preço por ordem do usuário. Ordens maiores movem mais o preço; floats maiores absorvem melhor. Calibrado para simular a microestrutura real da B3: sua ordem afeta o mercado proporcionalmente ao seu tamanho."},
  /* ── DIVISÕES & CLUBES ── */
  {cat:"Divisões & Clubes",emoji:"🥇",term:"Série A",def:"Primeira divisão do futebol brasileiro. No FootStock, os 20 clubes da Série A têm maior liquidez, menor volatilidade e múltiplos de valuation mais altos (3,0×–5,0×). São os ativos mais negociados da plataforma."},
  {cat:"Divisões & Clubes",emoji:"🥈",term:"Série B",def:"Segunda divisão do futebol brasileiro. Os 20 clubes da Série B no FootStock têm menor liquidez, maior volatilidade e múltiplos menores (0,8×–1,5×). Maior risco, mas possibilidade de maiores retornos."},
  {cat:"Divisões & Clubes",emoji:"📜",term:"Ticker",def:"Código de negociação único de cada ativo, com 4 caracteres (ex: URU3 = Flamengo, POR4 = Palmeiras). No FootStock, os nomes dos clubes são ficcionalizados para fins de simulação, mas os fundamentos são reais."},
  {cat:"Divisões & Clubes",emoji:"🏟️",term:"SAF (Sociedade Anônima do Futebol)",def:"Modelo jurídico que permite clubes operarem como empresas com fins lucrativos. Clubes SAF (Botafogo, Cruzeiro, Vasco, Atlético-MG, Bahia) atraem investidores estrangeiros e têm valuation diferente de associações tradicionais."},
  /* ── PLANOS & APP ── */
  {cat:"Planos & Funcionalidades",emoji:"⚡",term:"Plano Jogador (Grátis)",def:"Plano básico com FS$2.000 de saldo inicial, cotações com delay de 1 hora e até 2 ordens por dia. Ideal para quem quer conhecer a plataforma sem compromisso financeiro."},
  {cat:"Planos & Funcionalidades",emoji:"⭐",term:"Plano Craque",def:"Plano intermediário com FS$5.000 adicional, cotações com delay de 30 minutos, 5 ordens por dia, acesso à IA básica e Ordens Limitadas. Ideal para quem quer praticar estratégias mais sofisticadas."},
  {cat:"Planos & Funcionalidades",emoji:"👑",term:"Plano Lenda",def:"Plano premium com FS$25.000 adicional, cotações em tempo real, ordens ilimitadas, IA VIP, Short Selling e Alavancagem 2×. Para quem quer a experiência completa de mercado."},
  {cat:"Planos & Funcionalidades",emoji:"🤖",term:"Assessor IA",def:"Ferramenta de análise alimentada por IA que avalia os fundamentos dos clubes, interpreta notícias e sugere operações com base no perfil de risco do usuário. Disponível em versão básica (Craque) e VIP (Lenda)."},
  {cat:"Planos & Funcionalidades",emoji:"📑",term:"Extrato",def:"Histórico completo de todas as operações realizadas: compras, vendas, short, ordens agendadas, limitadas e OCOs. Permite rastrear a evolução do patrimônio e revisar decisões passadas."},
  {cat:"Planos & Funcionalidades",emoji:"🏆",term:"Ligas",def:"Competições entre usuários do FootStock. Cada liga tem ranking de rentabilidade, regras próprias e prêmios simbólicos. Adiciona o aspecto social e competitivo à experiência de investimentos."},
  {cat:"Planos & Funcionalidades",emoji:"📡",term:"Cotações em Tempo Real",def:"Preços atualizados a cada tick do motor (≈2 segundos). Disponível apenas no Plano Lenda. Os planos Jogador (delay 1h) e Craque (delay 30min) recebem dados defasados para simular a assimetria de informação do mercado real."},
  {cat:"Planos & Funcionalidades",emoji:"⚙️",term:"Alavancagem 2×",def:"Permite operar com o dobro do capital disponível, tomando crédito da plataforma. Dobra o potencial de ganho, mas também dobra o risco de perda. Disponível exclusivamente no Plano Lenda."},
  {cat:"Planos & Funcionalidades",emoji:"📰",term:"Tela de Notícias",def:"Feed com notícias reais sobre os clubes, coletadas automaticamente a cada 5 minutos. As notícias são injetadas no motor de preços via Impact Matrix, movendo os ativos conforme o impacto da notícia."},
];

const GLOSSARY_CATS=[...new Set(GLOSSARY_TERMS.map(t=>t.cat))];

/* ══════════════════════════════════════════════════════════════
   APP HEADER COMPARTILHADO
   Replica exatamente o cabeçalho do MainApp em todas as telas
   overlay (Notícias, Comunidade, Glossário, Perfil, etc.)

   Props do objeto `hdr`:
     user, marketSession, userPlan, planDue, isSuspended,
     news, newsLog, adIdx, onShowNews, onExit
   + pageTitle: string — título da tela atual (ex: "📰 Notícias")
   + onBack: callback — botão "← Voltar"
   + backLabel: string (opcional, default "← Voltar")
══════════════════════════════════════════════════════════════ */
const ADS_LIST=[
  {bg:"linear-gradient(135deg,#1a1a2e,#16213e)",border:"rgba(201,168,76,.3)",label:"PATROCINADO",labelColor:ACCENT,logo:"🏦",logoBg:"rgba(201,168,76,.12)",title:"Nubank Invest",subtitle:"Invista com zero taxa operacional",cta:"Abrir conta grátis",ctaBg:ACCENT,ctaColor:"#000"},
  {bg:"linear-gradient(135deg,#1a0033,#2d0052)",border:"rgba(162,89,255,.3)",label:"PATROCINADO",labelColor:"#a259ff",logo:"🎓",logoBg:"rgba(162,89,255,.15)",title:"Udemy",subtitle:"Cursos de finanças e mercado financeiro",cta:"Ver cursos",ctaBg:"#a259ff",ctaColor:"#fff"},
  {bg:"linear-gradient(135deg,#001a2e,#00264d)",border:"rgba(201,168,76,.25)",label:"PATROCINADO",labelColor:ACCENT2,logo:"📱",logoBg:"rgba(201,168,76,.12)",title:"Inter Shop",subtitle:"Cashback de 5% em todas as compras",cta:"Conhecer",ctaBg:ACCENT2,ctaColor:"#000"},
];

function AppHeader({hdr={}, pageTitle="", onBack, backLabel="← Voltar", children}){
  const {user={},marketSession={},userPlan="Jogador",planDue=null,isSuspended=false,
         news=null,newsLog=[],adIdx=0,onShowNews,onExit}=hdr;

  const ad=ADS_LIST[adIdx%ADS_LIST.length];
  const newsItem=news||newsLog[0];

  return <div style={{flexShrink:0,zIndex:20}}>
    {/* ── Linha 1: Logo/nome · Sessão · Plano · Avatar ── */}
    <div style={{padding:"6px 14px 0"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        {/* Brand (sem dropdown nas telas overlay) */}
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <div style={{width:32,height:32,borderRadius:9,background:"linear-gradient(145deg,#1a1510,#0e0c09)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 0 1px rgba(201,168,76,.2),0 4px 16px rgba(201,168,76,.08)"}}><FootStockLogo size={26} rounded={false}/></div>
          <div>
            <div style={{fontSize:14,fontWeight:700,color:"#fff",letterSpacing:"-0.3px",lineHeight:1,fontFamily:SANS_BODY,display:"flex",alignItems:"baseline",gap:4}}>FootStock<span style={{fontSize:8,fontWeight:900,color:GOLD,letterSpacing:"2px"}}>BETA</span></div>
            <div style={{fontSize:7,color:"rgba(201,168,76,.8)",fontWeight:600,letterSpacing:"2px",marginTop:1}}>A BOLSA DO FUTEBOL</div>
          </div>
        </div>
        {/* Direita: sessão + plano + avatar */}
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          {marketSession.color&&<div style={{background:marketSession.bg,border:`1px solid ${marketSession.border}`,borderRadius:20,padding:"3px 9px",display:"flex",alignItems:"center",gap:5}}>
            ...
            <span style={{fontSize:7,fontWeight:900,color:marketSession.color,fontFamily:SANS_BODY,letterSpacing:"0.5px"}}>{marketSession.short}</span>
          </div>}
          {(()=>{
            if(isSuspended) return <div style={{background:"rgba(244,63,94,.18)",border:"1px solid rgba(244,63,94,.4)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:RED,fontFamily:SANS_BODY}}>🔴 SUSPENSO</div>;
            const daysLeft=planDue?Math.ceil((planDue-new Date())/(1000*60*60*24)):null;
            if(daysLeft!=null&&daysLeft>=0&&daysLeft<=5&&userPlan!=="Jogador") return <div style={{background:"rgba(245,158,11,.15)",border:"1px solid rgba(245,158,11,.4)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:GOLD,fontFamily:SANS_BODY}}>⚠ {daysLeft===0?"vence hoje":`${daysLeft}d`}</div>;
            return <div style={{background:userPlan==="Lenda"?"linear-gradient(135deg,#C9A84C,#a07828)":userPlan==="Craque"?"linear-gradient(135deg,#C9A84C,#8a6820)":"rgba(255,255,255,.08)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:userPlan==="Lenda"?"#1a0800":userPlan==="Craque"?"#062011":"#fff",fontFamily:SANS_BODY}}>{userPlan==="Lenda"?"👑":userPlan==="Craque"?"⭐":"⚡"} {userPlan.toUpperCase()}</div>;
          })()}
          {user?.name&&<Avatar user={user} size={28} fontSize={10}/>}
        </div>
      </div>

      {/* ── Linha 2: Banner patrocinador ── */}
      <div style={{background:ad.bg,border:`1px solid ${ad.border}`,borderRadius:10,padding:"7px 10px",marginBottom:6,display:"flex",alignItems:"center",gap:9,position:"relative",overflow:"hidden",cursor:"pointer",minHeight:46}}>
        ...
        <div style={{width:32,height:32,borderRadius:8,background:ad.logoBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{ad.logo}</div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:7,color:ad.labelColor,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:1}}>{ad.label}</div>
          <div style={{fontSize:10,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,lineHeight:1.2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ad.title}</div>
          <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{ad.subtitle}</div>
        </div>
        <div style={{background:ad.ctaBg,borderRadius:7,padding:"4px 8px",fontSize:8,fontWeight:900,color:ad.ctaColor,fontFamily:SANS_BODY,flexShrink:0,whiteSpace:"nowrap"}}>{ad.cta}</div>
        <div style={{position:"absolute",bottom:3,left:"50%",transform:"translateX(-50%)",display:"flex",gap:3}}>
          {ADS_LIST.map((_,i)=><div key={i} style={{width:i===adIdx%ADS_LIST.length?10:4,height:3,borderRadius:2,background:i===adIdx%ADS_LIST.length?ad.labelColor:"rgba(255,255,255,.2)",transition:"all .3s"}}/>)}
        </div>
      </div>

      {/* ── Linha 3: Ticker de notícias ── */}
      {newsItem&&<div onClick={onShowNews} style={{background:"rgba(255,255,255,.04)",border:`1px solid ${newsItem.sent>=0?"rgba(201,168,76,.2)":"rgba(244,63,94,.25)"}`,borderRadius:9,padding:"4px 10px",marginBottom:7,display:"flex",alignItems:"center",gap:6,overflow:"hidden",cursor:onShowNews?"pointer":"default",transition:"opacity .15s"}} onMouseEnter={e=>e.currentTarget.style.opacity=".8"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
        <span style={{fontSize:10,flexShrink:0}}>{newsItem.emoji||"📰"}</span>
        <span style={{fontSize:8,fontWeight:600,color:newsItem.sent>=0?ACCENT:RED,fontFamily:MONO,flexShrink:0,whiteSpace:"nowrap"}}>{newsItem.ticker} {newsItem.sent>=0?"▲":"▼"}{Math.abs((newsItem.pct||newsItem.sent*5)||0).toFixed(2)}%</span>
        {news&&<span style={{fontSize:8,color:"rgba(201,168,76,.6)",fontFamily:SANS_BODY,flexShrink:0,whiteSpace:"nowrap"}}>AO VIVO</span>}
        <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,flex:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{newsItem.headline}</span>
        <span style={{fontSize:8,color:MUTED,flexShrink:0,opacity:.5}}>›</span>
      </div>}

      {/* ── Linha 4: Título da tela + botão voltar ── */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingBottom:8,borderBottom:`1px solid ${BORDER}`}}>
        <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{pageTitle}</div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {onBack&&<button onClick={onBack} style={{background:"rgba(255,255,255,.05)",border:`1px solid ${BORDER}`,color:MUTED,borderRadius:8,padding:"4px 10px",fontSize:10,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:700}}>{backLabel}</button>}
          {onExit&&<ExitBtn onExit={onExit}/>}
        </div>
      </div>
    </div>
    {/* Slot para conteúdo específico da tela (tabs, filtros, etc.) */}
    {children&&<div style={{padding:"8px 14px 0"}}>{children}</div>}
  </div>;
}

function GlossaryScreen({hdr={},onBack,onExit}){
  const [search,setSearch]=useState("");
  const [selCat,setSelCat]=useState("Todos");
  const [expanded,setExpanded]=useState(null);

  const filtered=GLOSSARY_TERMS.filter(t=>{
    const matchCat=selCat==="Todos"||t.cat===selCat;
  ...
  const catEmojis={
    ...
    "Sentimento & Análise":"🧠",
    "Divisões & Clubes":"🏆",
    "Planos & Funcionalidades":"🎖️",
  };

  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <AppHeader hdr={hdr} pageTitle="📖 Glossário" onBack={onBack} onExit={onExit}>
      {/* Busca + pills de categoria — slot abaixo do título */}
      <div style={{position:"relative",marginBottom:8}}>
        <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:11,color:MUTED,pointerEvents:"none"}}>🔍</span>
        <input value={search} onChange={e=>{setSearch(e.target.value);setSelCat("Todos");}} placeholder="Buscar termo..."
          style={{width:"100%",background:"rgba(201,168,76,.05)",border:`1px solid ${search?ACCENT:BORDER}`,borderRadius:10,padding:"8px 12px 8px 32px",color:TEXT,fontSize:11,fontFamily:SANS_BODY,outline:"none",boxSizing:"border-box",caretColor:ACCENT}}/>
        {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:MUTED,cursor:"pointer",fontSize:13,padding:0}}>×</button>}
      </div>
      {!search&&<div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:6,scrollbarWidth:"none"}}>
        {["Todos",...GLOSSARY_CATS].map(c=>{
          const active=selCat===c;
          return <button key={c} onClick={()=>setSelCat(c)} style={{flexShrink:0,padding:"4px 10px",borderRadius:20,border:`1px solid ${active?ACCENT:BORDER}`,background:active?"rgba(201,168,76,.12)":"transparent",color:active?ACCENT:MUTED,fontSize:9,fontWeight:active?800:500,fontFamily:SANS_BODY,cursor:"pointer",whiteSpace:"nowrap",transition:"all .15s"}}>
            {c==="Todos"?"Todos":catEmojis[c]+" "+c.split(" ")[0]}
          </button>;
        })}
      </div>}
    </AppHeader>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"12px 14px 24px"}}>
      {search&&filtered.length===0&&<div style={{textAlign:"center",padding:"40px 20px",color:MUTED,fontFamily:SANS_BODY,fontSize:12}}>Nenhum termo encontrado para "{search}"</div>}
      {search
        ? filtered.map((t,i)=><GlossaryCard key={i} term={t} expanded={expanded===i} onToggle={()=>setExpanded(expanded===i?null:i)}/>)
        : Object.entries(grouped).map(([cat,terms])=>(
          <div key={cat} style={{marginBottom:18}}>
            <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8,paddingBottom:6,borderBottom:`1px solid ${BORDER}`}}>
              <span style={{fontSize:14}}>{catEmojis[cat]}</span>
              <span style={{fontSize:10,fontWeight:800,color:ACCENT,letterSpacing:"0.8px",fontFamily:SANS_BODY}}>{cat.toUpperCase()}</span>
              <span style={{fontSize:9,color:MUTED,fontFamily:MONO,marginLeft:"auto"}}>{terms.length} termo{terms.length!==1?"s":""}</span>
            </div>
            {terms.map((t,i)=>{const gKey=cat+i;return <GlossaryCard key={gKey} term={t} expanded={expanded===gKey} onToggle={()=>setExpanded(expanded===gKey?null:gKey)}/>;})}</div>))
      }
    </div>
    <div style={{padding:"8px 16px 12px",borderTop:`1px solid ${BORDER}`,flexShrink:0,textAlign:"center"}}>
      <div style={{fontSize:9,color:"rgba(201,168,76,.4)",fontFamily:SANS_BODY}}>FootStock · Educação Financeira · Fontes: B3, CVM, Sports Value, Transfermarkt</div>
    </div>
  </div>;
}

function GlossaryCard({term,expanded,onToggle}){
  return <div onClick={onToggle} style={{background:expanded?"rgba(201,168,76,.05)":CARD,border:`1px solid ${expanded?ACCENT:BORDER}`,borderRadius:13,marginBottom:6,overflow:"hidden",cursor:"pointer",transition:"all .2s"}}>
    <div style={{padding:"10px 13px",display:"flex",alignItems:"center",gap:10}}>
      <span style={{fontSize:16,flexShrink:0}}>{term.emoji}</span>
      <span style={{flex:1,fontSize:11,fontWeight:700,color:expanded?"#fff":"rgba(255,255,255,.85)",fontFamily:SANS_BODY,lineHeight:1.3}}>{term.term}</span>
      <span style={{fontSize:14,color:expanded?ACCENT:MUTED,transition:"transform .2s",transform:expanded?"rotate(180deg)":"rotate(0deg)",flexShrink:0}}>⌄</span>
    </div>
    {expanded&&<div style={{padding:"0 13px 12px",borderTop:`1px solid ${BORDER}`}}>
      <div style={{height:8}}/>
      <div style={{fontSize:11,color:"rgba(255,255,255,.75)",fontFamily:SANS_BODY,lineHeight:1.7}}>{term.def}</div>
      <div style={{marginTop:8,display:"flex",alignItems:"center",gap:5}}>
        <div style={{width:4,height:4,borderRadius:"50%",background:ACCENT,opacity:.5}}/>
        <span style={{fontSize:8,color:"rgba(201,168,76,.5)",fontFamily:SANS_BODY,letterSpacing:"0.5px"}}>{term.cat.toUpperCase()}</span>
      </div>
    </div>}
  </div>;
}
...
/* ── PROFILE ── */
function ProfileScreen({hdr={},user,onBack,onExit}){
  ...
  const iS={width:"100%",background:SURFACE,border:`1.5px solid ${ACCENT}`,borderRadius:10,padding:"10px 13px",color:TEXT,fontSize:12,fontFamily:SANS_BODY,outline:"none",boxSizing:"border-box"};
  const roS={background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px 13px",fontSize:12,color:TEXT,fontFamily:SANS_BODY,display:"flex",alignItems:"center",justifyContent:"space-between"};
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <AppHeader hdr={hdr} pageTitle="👤 Minha Conta" onBack={onBack} onExit={onExit}/>
    <div style={{flex:1,overflowY:"auto",padding:"8px 16px"}}>
      <div style={{background:"linear-gradient(135deg,rgba(201,168,76,.06),rgba(201,168,76,.03))",border:`1px solid ${BORDER}`,borderRadius:20,padding:"20px 16px",marginBottom:18,display:"flex",alignItems:"center",gap:14}}><Avatar user={user} size={60} fontSize={20}/><div><div style={{fontSize:18,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>{form.name}</div><div style={{display:"inline-flex",alignItems:"center",gap:5,background:`${user.planColor}20`,border:`1px solid ${user.planColor}40`,borderRadius:20,padding:"3px 10px"}}><span style={{fontSize:12,display:"flex",alignItems:"center"}}>{user.plan==="Lenda"?"👑":user.plan==="Craque"?"🎯":<FootStockLogo size={14} rounded={false}/>}</span><span style={{fontSize:10,fontWeight:800,color:user.planColor,fontFamily:SANS_BODY}}>Plano {user.plan}</span></div></div></div>
      {saved&&<div style={{background:"rgba(201,168,76,.08)",border:`1px solid ${ACCENT}40`,borderRadius:12,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14}}>✅</span><span style={{fontSize:12,fontWeight:700,color:ACCENT,fontFamily:SANS_BODY}}>Dados atualizados!</span></div>}
      <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:10}}>DADOS PESSOAIS</div>
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:"14px",marginBottom:14}}>
        {[{l:"NOME COMPLETO",k:"name"},{l:"E-MAIL",k:"email"},{l:"TELEFONE",k:"phone"},{l:"CIDADE",k:"city"}].map(f=><div key={f.k} style={{marginBottom:12}}><div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS_BODY,marginBottom:5}}>{f.l}</div>{editing?<input style={iS} value={form[f.k]} onChange={e=>setForm({...form,[f.k]:e.target.value})}/> :<div style={roS}>{form[f.k]}</div>}</div>)}
        {[{l:"CPF",v:user.cpf},{l:"DATA DE NASCIMENTO",v:user.birthdate}].map(f=><div key={f.l} style={{marginBottom:12}}><div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS_BODY,marginBottom:5}}>{f.l}</div><div style={roS}>{f.v}<span style={{fontSize:10,color:"#334155"}}>🔒</span></div></div>)}
      </div>
    </div>
    <div style={{padding:"10px 16px 14px",flexShrink:0}}>
      {editing?<div style={{display:"flex",gap:8}}><button onClick={()=>setEditing(false)} style={{flex:1,padding:"12px",borderRadius:13,border:`1px solid ${BORDER}`,background:"none",color:MUTED,fontSize:12,fontWeight:700,fontFamily:SANS_BODY,cursor:"pointer"}}>Cancelar</button><button onClick={()=>{setSaved(true);setEditing(false);setTimeout(()=>setSaved(false),2500);}} style={{flex:2,padding:"12px",borderRadius:13,border:"none",background:"linear-gradient(135deg,#C9A84C,#8a6820)",color:BG,fontSize:12,fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer"}}>Salvar</button></div>:<button onClick={()=>setEditing(true)} style={{width:"100%",padding:"13px",borderRadius:13,border:`1px solid ${ACCENT}50`,background:`${ACCENT}10`,color:ACCENT,fontSize:12,fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer"}}>✎ Editar dados pessoais</button>}
    </div>
  </div>;
}

/* ── EXTRATO ── */
function ExtratoScreen({hdr={},txLog,scheduledOrders,limitedOrders=[],ocoOrders=[],cancelScheduled,cancelLimited,cancelOco,onBack,onExit}){
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <AppHeader hdr={hdr} pageTitle="📑 Extrato" onBack={onBack} backLabel="← Carteira" onExit={onExit}>
      {/* Tabs — slot abaixo do título */}
      <div style={{display:"flex",gap:4}}>
        {[{id:"all",label:"Operações",count:txLog.length},{id:"scheduled",label:"Agendadas",count:pendingOrders.length},{id:"limited",label:"Precificadas",count:pendingLimited.length},{id:"oco",label:"SL / TP",count:pendingOco.length}].map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{flex:1,padding:"7px 4px",borderRadius:9,border:`1px solid ${activeTab===t.id?"rgba(201,168,76,.4)":BORDER}`,background:activeTab===t.id?"rgba(201,168,76,.06)":SURFACE,color:activeTab===t.id?ACCENT:MUTED,fontSize:9,fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
            {t.label}
            {t.count>0&&<span style={{background:activeTab===t.id?"rgba(201,168,76,.2)":"rgba(255,255,255,.08)",color:activeTab===t.id?ACCENT:"rgba(255,255,255,.6)",borderRadius:10,padding:"1px 5px",fontSize:7,fontWeight:900}}>{t.count}</span>}
          </button>
        ))}
      </div>
    </AppHeader>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"0 14px 16px"}}>
      ...
      {/* ── ABA OPERAÇÕES ── */}
      {activeTab==="all"&&<>
        <div style={{background:"linear-gradient(135deg,rgba(201,168,76,.06),rgba(212,180,102,.04))",border:"1px solid rgba(201,168,76,.2)",borderRadius:18,padding:"16px",marginBottom:18}}>
          <div style={{fontSize:8,color:MUTED,letterSpacing:"1.5px",fontWeight:700,marginBottom:6,fontFamily:SANS_BODY}}>RESUMO DO PERÍODO</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
            ...
            ].map(s=><div key={s.l}><div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,letterSpacing:"0.5px",marginBottom:4}}>{s.l}</div><div style={{fontSize:11,fontWeight:900,color:s.c,fontFamily:MONO}}>{s.i} {s.v}</div></div>)}
          </div>
        </div>
        {txLog.length===0&&<div style={{...card,padding:"32px",textAlign:"center"}}><div style={{fontSize:28,marginBottom:10}}>📋</div><div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Nenhuma transação ainda</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginTop:5}}>Suas compras e vendas aparecerão aqui.</div></div>}
        {days.map(day=>{
          const dayPnl=day.closeBalance-day.openBalance;
          return <div key={day.date} style={{marginBottom:18}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:7,height:7,borderRadius:"50%",background:ACCENT,boxShadow:`0 0 8px ${ACCENT}60`}}/><span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{fmtDate(day.date)}</span></div>
            ...
            <div style={{background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:11,padding:"9px 13px",marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:8,background:"rgba(74,85,104,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>🌅</div><div><div style={{fontSize:10,fontWeight:700,color:"#a8b8cc",fontFamily:SANS_BODY}}>Saldo inicial</div><div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Abertura de pregão</div></div></div>
            ...
            {day.txs.map((tx,i)=>{
                ...
                return <div key={i} style={{background:`${planColor}10`,border:`1.5px solid ${planColor}40`,borderRadius:11,padding:"10px 13px",marginBottom:6,display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                      <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Upgrade de Plano</span>
                      <span style={{fontSize:8,background:`${planColor}25`,color:planColor,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{planIcon} {tx.plan.toUpperCase()}</span>
                    </div>
                    <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Bônus de boas-vindas · Plano {tx.plan}</div>
                    <div style={{fontSize:8,color:"#334155",fontFamily:SANS_BODY,marginTop:1}}>{tx.time}</div>
                  </div>
              ...
              const txBg=isBuy?"rgba(201,168,76,.05)":isShortOpen?"rgba(244,63,94,.05)":isShortClose?(tx.pnl>=0?"rgba(201,168,76,.05)":"rgba(244,63,94,.05)"):"rgba(244,63,94,.05)";
              const txBorder=isBuy?"rgba(201,168,76,.18)":isShortOpen?"rgba(244,63,94,.18)":isShortClose?(tx.pnl>=0?"rgba(201,168,76,.18)":"rgba(244,63,94,.18)"):"rgba(244,63,94,.18)";
              ...
              return <div key={i} style={{background:txBg,border:`1px solid ${txBorder}`,borderRadius:11,padding:"9px 13px",marginBottom:6,display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{tx.ticker}</span>
                    <span style={{fontSize:8,background:`${txColor}20`,color:txColor,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{txLabel}</span>
                    {tx.scheduled&&<span style={{fontSize:7,background:"rgba(201,168,76,.12)",color:"#D4B466",borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>📅 AGENDADA</span>}
                    {tx.oco&&<span style={{fontSize:7,background:"rgba(245,166,35,.2)",color:GOLD,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>🛡 OCO</span>}
                    {isShortClose&&<span style={{fontSize:7,background:tx.pnl>=0?"rgba(201,168,76,.12)":"rgba(244,63,94,.15)",color:tx.pnl>=0?ACCENT:RED,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>P&L {tx.pnl>=0?"+":""}FS${(tx.pnl||0).toFixed(2)}</span>}
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{tx.qty} cota{tx.qty!==1?"s":""}{isShort?` · Notional FS$${tx.total?.toFixed(2)||"—"}`:` · FS$${tx.price?.toFixed(2)} cada`}{tx.fee?<span style={{color:"#f5a623",marginLeft:6}}>taxa FS${tx.fee.toFixed(2)}</span>:""}</div>
                  <div style={{fontSize:8,color:"#334155",fontFamily:SANS_BODY,marginTop:1}}>{tx.time}</div>
                </div>
                <div style={{textAlign:"right"}}>
            ...
            <div style={{background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:11,padding:"9px 13px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:8,background:dayPnl>=0?"rgba(201,168,76,.12)":"rgba(244,63,94,.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>{dayPnl>=0?"🌙":"📉"}</div><div><div style={{fontSize:10,fontWeight:700,color:"#a8b8cc",fontFamily:SANS_BODY}}>Saldo final</div><div style={{fontSize:9,color:dayPnl>=0?ACCENT:RED,fontFamily:SANS_BODY,marginTop:1}}>{dayPnl>=0?"▲":""} FS${Math.abs(dayPnl).toLocaleString("pt-BR",{minimumFractionDigits:2})}</div></div></div>
      ...
      {/* ── ABA AGENDADAS ── */}
      {activeTab==="scheduled"&&<>
        {scheduledOrders.length===0&&<div style={{...card,padding:"32px",textAlign:"center"}}><div style={{fontSize:28,marginBottom:10}}>📅</div><div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Nenhuma ordem agendada</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginTop:5}}>Ordens agendadas fora do horário de mercado aparecerão aqui.</div></div>}

        {/* Pendentes */}
        {pendingOrders.length>0&&<>
          <div style={{fontSize:8,color:"#D4B466",fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8}}>AGUARDANDO EXECUÇÃO · {pendingOrders.length}</div>
          {pendingOrders.map(o=>{
            const club=CLUBS.find(c=>c.ticker===o.ticker),isBuy=o.side==="buy";
            const accentCol=isBuy?ACCENT:RED;
            return <div key={o.id} style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.22)",borderRadius:13,padding:"12px 13px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:8,background:isBuy?`${ACCENT}20`:`${RED}20`,color:accentCol,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{isBuy?"COMPRA":"VENDA"}</span>
                    <span style={{fontSize:7,background:"rgba(201,168,76,.15)",color:"#D4B466",borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>⏳ PENDENTE</span>
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""} · {o.priceType==="close"?"Preço de fechamento":`Limite FS$${o.fixedPrice?.toFixed(2)}`}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,marginTop:1}}>Agendada {fmtDatetime(o.scheduledAt)}</div>
                </div>
                <button onClick={()=>cancelScheduled(o.id)} style={{background:"rgba(244,63,94,.15)",border:"1px solid rgba(244,63,94,.3)",borderRadius:8,color:"#f43f5e",fontSize:10,fontWeight:800,padding:"4px 9px",cursor:"pointer",fontFamily:SANS_BODY,flexShrink:0}}>✕</button>
              </div>
              <div style={{background:"rgba(245,166,35,.07)",border:"1px solid rgba(245,166,35,.18)",borderRadius:8,padding:"5px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:8,color:"#d4a017",fontFamily:SANS_BODY}}>⚡ Executa na abertura · 11h00</span>
        ...
        {cancelledOrders.length>0&&<>
          <div style={{fontSize:8,color:RED,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8,marginTop:16}}>CANCELADAS · {cancelledOrders.length}</div>
          {cancelledOrders.map(o=>{
            ...
            return <div key={o.id} style={{background:"rgba(244,63,94,.05)",border:"1px solid rgba(244,63,94,.15)",borderRadius:13,padding:"12px 13px",marginBottom:8,opacity:.75}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                    <span style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:8,background:"rgba(244,63,94,.15)",color:RED,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{isBuy?"COMPRA":"VENDA"}</span>
                    <span style={{fontSize:7,background:"rgba(244,63,94,.2)",color:"#f87171",borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>✕ CANCELADA</span>
                  </div>
                  <div style={{fontSize:9,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""} · {o.priceType==="close"?"Preço de fechamento":`Limite FS$${o.fixedPrice?.toFixed(2)}`}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.38)",fontFamily:SANS_BODY,marginTop:1}}>Cancelada {fmtDatetime(o.cancelledAt||o.scheduledAt)}</div>
                </div>
              </div>
            </div>;
          })}
      ...
      {/* ── ABA PRECIFICADAS ── */}
      {activeTab==="limited"&&<>
        {limitedOrders.length===0&&<div style={{...card,padding:"32px",textAlign:"center"}}><div style={{fontSize:28,marginBottom:10}}>🎯</div><div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Nenhuma ordem precificada</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginTop:5}}>Crie ordens com preço limite no modal de Compra ou Venda.</div></div>}

        {/* Pendentes */}
        {pendingLimited.length>0&&<>
          <div style={{fontSize:8,color:GOLD,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8}}>AGUARDANDO MERCADO · {pendingLimited.length}</div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:8,background:isBuy?`${ACCENT}20`:`${RED}20`,color:accentCol,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{isBuy?"COMPRA":"VENDA"}</span>
                    <span style={{fontSize:7,background:"rgba(245,166,35,.2)",color:GOLD,borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>🎯 LIMITE</span>
                    <span style={{fontSize:7,background:"rgba(201,168,76,.15)",color:"#D4B466",borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>⏳ PENDENTE</span>
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,marginTop:1}}>Criada {fmtDatetime(o.createdAt)}</div>
                </div>
                <button onClick={()=>cancelLimited(o.id)} style={{background:"rgba(244,63,94,.15)",border:"1px solid rgba(244,63,94,.3)",borderRadius:8,color:"#f43f5e",fontSize:10,fontWeight:800,padding:"4px 9px",cursor:"pointer",fontFamily:SANS_BODY,flexShrink:0}}>✕</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                <div style={{background:"rgba(245,166,35,.08)",border:"1px solid rgba(245,166,35,.2)",borderRadius:9,padding:"7px 10px"}}>
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:2}}>PREÇO LIMITE<InfoTip term="Ordem Limitada (Limit)"/></div>
                  <div style={{fontSize:13,fontWeight:900,color:GOLD,fontFamily:MONO}}>FS${o.limitPrice.toFixed(2)}</div>
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>{isBuy?"Executa se ASK ≤ limite":"Executa se BID ≥ limite"}</div>
                </div>
                <div style={{background:"rgba(255,255,255,.04)",border:`1px solid ${BORDER}`,borderRadius:9,padding:"7px 10px"}}>
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:2}}>PREÇO NO REGISTO</div>
                  <div style={{fontSize:13,fontWeight:900,color:"#fff",fontFamily:MONO}}>FS${(o.mktPrice||0).toFixed(2)}</div>
                  {mktDiff!==null&&<div style={{fontSize:7,color:mktDiff>0?ACCENT:RED,fontFamily:SANS_BODY,marginTop:1}}>{mktDiff>0?"+":""}{mktDiff}% vs limite</div>}
                </div>
              </div>
            </div>;
          })}
        ...
        {executedLimited.length>0&&<>
          <div style={{fontSize:8,color:ACCENT,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8,marginTop:16}}>EXECUTADAS · {executedLimited.length}</div>
          {executedLimited.map(o=>{
            const club=CLUBS.find(c=>c.ticker===o.ticker),isBuy=o.side==="buy";
            const accentCol=isBuy?ACCENT:RED;
            return <div key={o.id} style={{background:"rgba(201,168,76,.05)",border:"1px solid rgba(201,168,76,.2)",borderRadius:13,padding:"12px 13px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:34,height:34,borderRadius:10,background:`linear-gradient(145deg,${club?.color||"#333"},${club?.color||"#333"}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:900,color:club?.c2||"#fff",flexShrink:0}}>{o.ticker.replace(/\d/g,"")}</div>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:8,background:isBuy?`${ACCENT}20`:`${RED}20`,color:accentCol,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{isBuy?"COMPRA":"VENDA"}</span>
                    <span style={{fontSize:7,background:"rgba(201,168,76,.2)",color:ACCENT,borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>✓ EXECUTADA</span>
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""} · Limite FS${o.limitPrice.toFixed(2)} · Exec. FS${(o.execPrice||o.limitPrice).toFixed(2)}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,marginTop:1}}>Executada {fmtDatetime(o.executedAt||o.createdAt)}</div>
                </div>
              </div>
            </div>;
          })}
        ...
        {cancelledLimited.length>0&&<>
          <div style={{fontSize:8,color:RED,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8,marginTop:16}}>CANCELADAS · {cancelledLimited.length}</div>
          {cancelledLimited.map(o=>{
            ...
            return <div key={o.id} style={{background:"rgba(244,63,94,.04)",border:"1px solid rgba(244,63,94,.15)",borderRadius:13,padding:"12px 13px",marginBottom:8,opacity:.7}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                    <span style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:8,background:"rgba(244,63,94,.12)",color:RED,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{isBuy?"COMPRA":"VENDA"}</span>
                    <span style={{fontSize:7,background:"rgba(244,63,94,.18)",color:"#f87171",borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>✕ CANCELADA</span>
                  </div>
                  <div style={{fontSize:9,color:"rgba(255,255,255,.48)",fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""} · Limite FS${o.limitPrice.toFixed(2)}</div>
                  {o.reason&&<div style={{fontSize:8,color:"rgba(244,63,94,.5)",fontFamily:SANS_BODY,marginTop:1}}>{o.reason}</div>}
                  <div style={{fontSize:8,color:"rgba(255,255,255,.36)",fontFamily:SANS_BODY,marginTop:1}}>Cancelada {fmtDatetime(o.cancelledAt||o.createdAt)}</div>
                </div>
              </div>
            </div>;
          })}
      ...
      {/* ── ABA SL / TP (OCO) ── */}
      {activeTab==="oco"&&<>
        {ocoOrders.length===0&&<div style={{...card,padding:"32px",textAlign:"center"}}><div style={{fontSize:28,marginBottom:10}}>🛡</div><div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Nenhuma ordem OCO</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginTop:5}}>Crie ordens Stop Loss / Take Profit no modal de Venda (exclusivo plano Lenda).</div></div>}

        {/* Pendentes */}
        {pendingOco.length>0&&<>
          <div style={{fontSize:8,color:GOLD,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8}}>ORDENS ATIVAS · {pendingOco.length}</div>
          {pendingOco.map(o=>{
            ...
            return <div key={o.id} style={{background:"rgba(245,166,35,.06)",border:"1px solid rgba(245,166,35,.3)",borderRadius:13,padding:"12px 13px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:7,background:"rgba(245,166,35,.2)",color:GOLD,borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>🛡 OCO</span>
                    <span style={{fontSize:7,background:"rgba(201,168,76,.15)",color:"#D4B466",borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>⏳ ATIVO</span>
                    <span style={{fontSize:7,background:"rgba(245,166,35,.15)",color:GOLD,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>👑 LENDA</span>
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""} · Registro FS${(o.mktPrice||0).toFixed(2)}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,marginTop:1}}>Criada {fmtDatetime(o.createdAt)}</div>
                </div>
                <button onClick={()=>cancelOco(o.id)} style={{background:"rgba(244,63,94,.15)",border:"1px solid rgba(244,63,94,.3)",borderRadius:8,color:"#f43f5e",fontSize:10,fontWeight:800,padding:"4px 9px",cursor:"pointer",fontFamily:SANS_BODY,flexShrink:0}}>✕</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                {o.slPrice&&<div style={{background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.25)",borderRadius:9,padding:"8px 10px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:3}}><span style={{fontSize:10}}>🛑</span><span style={{fontSize:7,color:RED,fontWeight:800,fontFamily:SANS_BODY}}>STOP LOSS<InfoTip term="Stop Loss (SL)"/></span></div>
                  <div style={{fontSize:14,fontWeight:900,color:RED,fontFamily:MONO}}>FS${o.slPrice.toFixed(2)}</div>
                  {o.slPct&&<div style={{fontSize:7,color:"rgba(244,63,94,.6)",fontFamily:SANS_BODY,marginTop:1}}>−{o.slPct}% do registro</div>}
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginTop:2}}>Dispara se BID ≤ limite</div>
                </div>}
                {o.tpPrice&&<div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.15)",borderRadius:9,padding:"8px 10px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:3}}><span style={{fontSize:10}}>🎯</span><span style={{fontSize:7,color:ACCENT,fontWeight:800,fontFamily:SANS_BODY}}>TAKE PROFIT<InfoTip term="Take Profit (TP)"/></span></div>
                  <div style={{fontSize:14,fontWeight:900,color:ACCENT,fontFamily:MONO}}>FS${o.tpPrice.toFixed(2)}</div>
                  {o.tpPct&&<div style={{fontSize:7,color:"rgba(201,168,76,.6)",fontFamily:SANS_BODY,marginTop:1}}>+{o.tpPct}% do registro</div>}
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginTop:2}}>Dispara se BID ≥ limite</div>
                </div>}
              </div>
            </div>;
        ...
        {executedOco.length>0&&<>
          <div style={{fontSize:8,color:ACCENT,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8,marginTop:16}}>EXECUTADAS · {executedOco.length}</div>
          {executedOco.map(o=>{
            const club=CLUBS.find(c=>c.ticker===o.ticker);
            const isSL=o.triggerType==="sl";
            return <div key={o.id} style={{background:isSL?"rgba(244,63,94,.05)":"rgba(201,168,76,.05)",border:`1px solid ${isSL?"rgba(244,63,94,.2)":"rgba(201,168,76,.2)"}`,borderRadius:13,padding:"12px 13px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:34,height:34,borderRadius:10,background:`linear-gradient(145deg,${club?.color||"#333"},${club?.color||"#333"}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:900,color:club?.c2||"#fff",flexShrink:0}}>{o.ticker.replace(/\d/g,"")}</div>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{o.ticker}</span>
                    <span style={{fontSize:8,background:isSL?`${RED}20`:`${ACCENT}20`,color:isSL?RED:ACCENT,borderRadius:5,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>{isSL?"🛑 STOP LOSS":"🎯 TAKE PROFIT"}</span>
                    <span style={{fontSize:7,background:isSL?"rgba(244,63,94,.2)":"rgba(201,168,76,.2)",color:isSL?RED:ACCENT,borderRadius:5,padding:"1px 7px",fontWeight:800,fontFamily:SANS_BODY}}>✓ EXECUTADA</span>
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""} · Exec. FS${(o.execPrice||0).toFixed(2)} · Gatilho: {isSL?`SL FS$${o.slPrice?.toFixed(2)}`:`TP FS$${o.tpPrice?.toFixed(2)}`}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,marginTop:1}}>Executada {fmtDatetime(o.executedAt||o.createdAt)}</div>
                </div>
              </div>
            </div>;
        ...
        {cancelledOco.length>0&&<>
          <div style={{fontSize:8,color:RED,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8,marginTop:16}}>CANCELADAS · {cancelledOco.length}</div>
          {cancelledOco.map(o=>{
            const club=CLUBS.find(c=>c.ticker===o.ticker);
            return <div key={o.id} style={{background:"rgba(244,63,94,.04)",border:"1px solid rgba(244,63,94,.15)",borderRadius:13,padding:"12px 13px",marginBottom:8,opacity:.7}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:9,color:"rgba(255,255,255,.48)",fontFamily:SANS_BODY}}>{o.qty} cota{o.qty!==1?"s":""}{o.slPrice?` · SL FS$${o.slPrice.toFixed(2)}`:""}{o.tpPrice?` · TP FS$${o.tpPrice.toFixed(2)}`:""}</div>
                  {o.reason&&<div style={{fontSize:8,color:"rgba(244,63,94,.5)",fontFamily:SANS_BODY,marginTop:1}}>{o.reason}</div>}
                  <div style={{fontSize:8,color:"rgba(255,255,255,.36)",fontFamily:SANS_BODY,marginTop:1}}>Cancelada {fmtDatetime(o.cancelledAt||o.createdAt)}</div>
                </div>
              </div>
...
══════════════════════════════════════════════════════════════ */
/* ── CRIAR LIGA ── */
function CriarLigaScreen({hdr={},onBack,onExit,user}){
  ...
  const inputStyle={width:"100%",background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:10,
    padding:"10px 12px",color:"#e2e8f0",fontSize:11,fontFamily:SANS_BODY,outline:"none",
    boxSizing:"border-box"};
  const labelStyle={fontSize:8,fontWeight:800,color:MUTED,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:5,display:"block"};
  ...
  if(step===5) return (
    <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
      <div style={{padding:"10px 14px 0",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{width:28}}/>
        <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Liga criada!</div>
        <ExitBtn onExit={onExit}/>
      </div>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 24px",textAlign:"center"}}>
        ...
        <div style={{fontSize:20,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,marginBottom:6}}>{form.nome}</div>
        <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:28}}>{TIPOS.find(t=>t.id===form.tipo)?.label}</div>

        {form.tipo!=="publica"&&<>
          <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginBottom:8}}>LINK DE CONVITE</div>
          <div style={{background:SURFACE,border:`1px solid ${ACCENT}40`,borderRadius:14,padding:"14px 16px",marginBottom:24,width:"100%",boxSizing:"border-box"}}>
            ...
            <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginTop:8,display:"flex",alignItems:"center",gap:5}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,width:"100%",marginBottom:28}}>
          {[
            ...
            <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:4}}>{s.l}</div>
            <div style={{fontSize:10,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{s.v}</div>
          </div>)}
        </div>
        ...
        {form.premios.some(p=>p.valor)&&(
          <div style={{width:"100%",marginBottom:24}}>
            <div style={{fontSize:8,fontWeight:800,color:MUTED,letterSpacing:"1.5px",fontFamily:SANS_BODY,marginBottom:10}}>🏆 PREMIAÇÕES</div>
              ...
              {form.premios.filter(p=>p.valor).map((p,i)=>(
                <div key={i} style={{
                  ...
                  <div style={{flex:1,textAlign:"left"}}>
                    <div style={{fontSize:8,fontWeight:800,color:p.color,fontFamily:SANS_BODY,letterSpacing:"0.5px",marginBottom:2}}>{p.label.toUpperCase()}</div>
                    <div style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:SANS_BODY}}>{p.valor}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <button onClick={onBack} style={{width:"100%",background:"linear-gradient(135deg,#C9A84C,#8a6820)",border:"none",borderRadius:14,padding:"14px",fontSize:13,fontWeight:900,color:"#000",fontFamily:SANS_BODY,cursor:"pointer",marginBottom:10}}>
          Ir para minha liga →
        </button>
        <button onClick={onBack} style={{background:"none",border:"none",color:MUTED,fontSize:11,fontFamily:SANS_BODY,cursor:"pointer"}}>
          Voltar ao início
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
      <AppHeader hdr={hdr} pageTitle="🏆 Criar Liga"
        onBack={step===1?onBack:()=>setStep(s=>s-1)}
        backLabel={step===1?"← Ligas":"← Voltar"}
        onExit={onExit}>
        {/* Progress bar — slot abaixo do título */}
        <div>
          ...
          <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"8px 14px 20px"}}>
        ...
        {/* ── STEP 1: Tipo ── */}
        {step===1&&<>
          <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>Que tipo de liga?</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:20}}>Escolha o formato que melhor combina com você</div>
          {TIPOS.map(t=>{
            ...
            return <button key={t.id} onClick={()=>{if(!t.locked)set("tipo")(t.id);}}
              ...
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>{t.label}</div>
              <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.4}}>{t.desc}</div>
              {t.locked&&<div style={{fontSize:9,color:GOLD,marginTop:8,fontWeight:700,fontFamily:SANS_BODY}}>⚡ Requer plano Lenda</div>}
            </button>;
          })}

          {/* Emblema */}
          <div style={{marginTop:16,marginBottom:20}}>
            <div style={{fontSize:8,fontWeight:800,color:MUTED,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:10}}>ESCOLHA O EMBLEMA</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {EMBLEMAS.map(e=><button key={e} onClick={()=>{set("emblema")(e);set("emblemaCustom")(null);}}
                style={{width:42,height:42,borderRadius:10,fontSize:22,
                  background:form.emblema===e&&!form.emblemaCustom?"rgba(201,168,76,.12)":SURFACE,
              ...
              <label style={{width:42,height:42,borderRadius:10,fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",
                background:form.emblemaCustom?"rgba(201,168,76,.12)":SURFACE,
            ...
            {form.emblemaCustom&&<div style={{fontSize:8,color:ACCENT,fontFamily:SANS_BODY,marginTop:6}}>✓ Emblema personalizado carregado</div>}
          </div>

          <button onClick={()=>setStep(2)}
            style={{width:"100%",background:"linear-gradient(135deg,#C9A84C,#8a6820)",border:"none",borderRadius:14,padding:"14px",fontSize:13,fontWeight:900,color:"#000",fontFamily:SANS_BODY,cursor:"pointer"}}>
            Continuar →
          </button>
        </>}
        ...
        {step===2&&<>
          <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>Configure sua liga</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:20}}>Dê um nome e defina os parâmetros básicos</div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:4,textAlign:"right"}}>{form.nome.length}/40</div>
          </div>
            ...
            <div style={{display:"flex",gap:6}}>
              {DURACOES.map(d=><button key={d.id} onClick={()=>set("duracao")(d.id)}
                style={{flex:1,background:form.duracao===d.id?`${ACCENT}22`:SURFACE,
                  ...
                  color:form.duracao===d.id?ACCENT:MUTED,fontFamily:SANS_BODY,cursor:"pointer"}}>
                {d.l}
              </button>)}
            </div>
          </div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:2}}>
          ...
          {form.tipo==="amigos"&&<div style={{marginBottom:20,background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:12,padding:"12px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
                <div style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:SANS_BODY}}>Liga privada</div>
                <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Apenas quem tem o código pode entrar</div>
              </div>
          ...
          <button onClick={()=>form.nome.trim()?setStep(3):null}
            style={{width:"100%",background:form.nome.trim()?`linear-gradient(135deg,${ACCENT},${ACCENT2})`:"rgba(255,255,255,.1)",
              ...
              color:form.nome.trim()?"#000":MUTED,fontFamily:SANS_BODY,cursor:form.nome.trim()?"pointer":"not-allowed"}}>
            Continuar →
          </button>
        </>}
        ...
        {/* ── STEP 3: Regras ── */}
        {step===3&&<>
          <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>Regras da liga</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:16}}>Personalize restrições e premiações</div>
          ...
          {[
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div>
                  <div style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:SANS_BODY}}>{r.label}</div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{r.desc}</div>
                </div>
              </div>
          ...
          <div style={{marginBottom:6}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div>
                <div style={{fontSize:10,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>🏆 Premiações</div>
                <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Defina os prêmios por colocação</div>
              </div>
              {/* Badge "qualquer liga pode ter prêmios" */}
              <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:ACCENT,fontFamily:SANS_BODY}}>
                Opcional
              </div>
            </div>
                  ...
                  <div key={p.lugar} style={{
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div style={{
                        ...
                        <div style={{fontSize:6,fontWeight:800,color:p.valor?p.color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>{ordinals[i]||`${i+1}º`}</div>
                      </div>

                      {/* Input do prêmio */}
                      <div style={{flex:1}}>
                        <div style={{fontSize:7,fontWeight:800,color:p.valor?p.color:MUTED,letterSpacing:"0.8px",fontFamily:SANS_BODY,marginBottom:4}}>
                        ...
                        <input
                          ...
                          style={{
                            ...
                            color:"#e2e8f0",fontSize:11,fontFamily:SANS_BODY,
                    ...
                    {p.valor&&(
                      <div style={{
                        ...
                        <span style={{fontSize:9,fontWeight:700,color:p.color,fontFamily:SANS_BODY,flex:1}}>{p.valor}</span>
                        <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>confirmado</span>
                      </div>
                    )}
                  </div>
                );
            ...
            {form.premios.length<10&&(
              <button
                onClick={()=>{
                  ...
                  const EXTRA_COLORS=["#D4B466","#fb923c","#34d399","#60a5fa","#f472b6","#facc15","#4ade80","#D4B466"];
                ...
                style={{
                  ...
                  cursor:"pointer",transition:"all .2s",color:MUTED,fontFamily:SANS_BODY,
                  fontSize:11,fontWeight:700,
                }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=ACCENT;e.currentTarget.style.color=ACCENT;e.currentTarget.style.background="rgba(201,168,76,.04)";}}
              ...
              >
                <div style={{width:22,height:22,borderRadius:7,background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:900,color:ACCENT}}>+</div>
            ...
            {form.premios.every(p=>!p.valor)&&(
              <div style={{marginTop:10,background:"rgba(74,85,104,.08)",border:`1px solid ${BORDER}`,borderRadius:10,padding:"8px 11px",display:"flex",alignItems:"center",gap:7}}>
                ...
                <span style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.5}}>
          ...
          {/* Resumo das regras */}
          <div style={{background:"rgba(201,168,76,.05)",border:"1px solid rgba(201,168,76,.2)",borderRadius:12,padding:"12px",marginBottom:20,marginTop:12}}>
            <div style={{fontSize:9,fontWeight:800,color:ACCENT,fontFamily:SANS_BODY,marginBottom:8,letterSpacing:"1px"}}>📋 RESUMO DAS REGRAS</div>
            {[
            ...
            ].map((r,i)=><div key={i} style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>
              · {r}
            </div>)}
          </div>
        ...
        {/* ── STEP 4: Confirmar ── */}
        {step===4&&<>
          <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>Confirme sua liga</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:20}}>Revise tudo antes de criar</div>
          ...
          <div style={{background:`linear-gradient(135deg,#1a1000,#2a1800)`,border:"1px solid rgba(245,166,35,.3)",borderRadius:18,padding:"20px",marginBottom:16,textAlign:"center",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:4}}>{form.nome||"Sem nome"}</div>
            {form.descricao&&<div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:10,lineHeight:1.4}}>{form.descricao}</div>}
            <div style={{display:"inline-flex",alignItems:"center",gap:5,background:`${TIPOS.find(t=>t.id===form.tipo)?.color}22`,
              ...
              <span style={{fontSize:9,fontWeight:800,color:TIPOS.find(t=>t.id===form.tipo)?.color,fontFamily:SANS_BODY}}>
          ...
          {[
            ...
            <span style={{flex:1,fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>{d.label}</span>
            <span style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:SANS_BODY}}>{d.val}</span>
          </div>)}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {form.premios.filter(p=>p.valor).map((p,i)=>(
                  <div key={i} style={{
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontSize:8,fontWeight:800,color:p.color,letterSpacing:"0.5px",fontFamily:SANS_BODY,marginBottom:2}}>{p.label.toUpperCase()}</div>
                      <div style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:SANS_BODY}}>{p.valor}</div>
                    </div>
                  </div>
                ))}
          ...
          {!form.premios.some(p=>p.valor)&&(
            <div style={{padding:"11px 0",borderTop:`1px solid ${BORDER}`,display:"flex",alignItems:"center",gap:12}}>
              ...
              <span style={{flex:1,fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>Premiações</span>
              <span style={{fontSize:11,fontWeight:700,color:MUTED,fontFamily:SANS_BODY}}>Sem prêmios</span>
            </div>
          )}

          <button onClick={()=>setStep(5)}
            style={{width:"100%",marginTop:22,background:"linear-gradient(135deg,#C9A84C,#8a6820)",
              border:"none",borderRadius:14,padding:"15px",fontSize:13,fontWeight:900,
              color:"#000",fontFamily:SANS_BODY,cursor:"pointer"}}>
            🏆 Criar liga agora
          </button>
          <button onClick={()=>setStep(3)}
            style={{width:"100%",marginTop:8,background:"none",border:`1px solid ${BORDER}`,
              ...
              color:MUTED,fontFamily:SANS_BODY,cursor:"pointer"}}>
            Revisar regras
          </button>
        </>}
...
══════════════════════════════════════════════════════════════ */
function NewsScreen({hdr={},newsLog,prices,onBack,onExit,fireExternalNews,bgRealNews=[],bgNewsStatus="idle",bgLastFetch=null,bgNextIn=null,onManualRefresh,bgFallbackUsed=false,bgSource="rss"}){
  ...
  const catColor={"Financeira Crítica":"#f43f5e","Esportiva Majoritária":"#C9A84C","Esportiva Menor":"#D4B466","Mercado de Ativos":"#f5a623","Integridade/Saúde":"#C9A84C","Institucional":"#8899b0"};
...
{"resumo":"análise 2-3 frases","pontos_positivos":["até 3 fatores bullish"],"pontos_negativos":["até 3 fatores bearish"],"sentimento_geral":"BULLISH|NEUTRO|BEARISH","recomendacao":"COMPRAR|MANTER|VENDER","justificativa":"1 frase","nivel_risco":"BAIXO|MÉDIO|ALTO","noticias_recentes":[{"titulo":"manchete","sent":0.8,"emoji":"⚽","fonte":"portal"}]}`,
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    ...
    {/* ── CABEÇALHO PADRÃO ── */}
    <AppHeader hdr={hdr} pageTitle="📰 Notícias" onBack={onBack} onExit={onExit}>
      {/* Tabs Feed / Análise IA — slot de conteúdo abaixo do título */}
      <div style={{display:"flex",gap:4,background:SURFACE,borderRadius:10,padding:3}}>
        {[{id:"feed",l:"📋 Feed"},{id:"analysis",l:"🤖 Análise IA"}].map(t=>(
          <button key={t.id} onClick={()=>setActiveTab(t.id)}
            style={{flex:1,background:activeTab===t.id?"linear-gradient(135deg,rgba(201,168,76,.2),rgba(212,180,102,.12))":"transparent",border:`1px solid ${activeTab===t.id?"rgba(201,168,76,.3)":"transparent"}`,borderRadius:7,padding:"6px 4px",cursor:"pointer",fontSize:9,fontWeight:activeTab===t.id?800:600,color:activeTab===t.id?"#fff":MUTED,fontFamily:SANS_BODY,transition:"all .18s"}}>
            {t.l}
          </button>
        ))}
      </div>
    ...
    {/* TAB: FEED */}
    {activeTab==="feed"&&<>
      {/* ── Status da busca + filtros ── */}
      <div style={{padding:"8px 14px 6px",flexShrink:0}}>
        {/* Linha de status compacta */}
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
          {loading
            ?<div style={{width:7,height:7,borderRadius:"50%",border:`2px solid ${ACCENT}`,borderTop:"2px solid transparent",animation:"spin 0.8s linear infinite",flexShrink:0}}/>
            :<div style={{width:7,height:7,borderRadius:"50%",background:bgNewsStatus==="error"?RED:ACCENT,boxShadow:`0 0 5px ${bgNewsStatus==="error"?RED:ACCENT}80`,flexShrink:0,animation:"pls 2s infinite"}}/>}
          <span style={{fontSize:8,color:loading?"#a8b8cc":MUTED,fontFamily:SANS_BODY,flex:1}}>
            {loading?"Buscando notícias...":bgNewsStatus==="error"?"Erro na busca":bgSource==="web_search"?"Via web_search":bgSource==="stale"?"Cache (temporário)":"Atualizado via RSS"}
            {bgLastFetch&&!loading&&<> · {fmtAgo(bgLastFetch)}</>}
            {!loading&&bgNextIn!=null&&bgNextIn>0&&<> · ↻ {fmtCountdown(bgNextIn)}</>}
          </span>
          {bgFallbackUsed&&<span style={{fontSize:8,background:"rgba(245,166,35,.12)",color:GOLD,borderRadius:5,padding:"1px 6px",fontWeight:700,border:"1px solid rgba(245,166,35,.25)"}}>⚡ web_search</span>}
          {bgSource==="stale"&&<span style={{fontSize:8,background:"rgba(244,63,94,.10)",color:RED,borderRadius:5,padding:"1px 6px",fontWeight:700,border:"1px solid rgba(244,63,94,.2)"}}>⚠ cache</span>}
          <button onClick={onManualRefresh} disabled={loading}
            style={{background:loading?"rgba(255,255,255,.04)":"rgba(201,168,76,.08)",border:`1px solid ${loading?"transparent":"rgba(201,168,76,.3)"}`,borderRadius:7,padding:"3px 8px",cursor:loading?"not-allowed":"pointer",fontSize:8,fontWeight:800,color:loading?MUTED:ACCENT,fontFamily:SANS_BODY,flexShrink:0}}>
            {loading?"...":"↻"}
          </button>
        </div>
        {/* Busca + dropdown de clube */}
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <div style={{flex:1,background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:10,padding:"6px 10px",display:"flex",alignItems:"center",gap:7}}>
            <span style={{fontSize:11,opacity:.45}}>🔍</span>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar notícias..."
              style={{flex:1,background:"transparent",border:"none",color:"#fff",fontSize:11,fontFamily:SANS_BODY,outline:"none",caretColor:ACCENT}}/>
            {search&&<button onClick={()=>setSearch("")} style={{background:"none",border:"none",color:MUTED,cursor:"pointer",fontSize:12,padding:0,lineHeight:1}}>✕</button>}
          </div>
          <div style={{position:"relative",flexShrink:0}}>
            <select value={tickerFilter} onChange={e=>setTickerFilter(e.target.value)}
              style={{appearance:"none",WebkitAppearance:"none",background:tickerFilter!=="Todos"?"rgba(201,168,76,.12)":SURFACE,border:`1px solid ${tickerFilter!=="Todos"?"rgba(201,168,76,.4)":BORDER}`,borderRadius:10,padding:"7px 28px 7px 10px",color:tickerFilter!=="Todos"?ACCENT:"rgba(255,255,255,.7)",fontSize:10,fontWeight:tickerFilter!=="Todos"?800:600,fontFamily:SANS_BODY,cursor:"pointer",outline:"none",minWidth:90}}>
              <option value="Todos" style={{background:"#141210",color:"#fff"}}>⚽ Todos</option>
              {[...new Set(deduped.map(n=>n.ticker).filter(Boolean))].sort().map(t=>{
                const cl=CLUBS.find(c=>c.ticker===t);
                return <option key={t} value={t} style={{background:"#141210",color:"#fff"}}>{t}{cl?` · ${cl.realName||cl.name}`:""}</option>;
              })}
            </select>
            <span style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",fontSize:9,color:tickerFilter!=="Todos"?ACCENT:MUTED,pointerEvents:"none"}}>▼</span>
          </div>
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 14px 16px"}}>
        {loading&&bgRealNews.length===0&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"40px 20px",gap:10}}>
          <div style={{width:36,height:36,borderRadius:"50%",border:`3px solid rgba(201,168,76,.2)`,borderTop:`3px solid ${ACCENT}`,animation:"spin 1s linear infinite"}}/>
          <div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY,textAlign:"center"}}>Buscando notícias reais na web...<br/><span style={{fontSize:9,color:"rgba(255,255,255,.38)"}}>Anthropic Web Search</span></div>
        </div>}
        {!loading&&feedFiltered.length===0&&<div style={{textAlign:"center",padding:"40px 20px"}}>
          <div style={{fontSize:32,marginBottom:10}}>🌐</div>
          <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:6}}>Aguardando notícias...</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>A busca em segundo plano está em andamento.</div>
        </div>}
        {feedFiltered.map((n,i)=>{
          ...
          return <div key={n.id||i} style={{background:up?"rgba(201,168,76,.04)":"rgba(244,63,94,.04)",border:`1px solid ${up?"rgba(201,168,76,.2)":"rgba(244,63,94,.2)"}`,borderRadius:14,padding:"12px 13px",marginBottom:8,position:"relative",overflow:"hidden",animation:`fadeIn .3s ease ${Math.min(i,5)*.05}s both`}}>
            ...
            <div style={{paddingLeft:8}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:6,flexWrap:"wrap"}}>
                ...
                {isLive
                  ?<span style={{fontSize:7,background:"rgba(201,168,76,.08)",color:ACCENT,borderRadius:5,padding:"1px 5px",fontWeight:900,animation:"pls 2s infinite",flexShrink:0}}>● AO VIVO</span>
                  :<span style={{fontSize:7,background:"rgba(212,180,102,.1)",color:ACCENT2,borderRadius:5,padding:"1px 5px",fontWeight:800,flexShrink:0}}>🌐 REAL</span>}
                {n.fonte&&<span style={{fontSize:7,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY,flexShrink:0}}>{n.fonte}</span>}
                <span style={{fontSize:8,color:MUTED,fontFamily:MONO,marginLeft:"auto",flexShrink:0}}>{fmtAgo(n.ts)}</span>
              </div>
              <div style={{fontSize:12,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,lineHeight:1.5,marginBottom:n.resumo?5:8}}>{n.headline}</div>
              {n.resumo&&<div style={{fontSize:10,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY,lineHeight:1.4,marginBottom:8}}>{n.resumo}</div>}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{display:"flex",gap:5,alignItems:"center"}}>
                  <span style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY}}>Sentimento IA</span>
                  <span style={{fontSize:10,fontWeight:900,color:sentColor(sentVal),fontFamily:MONO}}>{up?"▲":"▼"}{Math.abs(sentVal*100).toFixed(0)}%</span>
                </div>
                {cl&&<button onClick={()=>fetchAnalysis(cl)} style={{background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.2)",borderRadius:8,padding:"3px 8px",cursor:"pointer",fontSize:8,fontWeight:800,color:ACCENT,fontFamily:SANS_BODY}}>🤖 Analisar</button>}
              </div>
            </div>
          </div>;
        })}
        <div style={{textAlign:"center",fontSize:8,color:"rgba(255,255,255,.32)",fontFamily:SANS_BODY,padding:"8px 0"}}>
    ...
    {activeTab==="analysis"&&<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{padding:"0 14px 8px",flexShrink:0}}>
        <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:6,fontWeight:700,letterSpacing:"1px"}}>SELECIONE UM CLUBE</div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {CLUBS.filter(c=>c.div==="A").map(cl=>{
            ...
            return <button key={cl.ticker} onClick={()=>fetchAnalysis(cl)} disabled={analysisLoading}
              style={{background:isSel?`${cl.color}28`:`${cl.color}10`,border:`1px solid ${isSel?cl.color:cl.color+"33"}`,borderRadius:8,padding:"5px 8px",cursor:analysisLoading?"not-allowed":"pointer",fontSize:8,fontWeight:isSel?900:700,color:isSel?cl.color:cl.color+"99",fontFamily:SANS_BODY,opacity:analysisLoading&&!isSel?.4:1,transition:"all .15s"}}>
              {cl.ticker}
            </button>;
          })}
        </div>
        {analysisError&&<div style={{background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.2)",borderRadius:8,padding:"7px 10px",marginTop:8,fontSize:9,color:"#f87171",fontFamily:SANS_BODY}}>⚠️ {analysisError}</div>}
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"0 14px 16px"}}>
        {analysisLoading&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"40px 20px",gap:10}}>
          <div style={{width:36,height:36,borderRadius:"50%",border:`3px solid rgba(201,168,76,.2)`,borderTop:`3px solid ${ACCENT}`,animation:"spin 1s linear infinite"}}/>
          <div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY,textAlign:"center"}}>Analisando {selectedClub?.realName}...<br/><span style={{fontSize:9,color:"rgba(255,255,255,.38)"}}>Buscando notícias + gerando análise</span></div>
        </div>}
        {!analysisLoading&&!analysis&&<div style={{textAlign:"center",padding:"30px 20px"}}>
          <div style={{fontSize:36,marginBottom:12}}>🤖</div>
          <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,marginBottom:6}}>Análise fundamentalista por clube</div>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.6}}>Selecione um clube para análise completa com notícias reais, sentimento e recomendação.</div>
        </div>}
        {!analysisLoading&&analysis&&(()=>{
          ...
          return <>
            <div style={{background:`linear-gradient(135deg,${cl.color}15,${cl.color}05)`,border:`1px solid ${cl.color}30`,borderRadius:16,padding:"13px",marginBottom:10,display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{cl.realName}</div><div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{cl.ticker} · FS${(a.curP||0).toFixed(2)} · {cl.mktCap}</div></div>
              <div style={{background:`${sc}18`,border:`1px solid ${sc}40`,borderRadius:10,padding:"5px 9px",textAlign:"center",flexShrink:0}}><div style={{fontSize:8,fontWeight:900,color:sc,fontFamily:SANS_BODY}}>{sentLabel(a.sentimento_geral)}</div></div>
            </div>
            <div style={{background:`${rc}0e`,border:`1px solid ${rc}40`,borderRadius:14,padding:"12px 14px",marginBottom:10,display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:2}}>RECOMENDAÇÃO IA</div><div style={{fontSize:16,fontWeight:900,color:rc,fontFamily:SANS_BODY}}>{a.recomendacao}</div><div style={{fontSize:9,color:"rgba(255,255,255,.6)",fontFamily:SANS_BODY,marginTop:2}}>{a.justificativa}</div></div>
              <div style={{background:`${rc}18`,borderRadius:8,padding:"4px 9px",flexShrink:0,textAlign:"center"}}><div style={{fontSize:7,fontWeight:800,color:rc,fontFamily:SANS_BODY}}>RISCO</div><div style={{fontSize:11,fontWeight:900,color:rc,fontFamily:SANS_BODY}}>{a.nivel_risco}</div></div>
            </div>
            <div style={{background:"rgba(255,255,255,.03)",border:`1px solid ${BORDER}`,borderRadius:12,padding:"11px 13px",marginBottom:10}}>
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,fontWeight:800,letterSpacing:"1px",marginBottom:5}}>📋 ANÁLISE</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,.8)",fontFamily:SANS_BODY,lineHeight:1.6}}>{a.resumo}</div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
              {[{l:"BULLISH",items:a.pontos_positivos||[],c:ACCENT,e:"✅"},{l:"BEARISH",items:a.pontos_negativos||[],c:RED,e:"⚠️"}].map(g=>(
                <div key={g.l} style={{background:`${g.c}08`,border:`1px solid ${g.c}22`,borderRadius:12,padding:"10px"}}>
                  <div style={{fontSize:8,fontWeight:800,color:g.c,fontFamily:SANS_BODY,marginBottom:5}}>{g.e} {g.l}</div>
                  {(g.items||[]).map((item,ii)=><div key={ii} style={{fontSize:9,color:"rgba(255,255,255,.7)",fontFamily:SANS_BODY,lineHeight:1.4,marginBottom:3}}>· {item}</div>)}
                  {(!g.items||!g.items.length)&&<div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>—</div>}
                </div>
              ))}
            </div>
            {(a.noticias_recentes||[]).length>0&&<>
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,fontWeight:800,letterSpacing:"1px",marginBottom:6}}>🌐 NOTÍCIAS RECENTES</div>
              {a.noticias_recentes.map((n,i)=>{const up=(n.sent??0)>=0;return <div key={i} style={{background:"rgba(255,255,255,.025)",border:`1px solid ${up?"rgba(201,168,76,.18)":"rgba(244,63,94,.18)"}`,borderRadius:11,padding:"9px 11px",marginBottom:6,display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:14,flexShrink:0}}>{n.emoji}</span>
                <div style={{flex:1}}><div style={{fontSize:10,color:"#fff",fontFamily:SANS_BODY,lineHeight:1.4}}>{n.titulo}</div>{n.fonte&&<div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>{n.fonte}</div>}</div>
                <span style={{fontSize:10,fontWeight:800,color:up?ACCENT:RED,fontFamily:MONO,flexShrink:0}}>{up?"▲":"▼"}</span>
              </div>;})}
            </>}
            <div style={{fontSize:8,color:"rgba(255,255,255,.36)",fontFamily:SANS_BODY,textAlign:"center",marginTop:8,lineHeight:1.6}}>⚠️ Análise gerada por IA com dados públicos. Não constitui recomendação financeira.</div>
          </>;
        })()}
      </div>
    </div>}
...
function ForumScreen({hdr={},user,prices,onBack,onExit}){
  ...
  const sq=searchForum.toLowerCase().trim();
  const filtered=posts
    .filter(p=>filterTicker==="Todos"||p.ticker===filterTicker||!p.ticker)
    .filter(p=>!sq||
      p.text?.toLowerCase().includes(sq)||
      p.ticker?.toLowerCase().includes(sq)||
      p.author?.toLowerCase().includes(sq)||
      CLUBS.find(c=>c.ticker===p.ticker)?.realName?.toLowerCase().includes(sq)||
      CLUBS.find(c=>c.ticker===p.ticker)?.name?.toLowerCase().includes(sq))
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    {/* ── CABEÇALHO PADRÃO ── */}
    <AppHeader hdr={hdr} pageTitle="💬 Comunidade" onBack={onBack} onExit={onExit}>
      {/* Busca + dropdown de time — slot abaixo do título */}
      <div style={{display:"flex",gap:6,alignItems:"center",paddingBottom:6}}>
        ...
        <div style={{flex:1,background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:10,padding:"6px 10px",display:"flex",alignItems:"center",gap:7}}>
          ...
          <input
            ...
            placeholder="Buscar posts, autor ou ticker..."
            style={{flex:1,background:"transparent",border:"none",color:"#fff",fontSize:11,fontFamily:SANS_BODY,outline:"none",caretColor:ACCENT}}/>
          {searchForum&&<button onClick={()=>setSearchForum("")} style={{background:"none",border:"none",color:MUTED,cursor:"pointer",fontSize:12,padding:0,lineHeight:1}}>✕</button>}
        </div>
        {/* Dropdown de time */}
        <div style={{position:"relative",flexShrink:0}}>
          <select
            ...
            style={{appearance:"none",WebkitAppearance:"none",background:filterTicker!=="Todos"?"rgba(201,168,76,.12)":SURFACE,border:`1px solid ${filterTicker!=="Todos"?"rgba(201,168,76,.4)":BORDER}`,borderRadius:10,padding:"7px 26px 7px 10px",color:filterTicker!=="Todos"?ACCENT:"rgba(255,255,255,.7)",fontSize:10,fontWeight:filterTicker!=="Todos"?800:600,fontFamily:SANS_BODY,cursor:"pointer",outline:"none",minWidth:88}}>
            <option value="Todos" style={{background:"#141210",color:"#fff"}}>⚽ Todos</option>
            <optgroup label="── Série A ──" style={{background:"#141210",color:MUTED}}>
              {CLUBS.filter(c=>c.div==="A").map(c=>(
                <option key={c.ticker} value={c.ticker} style={{background:"#141210",color:"#fff"}}>{c.ticker} · {c.realName||c.name}</option>
              ))}
            </optgroup>
            <optgroup label="── Série B ──" style={{background:"#141210",color:MUTED}}>
              {CLUBS.filter(c=>c.div==="B").map(c=>(
                <option key={c.ticker} value={c.ticker} style={{background:"#141210",color:"#fff"}}>{c.ticker} · {c.realName||c.name}</option>
              ))}
            </optgroup>
          </select>
          <span style={{position:"absolute",right:7,top:"50%",transform:"translateY(-50%)",fontSize:9,color:filterTicker!=="Todos"?ACCENT:MUTED,pointerEvents:"none"}}>▼</span>
        </div>
      </div>
    </AppHeader>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"0 14px 8px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0 8px"}}>
        <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{filtered.length} comentário{filtered.length!==1?"s":""}{club?` · ${club.realName||club.name}`:""}{sq?` · "${searchForum}"`:""}</div>
        <div style={{display:"flex",gap:4}}>
          {[{id:"recente",l:"Recentes"},{id:"likes",l:"Top Likes"}].map(s=>(
            <button key={s.id} onClick={()=>setSortBy(s.id)} style={{padding:"3px 9px",borderRadius:8,border:`1px solid ${sortBy===s.id?ACCENT:BORDER}`,background:sortBy===s.id?"rgba(201,168,76,.06)":"transparent",color:sortBy===s.id?ACCENT:MUTED,fontSize:8,fontWeight:800,cursor:"pointer",fontFamily:SANS_BODY}}>{s.l}</button>
          ))}
        </div>
      </div>
      {filtered.length===0&&<div style={{textAlign:"center",padding:"40px 20px"}}>
        <div style={{fontSize:28,marginBottom:10}}>{sq||filterTicker!=="Todos"?"🔍":"💬"}</div>
        <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{sq||filterTicker!=="Todos"?"Nenhum resultado":"Nenhum comentário ainda"}</div>
        <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginTop:4}}>{sq?`Nenhum post encontrado para "${searchForum}"`:filterTicker!=="Todos"?`Ainda não há posts sobre ${club?.realName||club?.name||filterTicker}`:"Seja o primeiro a comentar!"}</div>
      </div>}
      {filtered.map(post=>{
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:9,marginBottom:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2,flexWrap:"wrap"}}>
                ...
                {isOwn&&<span style={{fontSize:7,color:ACCENT,fontFamily:SANS_BODY,opacity:.6}}>você</span>}
              </div>
              <div style={{fontSize:11,color:"rgba(255,255,255,.85)",fontFamily:SANS_BODY,lineHeight:1.6,wordBreak:"break-word"}}>{post.text}</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <span style={{fontSize:8,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY}}>{fmtAgo(post.ts)}</span>
            <button onClick={()=>toggleLike(post.id)} style={{display:"flex",alignItems:"center",gap:4,background:"none",border:"none",cursor:"pointer",padding:"3px 8px",borderRadius:8,background:isLiked?"rgba(201,168,76,.08)":"rgba(255,255,255,.04)"}}>
      ...
      <div style={{background:CARD,borderRadius:14,border:`1px solid ${BORDER}`,padding:"10px 12px"}}>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          ...
          <textarea
            ...
            style={{flex:1,background:"transparent",border:"none",color:"#fff",fontSize:11,fontFamily:SANS_BODY,outline:"none",resize:"none",lineHeight:1.5,minHeight:44,caretColor:"#C9A84C"}}
            rows={2}
          />
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:7,color:"rgba(255,255,255,.38)",fontFamily:SANS_BODY}}>• e-mail, fone e CPF são removidos</span>
          </div>
          <button onClick={handlePost} disabled={!text.trim()||posting}
            style={{padding:"6px 14px",borderRadius:10,border:"none",cursor:text.trim()?"pointer":"default",background:text.trim()?`linear-gradient(135deg,${ACCENT},#5b52e8)`:"rgba(255,255,255,.08)",color:text.trim()?BG:"rgba(255,255,255,.2)",fontSize:10,fontWeight:800,fontFamily:SANS_BODY,transition:"all .2s"}}>
            {posting?"...":"Publicar →"}
          </button>
        </div>
      </div>
...
5. Se a notícia afeta múltiplos clubes, crie uma entrada por clube`,
  ...
  const pill=a=>({flex:1,padding:"7px 4px",borderRadius:8,border:"none",cursor:"pointer",fontSize:10,fontWeight:a?600:400,background:a?"rgba(201,168,76,.12)":"transparent",color:a?ACCENT:MUTED,fontFamily:SANS_BODY,transition:"all .15s",letterSpacing:"0.1px"});
    ...
    return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
      ...
      <div style={{padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${BORDER}`,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          ...
          <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>FootStock</div>
        </div>
        <button onClick={onExit} style={{background:"rgba(244,63,94,.1)",border:"1px solid rgba(244,63,94,.25)",color:RED,borderRadius:8,padding:"5px 11px",fontSize:10,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:700}}>Sair</button>
      </div>
      {/* conteúdo */}
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 28px",textAlign:"center"}}>
        ...
        <div style={{fontSize:10,color:RED,fontWeight:800,letterSpacing:"2px",fontFamily:SANS_BODY,marginBottom:8}}>CONTA SUSPENSA</div>
        <div style={{fontSize:20,fontWeight:900,color:"#fff",fontFamily:SANS_BODY,letterSpacing:"-0.5px",marginBottom:10,lineHeight:1.2}}>Sua assinatura<br/><span style={{color:RED}}>está vencida</span></div>
        <div style={{fontSize:12,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.7,marginBottom:24}}>
        ...
        <div style={{width:"100%",background:"rgba(244,63,94,.06)",border:"1px solid rgba(244,63,94,.2)",borderRadius:16,padding:"16px",marginBottom:16}}>
          <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginBottom:12}}>Suas posições e histórico estão preservados.</div>
          <button onClick={()=>onUpgrade&&onUpgrade(planLabel,planLabel,(plan,bil)=>applyUpgrade(plan==="craque"?"Craque":"Lenda",bil||planBillingCycle))}
            style={{width:"100%",padding:"15px",borderRadius:14,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${planCol},${userPlan==="Lenda"?"#e8830a":ACCENT2})`,color:BG,fontSize:14,fontWeight:900,fontFamily:SANS_BODY,boxShadow:`0 6px 24px ${planCol}40`,marginBottom:10}}>
            Renovar Plano {planLabel} →
          </button>
          <button onClick={()=>{setUserPlan("Jogador");setPlanDue(null);}}
            style={{width:"100%",padding:"11px",borderRadius:12,border:`1px solid ${BORDER}`,background:"transparent",color:MUTED,fontSize:12,fontWeight:700,fontFamily:SANS_BODY,cursor:"pointer"}}>
            Continuar como Jogador (grátis)
          </button>
        </div>
        <div style={{fontSize:9,color:"rgba(255,255,255,.2)",fontFamily:SANS_BODY}}>Dúvidas? contato@footstock.com.br</div>
      </div>
    </div>;
  }

  if(showExtrato)   return <ExtratoScreen hdr={{user,marketSession,userPlan,planDue,isSuspended,news,newsLog,adIdx,onShowNews:()=>setShowExtrato(false),onExit}} txLog={txLog} scheduledOrders={scheduledOrders} limitedOrders={limitedOrders} ocoOrders={ocoOrders} cancelScheduled={id=>setScheduledOrders(p=>p.map(o=>o.id===id?{...o,status:"cancelled",cancelledAt:new Date().toISOString()}:o))} cancelLimited={id=>setLimitedOrders(p=>p.map(o=>o.id===id?{...o,status:"cancelled",cancelledAt:new Date().toISOString()}:o))} cancelOco={id=>setOcoOrders(p=>p.map(o=>o.id===id?{...o,status:"cancelled",cancelledAt:new Date().toISOString()}:o))} onBack={()=>setShowExtrato(false)} onExit={onExit}/>;
  if(showForum)     return <ForumScreen hdr={{user,marketSession,userPlan,planDue,isSuspended,news,newsLog,adIdx,onShowNews:()=>setShowNews(true),onExit}} user={user} prices={prices} onBack={()=>setShowForum(false)} onExit={onExit}/>;
  if(showGlossary)  return <GlossaryScreen hdr={{user,marketSession,userPlan,planDue,isSuspended,news,newsLog,adIdx,onShowNews:()=>setShowNews(true),onExit}} onBack={()=>setShowGlossary(false)} onExit={onExit}/>;
  if(showNews)      return <NewsScreen hdr={{user,marketSession,userPlan,planDue,isSuspended,news,newsLog,adIdx,onShowNews:null,onExit}} newsLog={newsLog} prices={prices} onBack={()=>setShowNews(false)} onExit={onExit} fireExternalNews={fireExternalNews} bgRealNews={bgRealNews} bgNewsStatus={bgNewsStatus} bgLastFetch={bgLastFetch} bgNextIn={bgNextIn} onManualRefresh={fetchBgNews} bgFallbackUsed={bgFallbackUsed} bgSource={bgSource}/>;
  if(showProfile)   return <ProfileScreen hdr={{user,marketSession,userPlan,planDue,isSuspended,news,newsLog,adIdx,onShowNews:()=>setShowNews(true),onExit}} user={user} onBack={()=>setShowProfile(false)} onExit={onExit}/>;
  if(showCriarLiga) return <CriarLigaScreen hdr={{user,marketSession,userPlan,planDue,isSuspended,news,newsLog,adIdx,onShowNews:()=>setShowNews(true),onExit}} onBack={()=>setShowCriarLiga(false)} onExit={onExit} user={user}/>;
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden",position:"relative"}}>
    ...
    {showClosedAlert&&<div style={{position:"absolute",inset:0,zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",background:"rgba(0,0,0,.6)",backdropFilter:"blur(6px)"}} onClick={()=>setShowClosedAlert(false)}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"linear-gradient(180deg,#130f08,#080808)",borderRadius:"24px 24px 0 0",padding:"8px 20px 32px",boxShadow:"0 -8px 40px rgba(0,0,0,.7)",border:"1px solid rgba(244,63,94,.18)",borderBottom:"none"}}>
        {/* Puxador */}
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6}}>
          ...
          <div>
            <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY,lineHeight:1.1}}>Mercado Fechado</div>
            <div style={{fontSize:10,color:"rgba(244,63,94,.85)",fontWeight:700,fontFamily:SANS_BODY,marginTop:3}}>Operações indisponíveis neste momento</div>
          </div>
        </div>
        <div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.6,marginBottom:16}}>
          Você pode explorar ativos, acompanhar notícias e montar sua estratégia. As ordens serão liberadas na próxima sessão.
        </div>
        {/* Tabela de horários */}
        <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"1.2px",fontFamily:SANS_BODY,marginBottom:8}}>HORÁRIO DE FUNCIONAMENTO · BRASÍLIA</div>
        <div style={{borderRadius:14,overflow:"hidden",border:`1px solid ${BORDER}`,marginBottom:20}}>
          {[
            ...
            {id:"closing", emoji:"🔵", label:"Call de Fechamento", horario:"00h45 – 01h00", color:"#D4B466"},
            {id:"after",   emoji:"🟣", label:"After-Market",       horario:"01h00 – 01h30", color:"#C9A84C"},
            ...
            <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 13px",background:s.id==="closed"?"rgba(244,63,94,.06)":"transparent",borderBottom:i<arr.length-1?`1px solid ${BORDER}`:"none"}}>
              ...
              <span style={{flex:1,fontSize:10,fontWeight:s.id==="closed"?800:600,color:s.id==="closed"?"#fff":"rgba(255,255,255,.65)",fontFamily:SANS_BODY}}>{s.label}</span>
              <span style={{fontSize:10,fontWeight:700,color:s.id==="closed"?s.color:"rgba(255,255,255,.6)",fontFamily:MONO}}>{s.horario}</span>
              {s.id==="closed"&&<span style={{fontSize:8,background:"rgba(244,63,94,.15)",color:"#f43f5e",borderRadius:6,padding:"1px 6px",fontWeight:800,fontFamily:SANS_BODY}}>AGORA</span>}
            </div>
          ))}
        </div>
        <button onClick={()=>setShowClosedAlert(false)} style={{width:"100%",padding:"14px",borderRadius:14,border:"none",cursor:"pointer",background:"linear-gradient(135deg,#C9A84C,#8a6820)",color:"#fff",fontSize:14,fontWeight:800,fontFamily:SANS_BODY,boxShadow:`0 4px 20px rgba(201,168,76,.2)`}}>
    ...
    <div style={{padding:"6px 14px 0",position:"relative",zIndex:20,flexShrink:0,transition:"margin-top .3s"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        <div style={{position:"relative"}}>
          <div onClick={()=>setMenuOpen(v=>!v)} style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",padding:"4px 7px 4px 4px",borderRadius:11,background:menuOpen?"rgba(255,255,255,.06)":"transparent",transition:"background .15s"}}>
            <div style={{width:32,height:32,borderRadius:9,background:"linear-gradient(145deg,#1a1510,#0e0c09)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 0 1px rgba(201,168,76,.2),0 4px 16px rgba(201,168,76,.08)"}}><FootStockLogo size={26} rounded={false}/></div>
            <div><div style={{fontSize:14,fontWeight:700,color:"#fff",letterSpacing:"-0.3px",lineHeight:1,fontFamily:SANS_BODY,display:"flex",alignItems:"baseline",gap:4}}>FootStock<span style={{fontSize:8,fontWeight:900,color:GOLD,letterSpacing:"2px",fontFamily:SANS_BODY}}>BETA</span></div><div style={{fontSize:7,color:"rgba(201,168,76,.8)",fontWeight:600,letterSpacing:"2px",marginTop:1}}>A BOLSA DO FUTEBOL</div></div>
            <span style={{fontSize:9,color:MUTED,transition:"transform .2s",transform:menuOpen?"rotate(180deg)":"rotate(0deg)"}}>▾</span>
          </div>
          {menuOpen&&<LogoDropdown user={user} onProfile={()=>{setShowProfile(true);setMenuOpen(false);}} onAdmin={()=>{setShowAdmin(true);setMenuOpen(false);}} onGlossary={()=>{setShowGlossary(true);setMenuOpen(false);}} onExit={onExit} onClose={()=>setMenuOpen(false)}/>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          {/* Sessão de mercado */}
          ...
          {(()=>{
            ...
            if(isSuspended) return <div style={{background:"rgba(244,63,94,.18)",border:"1px solid rgba(244,63,94,.4)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:RED,fontFamily:SANS_BODY}}>🔴 SUSPENSO</div>;
            if(expiring) return <div style={{background:"rgba(245,158,11,.15)",border:"1px solid rgba(245,158,11,.4)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:GOLD,fontFamily:SANS_BODY}}>⚠ {daysLeft===0?"vence hoje":`${daysLeft}d`}</div>;
            return <div style={{background:userPlan==="Lenda"?"linear-gradient(135deg,#C9A84C,#a07828)":userPlan==="Craque"?"linear-gradient(135deg,#C9A84C,#8a6820)":"rgba(255,255,255,.08)",borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:800,color:userPlan==="Lenda"?"#1a0800":userPlan==="Craque"?"#062011":"#fff",fontFamily:SANS_BODY}}>{userPlan==="Lenda"?"👑":userPlan==="Craque"?"⭐":"⚡"} {userPlan.toUpperCase()}</div>;
          })()}
      ...
      {(()=>{
        const ads=[
          {bg:"linear-gradient(135deg,#1a1a2e,#16213e)",border:"rgba(201,168,76,.3)",
           label:"PATROCINADO",labelColor:ACCENT,
           logo:"🏦",logoBg:"rgba(201,168,76,.12)",
          ...
          {bg:"linear-gradient(135deg,#001a2e,#00264d)",border:"rgba(201,168,76,.25)",
           label:"PATROCINADO",labelColor:ACCENT2,
           logo:"📱",logoBlg:"rgba(201,168,76,.12)",
      ...
      {(news||newsLog[0])&&(()=>{
        ...
        return <div onClick={()=>setShowNews(true)} style={{background:"rgba(255,255,255,.04)",border:`1px solid ${n.sent>=0?"rgba(201,168,76,.2)":"rgba(244,63,94,.25)"}`,borderRadius:9,padding:"4px 10px",marginBottom:7,display:"flex",alignItems:"center",gap:6,overflow:"hidden",cursor:"pointer",transition:"opacity .15s"}} onMouseEnter={e=>e.currentTarget.style.opacity=".8"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
          ...
          {news&&<span style={{fontSize:8,color:"rgba(201,168,76,.6)",fontFamily:SANS_BODY,flexShrink:0,whiteSpace:"nowrap"}}>AO VIVO</span>}
          <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,flex:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{n.headline}</span>
          <span style={{fontSize:8,color:MUTED,flexShrink:0,opacity:.5}}>›</span>
        </div>;
      })()}
    ...
    <div style={{flex:1,overflowY:"auto",padding:"10px 14px",position:"relative",zIndex:1}}>
      ...
      {(()=>{
        ...
        return <div style={{margin:"0 14px 10px",background:"rgba(245,158,11,.08)",border:"1px solid rgba(245,158,11,.3)",borderRadius:12,padding:"10px 13px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:11,fontWeight:800,color:GOLD,fontFamily:SANS_BODY,marginBottom:2}}>
              {daysLeft===0?"Sua assinatura vence hoje!":`Sua assinatura vence em ${daysLeft} dia${daysLeft!==1?"s":""}`}
            </div>
            <div style={{fontSize:9,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY}}>Renove para não perder acesso às operações.</div>
          </div>
          <button onClick={()=>onUpgrade&&onUpgrade(userPlan,userPlan,(plan,bil)=>applyUpgrade(plan==="craque"?"Craque":"Lenda",bil||planBillingCycle))}
            style={{background:`linear-gradient(135deg,${planCol},${userPlan==="Lenda"?"#e8830a":ACCENT2})`,border:"none",borderRadius:9,padding:"7px 11px",fontSize:10,fontWeight:800,color:BG,fontFamily:SANS_BODY,cursor:"pointer",flexShrink:0}}>
            Renovar
          </button>
        </div>;
      })()}
      {tab==="Dashboard"&&(()=>{
        ...
        return <>
          {/* ── PATRIMÔNIO HERO ── */}
          <div style={{background:"linear-gradient(135deg,rgba(201,168,76,.06),rgba(212,180,102,.04))",border:"1px solid rgba(201,168,76,.12)",borderRadius:20,padding:"16px 16px 14px",marginBottom:10,position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:-50,right:-50,width:150,height:150,background:"radial-gradient(circle,rgba(201,168,76,.06),transparent 70%)",borderRadius:"50%",pointerEvents:"none"}}/>
            <div style={{fontSize:9,color:"rgba(255,255,255,.55)",fontWeight:600,letterSpacing:"1.5px",marginBottom:5,fontFamily:SANS_BODY}}>PATRIMÔNIO TOTAL<InfoTip term="Patrimônio Total"/></div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:10,fontWeight:800,color:hasData&&dashDiff!==0?(isUp?ACCENT:RED):MUTED,fontFamily:MONO,background:hasData&&dashDiff!==0?(isUp?"rgba(201,168,76,.08)":"rgba(244,63,94,.1)"):"rgba(100,116,139,.08)",border:`1px solid ${hasData&&dashDiff!==0?(isUp?"rgba(201,168,76,.3)":"rgba(244,63,94,.25)"):"rgba(100,116,139,.2)"}`,borderRadius:8,padding:"2px 7px"}}>
              ...
              {hasData&&<span style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Últimos {DASH_PERIOD_LABELS[dashPeriod]}</span>}
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:8}}>
            ...
            <div style={{...cd,padding:"12px 13px",position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",bottom:-8,right:-8,width:50,height:50,background:"radial-gradient(circle,rgba(201,168,76,.08),transparent)",borderRadius:"50%"}}/>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
                <div style={{width:20,height:20,borderRadius:6,background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10}}>💵</div>
                <span style={{fontSize:8,fontWeight:600,color:ACCENT,letterSpacing:"0.5px",fontFamily:SANS_BODY}}>SALDO LIVRE<InfoTip term="Saldo Disponível"/></span>
              </div>
              ...
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:6}}>Disponível p/ investir</div>
              <div style={{height:3,borderRadius:2,background:BORDER,overflow:"hidden"}}>
                <div style={{height:"100%",width:`${patrimonio>0?(balance/patrimonio*100).toFixed(0):100}%`,background:"linear-gradient(90deg,#C9A84C,#D4B466)",borderRadius:2,transition:"width .5s"}}/>
              </div>
              <div style={{fontSize:7,color:MUTED,marginTop:3,fontFamily:SANS_BODY}}>{patrimonio>0?(balance/patrimonio*100).toFixed(1):100}% do patrimônio</div>
            </div>
            ...
            <div style={{...cd,padding:"12px 13px",position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",bottom:-8,right:-8,width:50,height:50,background:"radial-gradient(circle,rgba(201,168,76,.08),transparent)",borderRadius:"50%"}}/>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
                <div style={{width:20,height:20,borderRadius:6,background:"rgba(201,168,76,.07)",border:"1px solid rgba(201,168,76,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10}}>📊</div>
                <span style={{fontSize:7,fontWeight:800,color:ACCENT2,letterSpacing:"0.8px",fontFamily:SANS_BODY}}>INVESTIDO<InfoTip term="Rentabilidade"/></span>
              </div>
              <div style={{fontSize:14,fontWeight:900,color:"#fff",fontFamily:MONO,marginBottom:2}}>
                FS${portI>0?portI.toLocaleString("pt-BR",{minimumFractionDigits:2}):"—"}
              </div>
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:6}}>{portE.length>0?`Em ${portE.length} ativo${portE.length>1?"s":""}` :"Nenhum ativo ainda"}</div>
              <div style={{height:3,borderRadius:2,background:BORDER,overflow:"hidden"}}>
              ...
              <div style={{fontSize:7,color:MUTED,marginTop:3,fontFamily:SANS_BODY}}>{patrimonio>0&&portI>0?(portI/patrimonio*100).toFixed(1):0}% do patrimônio</div>
            </div>
          </div>

          {/* ── RENTABILIDADE TOTAL ── */}
          <div style={{background:portPL>=0?"linear-gradient(135deg,rgba(201,168,76,.06),rgba(201,168,76,.02))":"linear-gradient(135deg,rgba(244,63,94,.07),rgba(244,63,94,.02))",border:`1px solid ${portPL>=0?"rgba(201,168,76,.2)":"rgba(244,63,94,.2)"}`,borderRadius:16,padding:"12px 14px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
            {/* Gauge mini circular */}
            {(()=>{
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:8,fontWeight:800,color:portPL>=0?ACCENT:RED,letterSpacing:"1px",marginBottom:3,fontFamily:SANS_BODY}}>RENTABILIDADE TOTAL<InfoTip term="Rentabilidade"/></div>
          ...
          <div style={{...cd,overflow:"hidden",marginBottom:10}}>
            ...
            <div style={{padding:"10px 12px 7px",display:"flex",alignItems:"flex-start",justifyContent:"space-between",borderBottom:`1px solid ${BORDER}`}}>
              <div>
                <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"1px",marginBottom:2,fontFamily:SANS_BODY}}>EVOLUÇÃO DA CARTEIRA</div>
              ...
              <div style={{textAlign:"right"}}>
                ...
                </>:<div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,textAlign:"right",marginTop:2}}>{hasData?"Sem variação no período":"0,00%"}</div>}
              </div>
            </div>
            {/* Chart body */}
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"4px 8px 10px",gap:3}}>
              {Object.keys(DASH_PERIODS).map(p=>{
                ...
                return <button key={p} onClick={()=>{setDashPeriod(p);setDashAnimKey(k=>k+1);}} style={{flex:1,background:active?"rgba(201,168,76,.08)":"transparent",border:`1px solid ${active?"rgba(201,168,76,.4)":"transparent"}`,borderRadius:8,padding:"5px 2px",fontSize:9,fontWeight:active?900:600,color:active?ACCENT:MUTED,fontFamily:SANS_BODY,cursor:"pointer",transition:"all .18s",letterSpacing:p==="TOTAL"?"-0.2px":"0"}}>
                  {p}
                </button>;
              })}
            </div>
          ...
          {/* ── POSIÇÕES / TOP AÇÕES ── */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
            <div style={{fontSize:8,fontWeight:800,color:MUTED,letterSpacing:"1.5px",fontFamily:SANS_BODY}}>
              {portE.length>0?<>MINHAS POSIÇÕES<InfoTip term="Carteira (Portfolio)"/></>:"DESTAQUES DO MERCADO"}
            </div>
            {portE.length>0&&<button onClick={()=>{setTab("Carteira");setClub(null);}} style={{background:"none",border:`1px solid ${BORDER}`,borderRadius:20,padding:"3px 9px",fontSize:8,fontWeight:700,color:ACCENT2,cursor:"pointer",fontFamily:SANS_BODY}}>Ver carteira →</button>}
          </div>
              ...
              return <div key={c.ticker} style={{...cd,padding:"9px 11px",marginBottom:6,display:"flex",alignItems:"center",gap:9,cursor:"pointer",transition:"border-color .2s",animation:`fadeIn .3s ease ${i*.05}s both`}}
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                    <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{c.ticker}</span>
                    <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>{c.name}</span>
                  </div>
                  <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>Rec: {c.revenueLabel} · EV {c.evMult}×</div>
                </div>
          ...
          {portE.length>0&&<>
            {portE.map((p,i)=>{
              ...
              return <div key={p.ticker} style={{...cd,padding:"10px 11px",marginBottom:6,cursor:"pointer",transition:"border-color .2s",animation:`fadeIn .3s ease ${i*.05}s both`}}
                ...
                onMouseEnter={e=>e.currentTarget.style.borderColor=isPos?"rgba(201,168,76,.3)":"rgba(244,63,94,.35)"}
                onMouseLeave={e=>e.currentTarget.style.borderColor=BORDER}>
                <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:6}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                      <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{p.ticker}</span>
                      <span style={{fontSize:7,fontWeight:700,color:MUTED,background:SURFACE,borderRadius:4,padding:"1px 5px",fontFamily:SANS_BODY}}>{p.qty} cotas</span>
                    </div>
                    <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>PM FS${p.avg.toFixed(2)} · Atual FS${(prices[p.ticker]||p.price).toFixed(2)}</div>
                  </div>
                  <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
          ...
          <div style={{marginTop:14,marginBottom:4}}>
            <button
              ...
              style={{width:"100%",padding:"15px",borderRadius:16,border:"none",cursor:"pointer",
                ...
                color:BG,fontSize:14,fontWeight:800,fontFamily:SANS_BODY,
                display:"flex",alignItems:"center",justifyContent:"center",gap:8,
                boxShadow:"0 8px 32px rgba(201,168,76,.2)",
                transition:"all .2s",
                letterSpacing:"-0.2px",
              }}
              onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.02)";e.currentTarget.style.boxShadow="0 12px 40px rgba(201,168,76,.4)";}}
              onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.boxShadow="0 8px 32px rgba(201,168,76,.2)";}}
            >
              <span style={{fontSize:18}}>⚡</span>
              {portE.length===0?"Comece a investir agora":"Investir mais agora"}
              <span style={{fontSize:14,opacity:.8}}>→</span>
            </button>
            {portE.length===0&&<div style={{textAlign:"center",marginTop:8,fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>
            ...
            return <div key={c.ticker} onClick={()=>setClub(c)} style={{...cd,padding:"9px 11px",cursor:"pointer",display:"grid",gridTemplateColumns:"36px 1fr auto 72px",gap:7,alignItems:"center",transition:"border-color .2s",borderColor:isFav?"rgba(245,166,35,.45)":news?.ticker===c.ticker?(news.sent>=0?"rgba(201,168,76,.5)":"rgba(244,63,94,.4)"):BORDER,background:isFav?"rgba(245,166,35,.04)":CARD}} onMouseEnter={e=>e.currentTarget.style.borderColor=isFav?GOLD:ACCENT} onMouseLeave={e=>e.currentTarget.style.borderColor=isFav?"rgba(245,166,35,.45)":BORDER}>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:4,marginBottom:2}}>
                  ...
                  {isCB&&<span style={{fontSize:6,background:"rgba(245,166,35,.2)",color:GOLD,padding:"1px 4px",borderRadius:3,fontWeight:800}}>🔒LE<InfoTip term="Circuit Breaker"/></span>}
              ...
              <div style={{textAlign:"right"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:3}}>
                  <span style={{fontSize:7,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY}}>hoje</span>
      ...
      {/* ── DETALHE CLUBE ── */}
      {tab==="Mercado"&&club&&<>
        <button onClick={()=>setClub(null)} style={{background:"none",border:"none",color:MUTED,fontSize:11,cursor:"pointer",fontFamily:SANS_BODY,fontWeight:700,marginBottom:10,padding:0}}>← Mercado</button>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          <div style={{flex:1}}><div style={{fontSize:18,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>{club.ticker}</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>{club.name} · Série {club.div}</div></div>
          <div style={{textAlign:"right"}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:4}}>
                  <span style={{fontSize:9,color:"rgba(255,255,255,.5)",fontFamily:SANS_BODY}}>hoje</span>
        ...
        <div style={{...cd,padding:"10px 8px",marginBottom:10}} onClick={()=>showCompareDropdown&&setShowCompareDropdown(false)}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 6px",marginBottom:6}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              {activeIndicators.length>0&&!compareClubs.length&&<span style={{fontSize:7,background:"rgba(201,168,76,.15)",color:ACCENT2,borderRadius:5,padding:"1px 6px",fontWeight:800}}>{activeIndicators.length} IND</span>}
              {compareClubs.length>0&&<span style={{fontSize:7,background:"rgba(201,168,76,.12)",color:"#D4B466",borderRadius:5,padding:"1px 6px",fontWeight:800}}>{compareClubs.length+1} ativos</span>}
            </div>
            {/* Toggle candle/linha */}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 6px",marginBottom:7,gap:6}}>
            ...
            <div style={{display:"flex",gap:5,alignItems:"center"}}>
              <button onClick={()=>{setShowIndicatorPanel(v=>!v);setShowCompareDropdown(false);setShowBookPanel(false);}} style={{background:showIndicatorPanel?"rgba(201,168,76,.15)":"rgba(255,255,255,.06)",border:`1px solid ${showIndicatorPanel?ACCENT:BORDER}`,borderRadius:8,padding:"3px 7px",cursor:"pointer",fontSize:8,fontWeight:800,color:showIndicatorPanel?ACCENT:"#a8b8cc",fontFamily:SANS_BODY,transition:"all .15s"}}>
                📊 {showIndicatorPanel?"▲":"▼"}
              </button>
              <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}>
                <button onClick={()=>{setShowCompareDropdown(v=>!v);setShowIndicatorPanel(false);setShowBookPanel(false);}} style={{background:compareClubs.length>0?"rgba(201,168,76,.15)":showCompareDropdown?"rgba(255,255,255,.1)":"rgba(255,255,255,.06)",border:`1px solid ${compareClubs.length>0?"rgba(201,168,76,.4)":showCompareDropdown?BORDER:BORDER}`,borderRadius:8,padding:"3px 7px",cursor:"pointer",fontSize:8,fontWeight:800,color:compareClubs.length>0?"#D4B466":"#a8b8cc",fontFamily:SANS_BODY,transition:"all .15s",display:"flex",alignItems:"center",gap:4}}>
                  ⊕ Comparar {compareClubs.length>0&&<span style={{background:"#C9A84C",color:"#fff",borderRadius:10,padding:"0 4px",fontSize:7}}>{compareClubs.length}</span>}
                </button>
                {/* Dropdown comparar */}
                {showCompareDropdown&&<div style={{position:"absolute",top:"calc(100% + 4px)",right:0,width:190,background:CARD,border:`1px solid ${BORDER}`,borderRadius:12,zIndex:60,boxShadow:"0 8px 32px rgba(0,0,0,.7)",overflow:"hidden"}}>
                  <div style={{padding:"8px 11px 6px",borderBottom:`1px solid ${BORDER}`}}>
                    <div style={{fontSize:9,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Comparar ativos</div>
                    <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Até 4 adicionais · base 100%</div>
                  </div>
                  {compareClubs.length>0&&<button onClick={()=>setCompareClubs([])} style={{width:"100%",padding:"6px 11px",background:"rgba(244,63,94,.08)",border:"none",borderBottom:`1px solid ${BORDER}`,color:RED,fontSize:8,fontWeight:800,cursor:"pointer",fontFamily:SANS_BODY,textAlign:"left"}}>✕ Limpar comparação</button>}
                  <div style={{maxHeight:200,overflowY:"auto"}}>
                    {CLUBS.filter(c=>c.ticker!==club.ticker).map(cl=>{
                      ...
                      return <div key={cl.ticker} onClick={()=>{if(maxReached)return;setCompareClubs(p=>selected?p.filter(x=>x!==cl.ticker):[...p,cl.ticker]);setChartMode("line");}} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 11px",cursor:maxReached?"not-allowed":"pointer",background:selected?`${cl.color}0d`:"transparent",borderBottom:`1px solid rgba(255,255,255,.04)`,opacity:maxReached?.4:1,transition:"background .1s"}}>
                        ...
                        <div style={{flex:1}}>
                          <div style={{fontSize:10,fontWeight:700,color:selected?"#fff":"rgba(255,255,255,.65)",fontFamily:SANS_BODY}}>{cl.name}</div>
              ...
              {/* Botão Book */}
              <button onClick={()=>{setShowBookPanel(v=>!v);setShowIndicatorPanel(false);setShowCompareDropdown(false);}} style={{background:showBookPanel?"rgba(201,168,76,.12)":"rgba(255,255,255,.06)",border:`1px solid ${showBookPanel?ACCENT:BORDER}`,borderRadius:8,padding:"3px 7px",cursor:"pointer",fontSize:8,fontWeight:800,color:showBookPanel?ACCENT:"#a8b8cc",fontFamily:SANS_BODY,transition:"all .15s"}}>
                📖 Book {showBookPanel?"▲":"▼"}
              </button>
            </div>
          </div>
          ...
          {showIndicatorPanel&&<div style={{borderTop:`1px solid ${BORDER}`,padding:"8px 6px 10px",marginBottom:6}}>
            <div style={{fontSize:8,color:MUTED,fontWeight:700,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:8}}>SELECIONE OS INDICADORES</div>
            {[
              ...
              {id:"mm21",label:"Média Móvel 21",       sub:"MM21 · Médio prazo",          color:"#D4B466",icon:"📈", lenda:true},
            ].map(ind=>{
              ...
              return <div key={ind.id} onClick={()=>{if(locked){setUpgradeModal({targetPlan:"Lenda",feature:"Médias Móveis MM9 e MM21"});return;}setActiveIndicators(p=>active?p.filter(x=>x!==ind.id):[...p,ind.id]);}} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 10px",borderRadius:10,border:`1px solid ${locked?"rgba(245,166,35,.25)":active?ind.color+"66":BORDER}`,background:locked?"rgba(245,166,35,.04)":active?`${ind.color}10`:"rgba(255,255,255,.02)",cursor:"pointer",transition:"all .15s",marginBottom:6,opacity:locked?.75:1}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{fontSize:11,fontWeight:800,color:locked?"rgba(255,255,255,.4)":active?"#fff":"rgba(255,255,255,.6)",fontFamily:SANS_BODY}}>{ind.label}</span>
                    {ind.lenda&&<span style={{fontSize:7,background:isLenda?"rgba(245,166,35,.18)":"rgba(245,166,35,.28)",color:GOLD,borderRadius:4,padding:"1px 5px",fontWeight:800,fontFamily:SANS_BODY}}>👑 LENDA</span>}
                  </div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>{locked?"Exclusivo do plano Lenda":ind.sub}</div>
                </div>
            ...
            return <div style={{borderTop:`1px solid ${BORDER}`,marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:6,padding:"8px 6px 6px",alignItems:"center"}}>
                <div style={{flex:1,background:"rgba(244,63,94,.07)",border:"1px solid rgba(244,63,94,.2)",borderRadius:8,padding:"5px 8px",textAlign:"center"}}>
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:2}}>MELHOR OFERTA VENDA<InfoTip term="Ask"/></div>
                  <div style={{fontSize:13,fontWeight:900,color:RED,fontFamily:MONO}}>FS${ob.ask.toFixed(2)}</div>
                </div>
                <div style={{textAlign:"center",flexShrink:0}}>
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY}}>SPREAD<InfoTip term="Spread"/></div>
                  <div style={{fontSize:10,fontWeight:800,color:GOLD,fontFamily:MONO}}>{spreadPct}%</div>
                </div>
                <div style={{flex:1,background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",borderRadius:8,padding:"5px 8px",textAlign:"center"}}>
                  <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:2}}>MELHOR OFERTA COMPRA<InfoTip term="Bid"/></div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,padding:"0 6px 8px"}}>
                ...
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:4,marginBottom:4}}>
                    <span style={{fontSize:7,color:RED,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY}}>VENDA (ASK)</span>
                    <span style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,textAlign:"right"}}>QTD</span>
                  </div>
                ...
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:4,marginBottom:4}}>
                    <span style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY}}>QTD</span>
                    <span style={{fontSize:7,color:ACCENT,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,textAlign:"right"}}>COMPRA (BID)</span>
                  </div>
                  {bidLevels.map((l,i)=>(
                    <div key={i} style={{position:"relative",marginBottom:2}}>
                      <div style={{position:"absolute",left:0,top:0,bottom:0,width:`${(l.qty/maxQty)*100}%`,background:"rgba(201,168,76,.08)",borderRadius:3}}/>
              ...
              <div style={{borderTop:`1px solid ${BORDER}`,padding:"6px 6px 4px"}}>
                <div style={{fontSize:7,color:MUTED,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:5}}>TIME & SALES<InfoTip term="Volume"/></div>
                <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:4,marginBottom:4}}>
                  {["PREÇO","QTD","HÁ"].map(h=><span key={h} style={{fontSize:6,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,textAlign:h==="HÁ"?"right":"left"}}>{h}</span>)}
                </div>
                {trades.map((t,i)=>(
                  <div key={i} style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:4,marginBottom:3,padding:"2px 0",borderBottom:i<trades.length-1?"1px solid rgba(255,255,255,.03)":"none"}}>
                    ...
                    <span style={{fontSize:7,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY,textAlign:"right"}}>{t.ago}s</span>
                  </div>
                ))}
              </div>
            </div>;
          ...
          {compareClubs.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",padding:"0 6px",marginBottom:7}}>
            <span style={{fontSize:8,background:`${ACCENT}18`,color:ACCENT,border:`1px solid ${ACCENT}44`,borderRadius:6,padding:"2px 7px",fontWeight:800,fontFamily:SANS_BODY}}>{club.ticker} ●</span>
            {compareClubs.map(t=>{
              const cl=CLUBS.find(c=>c.ticker===t);
              return <span key={t} onClick={()=>setCompareClubs(p=>p.filter(x=>x!==t))} style={{fontSize:8,background:`${cl?.color||MUTED}18`,color:cl?.color||MUTED,border:`1px solid ${cl?.color||MUTED}44`,borderRadius:6,padding:"2px 7px",fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer"}}>{t} ✕</span>;
            })}
          </div>}
        ...
        {user?.role==="admin"&&(()=>{
          ...
          return <div style={{...cd,padding:"12px 13px",marginBottom:8}}>
            <div style={{fontSize:8,color:MUTED,fontWeight:700,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:10}}>FUNDAMENTOS DO IPO · DADOS REAIS 2024<InfoTip term="IPO (Initial Public Offering)"/></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10}}>
              <div style={{background:SURFACE,borderRadius:10,padding:"9px 11px"}}>
                <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>RECEITA ANUAL (2024)<InfoTip term="Múltiplo EV/Receita"/></div>
                <div style={{fontSize:13,fontWeight:900,color:"#fff",fontFamily:MONO}}>{cl.revenueLabel}</div>
                <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Fonte: Sports Value</div>
              </div>
              <div style={{background:SURFACE,borderRadius:10,padding:"9px 11px"}}>
                <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>EQUITY VALUE<InfoTip term="Equity Value"/></div>
                <div style={{fontSize:13,fontWeight:900,color:"#fff",fontFamily:MONO}}>{cl.mktCap}</div>
                <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>EV/Rec {cl.evMult}× − dívida</div>
              </div>
              <div style={{background:SURFACE,borderRadius:10,padding:"9px 11px"}}>
                <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>PREÇO IPO<InfoTip term="Preço IPO"/></div>
                <div style={{fontSize:13,fontWeight:900,color:GOLD,fontFamily:MONO}}>FS${cl.ipoPrice.toFixed(2)}</div>
                <div style={{fontSize:8,color:curP>cl.ipoPrice?ACCENT:RED,fontFamily:SANS_BODY,marginTop:1}}>Atual: {curP>cl.ipoPrice?"▲":"▼"} {(((curP-cl.ipoPrice)/cl.ipoPrice)*100).toFixed(1)}% vs IPO</div>
              </div>
              <div style={{background:SURFACE,borderRadius:10,padding:"9px 11px"}}>
                <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>MKT CAP LIVE<InfoTip term="MKT Cap Live"/></div>
                <div style={{fontSize:13,fontWeight:900,color:"#fff",fontFamily:MONO}}>R${mktCapLive}M</div>
                <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Free-float {Math.round(cl.ff*100)}%</div>
              </div>
            </div>
            {/* Float distribution bar */}
            <div style={{marginBottom:4,fontSize:8,color:MUTED,fontFamily:SANS_BODY,fontWeight:700}}>DISTRIBUIÇÃO DO FLOAT TOTAL<InfoTip term="Float (Oferta em Circulação)"/> ({cl.totalShares.toLocaleString("pt-BR")} cotas)</div>
            ...
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div style={{display:"flex",alignItems:"center",gap:5}}>
                ...
                <span style={{fontSize:8,color:ACCENT2,fontFamily:SANS_BODY}}>Em carteiras: {circ.toLocaleString("pt-BR")} ({circPct}%)</span>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:5}}>
                <div style={{width:8,height:8,borderRadius:2,background:BORDER}}/> 
                <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>Mercado: {sup.toLocaleString("pt-BR")} ({supPct}%)</span>
              </div>
            </div>
          </div>;
        })()}
        ...
        {(()=>{
          ...
          return <>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:6}}>
              <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.3)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:8,color:ACCENT,fontWeight:700,letterSpacing:"0.5px",marginBottom:3,fontFamily:SANS_BODY}}>BID — COMPRA<InfoTip term="Bid"/></div>
                <div style={{fontSize:16,fontWeight:900,color:ACCENT,fontFamily:MONO}}>FS${ob.bid.toFixed(2)}</div>
                <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:2}}>Melhor oferta compradora</div>
              </div>
              <div style={{background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.3)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:8,color:RED,fontWeight:700,letterSpacing:"0.5px",marginBottom:3,fontFamily:SANS_BODY}}>ASK — VENDA<InfoTip term="Ask"/></div>
                <div style={{fontSize:16,fontWeight:900,color:RED,fontFamily:MONO}}>FS${ob.ask.toFixed(2)}</div>
                <div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:2}}>Melhor oferta vendedora</div>
              </div>
            </div>
            {/* Métricas de mercado */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:6}}>
              <div style={{...cd,padding:"9px 12px"}}><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>SPREAD BID/ASK<InfoTip term="Spread"/></div><div style={{fontSize:12,fontWeight:800,color:"#fff",fontFamily:MONO}}>{spread}%</div><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Custo implícito</div></div>
              <div style={{...cd,padding:"9px 12px"}}><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>VOL. SESSÃO<InfoTip term="Volume"/></div><div style={{fontSize:12,fontWeight:800,color:"#fff",fontFamily:MONO}}>{v24.toLocaleString("pt-BR")} cotas</div><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Ordens executadas</div></div>
              <div style={{...cd,padding:"9px 12px"}}><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>FAIR VALUE (OU)<InfoTip term="Fair Value"/></div><div style={{fontSize:12,fontWeight:800,color:"#fff",fontFamily:MONO}}>FS${fairVP.toFixed(2)}</div><div style={{fontSize:8,color:fairDelta>0?ACCENT:RED,fontFamily:SANS_BODY,marginTop:1}}>{fairDelta>0?"Abaixo":"Acima"} do justo {Math.abs(fairDelta).toFixed(2)}%</div></div>
              <div style={{...cd,padding:"9px 12px"}}><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginBottom:3}}>FLOAT<InfoTip term="Float (Oferta em Circulação)"/></div><div style={{fontSize:12,fontWeight:800,color:"#fff",fontFamily:MONO}}>{(params.float/1e6).toFixed(1)}M cotas</div><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>Liquidez disponível</div></div>
            </div>
            {/* Order Flow Imbalance */}
            <div style={{...cd,padding:"10px 13px",marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <div><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,letterSpacing:"0.5px",marginBottom:1}}>ORDER FLOW IMBALANCE<InfoTip term="OFI (Order Flow Imbalance)"/></div><div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Pressão acumulada de ordens</div></div>
                <span style={{fontSize:10,fontWeight:800,color:imbCol,fontFamily:SANS_BODY}}>{imbDir} {imbPct}%</span>
              </div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:8,color:ACCENT,fontFamily:SANS_BODY}}>▲ Compra {Math.round((50+imbV*50))}%</span><span style={{fontSize:8,color:RED,fontFamily:SANS_BODY}}>Venda {Math.round((50-imbV*50))}% ▼</span></div>
              <div style={{marginTop:8,fontSize:9,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.5,borderTop:`1px solid ${BORDER}`,paddingTop:7}}>
        ...
        <div style={{...cd,padding:"12px 13px",marginBottom:10}}>
          <div style={{fontSize:8,color:MUTED,marginBottom:7,fontWeight:700,letterSpacing:"0.5px",fontFamily:SANS_BODY}}>EXECUTAR ORDEM</div>
          {(()=>{
            const sup=supply[club.ticker]||0;
            ...
            return <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,background:SURFACE,borderRadius:9,padding:"8px 10px",marginBottom:9,border:`1px solid ${BORDER}`}}>
              <div><div style={{fontSize:7,color:MUTED,letterSpacing:"0.5px",fontFamily:SANS_BODY,marginBottom:2}}>SALDO LIVRE<InfoTip term="Saldo Disponível"/></div><div style={{fontSize:11,fontWeight:900,color:ACCENT,fontFamily:MONO}}>{(balance/1000).toFixed(1)}k</div></div>
              <div style={{textAlign:"center",borderLeft:`1px solid ${BORDER}`,borderRight:`1px solid ${BORDER}`,paddingLeft:4,paddingRight:4}}>
                <div style={{fontSize:7,color:MUTED,letterSpacing:"0.5px",fontFamily:SANS_BODY,marginBottom:2}}>OFERTA NO MERCADO</div>
                <div style={{fontSize:11,fontWeight:900,color:supColor,fontFamily:MONO}}>{sup.toLocaleString("pt-BR")}</div>
                <div style={{fontSize:6,color:supColor,fontFamily:SANS_BODY,marginTop:1}}>{supLabel}</div>
                ...
                <div style={{fontSize:6,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>{supPct}% do float</div>
              </div>
              <div style={{textAlign:"right"}}><div style={{fontSize:7,color:MUTED,letterSpacing:"0.5px",fontFamily:SANS_BODY,marginBottom:2}}>EM CARTEIRA</div><div style={{fontSize:11,fontWeight:900,color:"#fff",fontFamily:MONO}}>{wallet[club.ticker]?.qty||0}</div></div>
            </div>;
          })()}
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
            <button onClick={()=>setQty(Math.max(100,qty-100))} style={{background:SURFACE,border:`1px solid ${BORDER}`,color:"#a8b8cc",width:44,height:30,borderRadius:8,cursor:"pointer",fontWeight:800,fontSize:13,fontFamily:SANS_BODY}}>−100</button>
            ...
            <button onClick={()=>setQty(qty+100)} style={{background:SURFACE,border:`1px solid ${BORDER}`,color:"#a8b8cc",width:44,height:30,borderRadius:8,cursor:"pointer",fontWeight:800,fontSize:13,fontFamily:SANS_BODY}}>+100</button>
          </div>
          {(()=>{
            ...
            return <>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>Compra via ASK</span>
                <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY}}>Venda via BID</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9,background:"rgba(245,166,35,.07)",border:"1px solid rgba(245,166,35,.15)",borderRadius:8,padding:"4px 9px"}}>
                <span style={{fontSize:8,color:"#f5a623",fontFamily:SANS_BODY,display:"flex",alignItems:"center",gap:4}}><span style={{background:"rgba(245,166,35,.2)",borderRadius:4,padding:"1px 5px",fontWeight:800,fontSize:7}}>TAXA</span>operacional</span>
              ...
              {marketSession.id!=="main"&&<div style={{background:marketSession.bg,border:`1px solid ${marketSession.border}`,borderRadius:10,padding:"7px 10px",marginBottom:9,display:"flex",alignItems:"center",gap:7}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:9,fontWeight:800,color:marketSession.color,fontFamily:SANS_BODY}}>{marketSession.label}</div>
                  <div style={{fontSize:9,color:"#fff",fontWeight:600,fontFamily:SANS_BODY,marginTop:2,opacity:.9}}>
              ...
              {scheduledOrders.filter(o=>o.ticker===club.ticker&&o.status!=="cancelled"&&o.status!=="executed").map(o=>(
                <div key={o.id} style={{background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.25)",borderRadius:10,padding:"7px 10px",marginBottom:7,display:"flex",alignItems:"center",gap:7}}>
                  <span style={{fontSize:12,flexShrink:0}}>{o.side==="buy"?"▲":"▼"}</span>
                  <div style={{flex:1}}>
                    <div style={{fontSize:9,fontWeight:800,color:"#D4B466",fontFamily:SANS_BODY}}>Ordem agendada — {o.side==="buy"?"COMPRA":"VENDA"}</div>
                    <div style={{fontSize:8,color:"rgba(255,255,255,.7)",fontFamily:SANS_BODY,marginTop:1}}>
                      {o.qty} cota{o.qty!==1?"s":""} · {o.priceType==="close"?"Preço de fechamento":`Limite FS$${o.fixedPrice?.toFixed(2)}`}
                    </div>
                  </div>
                  <button onClick={()=>setScheduledOrders(p=>p.map(x=>x.id===o.id?{...x,status:"cancelled",cancelledAt:new Date().toISOString()}:x))} style={{background:"rgba(244,63,94,.15)",border:"1px solid rgba(244,63,94,.3)",borderRadius:6,color:"#f43f5e",fontSize:10,fontWeight:800,padding:"3px 7px",cursor:"pointer",fontFamily:SANS_BODY}}>✕</button>
                </div>
              ))}
              ...
              {(()=>{
                ...
                if(canSchedule){
                  return <><div style={{display:"flex",gap:7,marginBottom:6}}>
                    <button onClick={()=>{setScheduleModal({side:"buy",ticker:club.ticker,qty,closePrice:ob.ask,club});setSchedulePriceType("close");setScheduleFixedInput("");}} style={{flex:1,padding:"12px",borderRadius:12,border:"1px solid rgba(201,168,76,.4)",cursor:"pointer",background:"rgba(201,168,76,.08)",color:ACCENT,fontSize:11,fontWeight:800,fontFamily:SANS_BODY}}>
                      ▲ AGENDAR COMPRA<div style={{fontSize:7,opacity:.8,marginTop:1}}>executa na abertura</div>
                    </button>
                    <button onClick={()=>{setScheduleModal({side:"sell",ticker:club.ticker,qty,closePrice:ob.bid,club});setSchedulePriceType("close");setScheduleFixedInput("");}} style={{flex:1,padding:"12px",borderRadius:12,border:"1px solid rgba(244,63,94,.4)",cursor:"pointer",background:"rgba(244,63,94,.1)",color:RED,fontSize:11,fontWeight:800,fontFamily:SANS_BODY}}>
                      ▼ AGENDAR VENDA<div style={{fontSize:7,opacity:.8,marginTop:1}}>executa na abertura</div>
                    </button>
                  </div>
                  <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.18)",borderRadius:9,padding:"6px 10px",textAlign:"center"}}>
                    <span style={{fontSize:8,color:"#D4B466",fontFamily:SANS_BODY,fontWeight:700}}>⚡ Benefício {user.plan} — Agendamento de ordens</span>
                  </div></>;
                }
                // Mercado fechado + plano Jogador
                return <div style={{display:"flex",gap:7}}>
                  <button style={{flex:1,padding:"12px",borderRadius:12,border:"none",cursor:"default",background:`${ACCENT}15`,color:"#555",fontSize:11,fontWeight:800,fontFamily:SANS_BODY}}>
                    ▲ COMPRAR<div style={{fontSize:7,opacity:.8}}>mercado fechado</div>
                  </button>
                  <button style={{flex:1,padding:"12px",borderRadius:12,border:"none",cursor:"default",background:`${RED}15`,color:"#555",fontSize:11,fontWeight:800,fontFamily:SANS_BODY}}>
      ...
      {tab==="Carteira"&&<>
        {(()=>{
          ...
          return (
            <div style={{background:"linear-gradient(135deg,#0d1422,#090e18)",borderRadius:18,padding:"16px",marginBottom:12,border:`1px solid ${BORDER}`,position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:-30,right:-30,width:100,height:100,background:"radial-gradient(circle,rgba(201,168,76,.08),transparent)",borderRadius:"50%"}}/>
              <div style={{display:"flex",alignItems:"flex-start",gap:14}}>

                {/* ── Coluna esquerda: label + patrimônio + métricas alinhadas ── */}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:8,color:MUTED,letterSpacing:"1.5px",fontWeight:700,marginBottom:5,fontFamily:SANS_BODY}}>PATRIMÔNIO TOTAL</div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:5}}>
                    {[
                      ...
                      <div key={row.l} style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                        <span style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY,letterSpacing:"0.5px"}}>{row.l}</span>
        ...
        <div style={{fontSize:8,color:MUTED,fontWeight:700,marginBottom:9,letterSpacing:"1px",fontFamily:SANS_BODY}}>POSIÇÕES ABERTAS<InfoTip term="Carteira (Portfolio)"/></div>
        {portEntries.length===0&&<div style={{...cd,padding:"22px",textAlign:"center"}}><div style={{fontSize:22,marginBottom:7}}>📭</div><div style={{fontSize:12,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Carteira vazia</div><div style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,marginTop:3}}>Vá ao Mercado e compre seu primeiro ativo!</div></div>}
        {portEntries.map(p=>{
          ...
          return <div key={p.ticker} onClick={()=>{setTab("Mercado");setClub(p);}} style={{...cd,padding:"11px 13px",marginBottom:7,cursor:"pointer",transition:"border-color .2s"}} onMouseEnter={e=>e.currentTarget.style.borderColor=p.color+"99"} onMouseLeave={e=>e.currentTarget.style.borderColor=BORDER}>
            <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:7}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#fff",fontFamily:SANS_BODY}}>{p.ticker}</div><div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{p.qty} cotas · PM FS${p.avg.toFixed(2)}</div></div>
            ...
            <div style={{background:SURFACE,borderRadius:4,height:4,overflow:"hidden"}}><div style={{height:"100%",borderRadius:4,width:`${Math.min(100,Math.abs(plp)*4)}%`,background:pl>0?"linear-gradient(90deg,#C9A84C,#D4B466)":"linear-gradient(90deg,#f43f5e,#ff6080)",transition:"width .5s"}}/></div>
          </div>;
        })}

        {/* ── POSIÇÕES SHORT ── */}
        {isLenda&&Object.keys(shortPositions).length>0&&<>
          <div style={{fontSize:8,color:RED,fontWeight:700,marginTop:12,marginBottom:9,letterSpacing:"1px",fontFamily:SANS_BODY,display:"flex",alignItems:"center",gap:6}}>
          ...
          {Object.entries(shortPositions).map(([ticker,pos])=>{
            ...
            return <div key={ticker} style={{background:"rgba(244,63,94,.06)",border:"1.5px solid rgba(244,63,94,.25)",borderRadius:14,padding:"11px 13px",marginBottom:7}}>
              <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:7}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:12,fontWeight:600,color:"#fff",fontFamily:SANS_BODY}}>{ticker} <span style={{fontSize:8,color:RED,fontWeight:700}}>SHORT</span></div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{pos.qty} cotas · Entrada FS${pos.entryPrice.toFixed(2)}</div>
                </div>
                <div style={{textAlign:"right"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>Margem bloqueada</span>
              ...
              <div style={{background:SURFACE,borderRadius:4,height:4,overflow:"hidden",marginBottom:8}}>
                <div style={{height:"100%",borderRadius:4,width:`${Math.min(100,Math.abs(pnlPct)*2)}%`,background:pnlVal>=0?"linear-gradient(90deg,#C9A84C,#D4B466)":"linear-gradient(90deg,#f43f5e,#ff6080)",transition:"width .5s"}}/>
              </div>
              <button onClick={()=>{
              ...
              }} style={{width:"100%",padding:"7px",borderRadius:9,border:"1px solid rgba(244,63,94,.4)",background:"rgba(244,63,94,.12)",color:"#f87171",fontSize:10,fontWeight:800,cursor:"pointer",fontFamily:SANS_BODY}}>
      ...
      {tab==="Assessor"&&user?.role==="admin"&&<>
        ...
        <div style={{...cd,overflow:"hidden",marginBottom:12}}>
          <div style={{padding:"10px 13px 8px",borderBottom:`1px solid ${BORDER}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{fontSize:8,fontWeight:800,color:MUTED,letterSpacing:"1px",fontFamily:SANS_BODY}}>HORÁRIO DE OPERAÇÕES · BRASÍLIA<InfoTip term="Sessão de Negociação"/></div>
            <div style={{background:marketSession.bg,border:`1px solid ${marketSession.border}`,borderRadius:20,padding:"2px 8px",display:"flex",alignItems:"center",gap:5}}>
              ...
              <span style={{fontSize:8,fontWeight:900,color:marketSession.color,fontFamily:SANS_BODY}}>{marketSession.label}</span>
            </div>
          </div>
          <div style={{padding:"9px 13px"}}>
            {SESSIONS.map(s=>{
              ...
              return <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 9px",borderRadius:10,marginBottom:4,background:isActive?s.bg:"transparent",border:`1px solid ${isActive?s.border:BORDER}`,transition:"all .3s"}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:10,fontWeight:isActive?900:600,color:isActive?"#fff":"#a8b8cc",fontFamily:SANS_BODY}}>{s.label}</div>
                </div>
                ...
                {isActive&&<div style={{fontSize:7,background:s.color,color:BG,borderRadius:5,padding:"1px 5px",fontWeight:800,fontFamily:SANS_BODY,flexShrink:0}}>AGORA</div>}
              </div>;
            })}
            {marketSession.id==="closed"&&<div style={{textAlign:"center",padding:"4px 0",fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>Próxima abertura: pré-mercado às <span style={{color:"#f5a623",fontWeight:700}}>10:45</span></div>}
          </div>
        </div>
        <div style={{...cd,overflow:"hidden",marginBottom:12}}>
          <div style={{background:"#0d1422",padding:"11px 13px",display:"flex",alignItems:"center",gap:9,borderBottom:`1px solid ${BORDER}`}}>
            <div style={{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,#C9A84C,#8a6820)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>🤖</div>
            <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#fff",fontFamily:SANS_BODY}}>Assessor FootStock IA</div><div style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:5,height:5,borderRadius:"50%",background:ACCENT}}/><span style={{fontSize:9,color:ACCENT,fontFamily:SANS_BODY}}>Monitorando {CLUBS.length} ativos · 6 camadas de mercado</span></div></div>
            <div style={{background:"linear-gradient(135deg,#f59e0b,#d97706)",borderRadius:9,padding:"2px 7px",fontSize:8,fontWeight:800,color:"#1a0800",fontFamily:SANS_BODY}}>⚡ PRO</div>
          </div>
          <div style={{padding:"9px 11px",display:"flex",flexDirection:"column",gap:7,maxHeight:190,overflowY:"auto"}}>
            {newsLog.length===0&&<div style={{textAlign:"center",padding:"14px 0",fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>🔍 Aguardando primeiro evento…</div>}
            {newsLog.map((n,i)=><div key={i} style={{background:n.sent>=0?"rgba(201,168,76,.05)":"rgba(244,63,94,.06)",border:`1px solid ${n.sent>=0?"rgba(201,168,76,.2)":"rgba(244,63,94,.2)"}`,borderRadius:11,padding:"8px 10px",animation:i===0?"fadeIn .3s ease":"none"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:7}}>
                <span style={{fontSize:13,flexShrink:0}}>{n.emoji}</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:10,color:"#e2e8f0",fontFamily:SANS_BODY,lineHeight:1.4,marginBottom:4}}>{n.headline}</div>
                  <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                    ...
                    <span style={{fontSize:8,color:MUTED,background:SURFACE,borderRadius:4,padding:"1px 4px",fontFamily:SANS_BODY}}>{n.cat}</span>
                  </div>
                </div>
              </div>
            </div>)}
          </div>
          <div style={{padding:"7px 11px",borderTop:`1px solid ${BORDER}`,display:"flex",gap:6}}><div style={{flex:1,background:SURFACE,borderRadius:16,padding:"7px 11px",fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>/radar, /report, /call…</div><div style={{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,#C9A84C,#8a6820)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,cursor:"pointer",color:BG,fontWeight:900}}>↑</div></div>
        </div>
        <div style={{fontSize:8,color:MUTED,fontWeight:700,marginBottom:9,letterSpacing:"1px",fontFamily:SANS_BODY}}>SENTIMENTO IA + OFI · AO VIVO<InfoTip term="Sentimento (Sentiment)"/></div>
        <div style={{...cd,overflow:"hidden",marginBottom:12}}>
          {CLUBS.map((c,i)=>{
        ...
        <div style={{fontSize:8,color:MUTED,fontWeight:700,marginBottom:9,letterSpacing:"1px",fontFamily:SANS_BODY}}>MOTOR DE PREÇOS · 6 CAMADAS</div>
        <div style={{...cd,overflow:"hidden",marginBottom:12}}>
          {[
            ...
            {ico:"📐",name:"Kyle's Lambda",desc:"Impacto de cada ordem no preço",col:"#D4B466"},
            ...
            <div style={{flex:1}}><div style={{fontSize:10,fontWeight:700,color:"#cbd5e1",fontFamily:SANS_BODY}}>{r.name}</div><div style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>{r.desc}</div></div>
            <div style={{width:8,height:8,borderRadius:"50%",background:r.col,boxShadow:`0 0 6px ${r.col}80`,flexShrink:0}}/>
          </div>)}
        </div>
        <div style={{fontSize:8,color:MUTED,fontWeight:700,marginBottom:9,letterSpacing:"1px",fontFamily:SANS_BODY}}>MATRIZ DE IMPACTO DE NOTÍCIAS</div>
        <div style={{...cd,overflow:"hidden",marginBottom:12}}>
          {[{cat:"Financeira Crítica",imp:"±15%",col:RED},{cat:"Esportiva Majoritária",imp:"±10%",col:"#fb923c"},{cat:"Mercado de Ativos",imp:"±7%",col:GOLD},{cat:"Integridade/Saúde",imp:"±5%",col:ACCENT},{cat:"Institucional",imp:"±3%",col:ACCENT2},{cat:"Esportiva Menor",imp:"±2%",col:MUTED}].map((r,i)=><div key={r.cat} style={{display:"flex",alignItems:"center",gap:9,padding:"8px 13px",borderBottom:i<5?`1px solid ${BORDER}`:"none"}}>
            ...
            <div style={{flex:1,fontSize:10,fontWeight:700,color:"#cbd5e1",fontFamily:SANS_BODY}}>{r.cat}</div>
      ...
      {tab==="Ligas"&&<>
        <div style={{background:"linear-gradient(135deg,#1a1000,#2a1800)",borderRadius:18,padding:"16px",marginBottom:12,border:"1px solid #f5a62330",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{fontSize:8,fontWeight:800,color:GOLD,letterSpacing:"1.5px",marginBottom:5,fontFamily:SANS_BODY}}>🏆 LIGA PREMIADA · PRO</div>
          <div style={{fontSize:16,fontWeight:900,color:"#fff",marginBottom:2,fontFamily:SANS_BODY}}>FootStock Masters</div>
          <div style={{fontSize:10,color:"#a08040",marginBottom:10,fontFamily:SANS_BODY}}>Semana 8 de 12 · 847 participantes</div>
          <div style={{display:"flex",gap:14}}>
            {[{l:"PRÊMIO 1º",v:"PS5+R$500",c:GOLD},{l:"SUA POSIÇÃO",v:"#23/847",c:"#fff"},{l:"ENCERRA",v:"4d 12h",c:"#fff"}].map(i=><div key={i.l}><div style={{fontSize:8,color:"#a08040",fontFamily:SANS_BODY}}>{i.l}</div><div style={{fontSize:11,fontWeight:800,color:i.c,marginTop:2,fontFamily:SANS_BODY}}>{i.v}</div></div>)}
          </div>
        </div>
        <div style={{fontSize:8,color:MUTED,fontWeight:700,marginBottom:9,letterSpacing:"1px",fontFamily:SANS_BODY}}>RANKING DA SEMANA</div>
        {[{rank:1,n:"TradingMaster_SP",p:"+FS$14.280",i:"🥇"},{rank:2,n:"FutebolQuant",p:"+FS$12.640",i:"🥈"},{rank:3,n:"RioTrader99",p:"+FS$10.920",i:"🥉"},{rank:null},{rank:23,n:"Você",p:"+FS$5.400",i:"👤",me:true}].map((r,idx)=>
          ...
          :<div key={idx} style={{...cd,padding:"9px 13px",display:"flex",alignItems:"center",gap:9,marginBottom:5,background:r.me?"rgba(201,168,76,.06)":CARD,borderColor:r.me?"rgba(201,168,76,.3)":BORDER}}><div style={{fontSize:16,flexShrink:0}}>{r.i}</div><div style={{fontSize:10,fontWeight:700,color:MUTED,width:18,fontFamily:SANS_BODY}}>#{r.rank}</div><div style={{flex:1,fontSize:11,fontWeight:r.me?800:600,color:r.me?ACCENT:"#cbd5e1",fontFamily:SANS_BODY}}>{r.n}</div><div style={{fontSize:11,fontWeight:800,color:ACCENT,fontFamily:MONO}}>{r.p}</div></div>
        )}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginTop:14,marginBottom:14}}>
          {[{n:"Liga Amigos",d:"Até 20 jogadores",i:"👥"},{n:"Liga Pública",d:"Aberta a todos",i:"🌐"},{n:"Liga PRO",d:"Com prêmios físicos",i:"⚡",pro:true},{n:"Criar Liga",d:"Configure as regras",i:"➕"}].map(l=><button key={l.n} onClick={()=>l.n==="Criar Liga"&&setShowCriarLiga(true)} style={{...cd,padding:"13px",cursor:"pointer",textAlign:"left",borderColor:l.pro?"rgba(245,166,35,.4)":l.n==="Criar Liga"?`${ACCENT}40`:BORDER}}>
            ...
            <div style={{fontSize:11,fontWeight:800,color:"#e2e8f0",marginBottom:2,fontFamily:SANS_BODY}}>{l.n}</div>
            <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY}}>{l.d}</div>
            {l.pro&&<div style={{fontSize:8,color:GOLD,marginTop:6,fontWeight:800,fontFamily:SANS_BODY}}>⚡ EXCLUSIVO PRO</div>}
          </button>)}
        </div>
      </>}
      ...
      {tab==="Msgs"&&(()=>{
        const MSGS=[
          ...
          {id:4, type:"marketing",icon:"🎯", color:"#D4B466",
        ...
        return <>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div>
              <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Mensagens</div>
              <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>
            ...
            {unreadCount>0&&<button onClick={()=>setReadMsgs(new Set(MSGS.map(m=>m.id)))}
              style={{background:"none",border:`1px solid ${BORDER}`,borderRadius:8,padding:"4px 10px",
                      color:MUTED,fontSize:9,fontFamily:SANS_BODY,cursor:"pointer"}}>
            ...
            {types.map(t=>{
              ...
              return <button key={t} onClick={()=>setMsgFilter(t)}
                style={{background:active?ACCENT2+"22":SURFACE,
                        ...
                        fontFamily:SANS_BODY,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>
                {t}
              </button>;
            })}
          </div>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:3}}>
                    <span style={{fontSize:7,fontWeight:800,color:msg.color,
                                  letterSpacing:"0.8px",fontFamily:SANS_BODY}}>
                      {typeLabels[msg.type]}
                    </span>
                    <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>{msg.time}</span>
                  </div>

                  {/* Título */}
                  <div style={{fontSize:11,fontWeight:isUnread?900:700,
                               color:isUnread?"#fff":"#cbd5e1",
                               fontFamily:SANS_BODY,marginBottom:4,lineHeight:1.3}}>
                    {msg.title}
                  </div>

                  {/* Corpo */}
                  <div style={{fontSize:9,color:isUnread?MUTED:"#7a8ba8",
                               fontFamily:SANS_BODY,lineHeight:1.5}}>
                    {msg.body}
                  </div>
                </div>
              </div>
          ...
          {filtered.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:MUTED,fontSize:11,fontFamily:SANS_BODY}}>
    ...
    <div style={{flexShrink:0,background:"rgba(7,9,15,.97)",borderTop:`1px solid ${BORDER}`,padding:"7px 8px 4px",display:"flex",justifyContent:"space-around",zIndex:10}}>
      {[{t:"Dashboard",i:"🏠",l:"Home"},{t:"Mercado",i:"📊",l:"Mercado"},{t:"Carteira",i:"💼",l:"Carteira"},{t:"Forum",i:"💬",l:"Comunidade"},{t:"Extrato",i:"📋",l:"Extrato"},{t:"Msgs",i:"✉️",l:"Msgs"}].map(n=>{
        ...
        return <button key={n.t} onClick={()=>{if(isExtrato){setShowForum(false);setShowExtrato(true);}else if(isForum){setShowExtrato(false);setShowForum(true);}else{setShowExtrato(false);setShowForum(false);setTab(n.t);setClub(null);}}} style={{background:"none",border:"none",cursor:"pointer",padding:"3px 4px",display:"flex",flexDirection:"column",alignItems:"center",gap:2,position:"relative"}}>
          ...
          {unread>0&&<div style={{position:"absolute",top:0,right:2,background:RED,borderRadius:10,minWidth:14,height:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,padding:"0 3px"}}>{unread}</div>}
          <div style={{fontSize:7,fontWeight:700,color:active?ACCENT:MUTED,letterSpacing:"0.3px",fontFamily:SANS_BODY}}>{n.l}</div>
    ...
    {orderFeedback&&(()=>{
      ...
      const cfg=isOk
        ? isScheduled
          ? {icon:"📅",accent:"#D4B466",bg:"rgba(201,168,76,.08)",border:"rgba(201,168,76,.3)",btnBg:"linear-gradient(135deg,#C9A84C,#8a6820)",btnShadow:"rgba(201,168,76,.3)"}
          : isBuy
            ? {icon:"✅",accent:ACCENT,bg:"rgba(201,168,76,.08)",border:"rgba(201,168,76,.3)",btnBg:`linear-gradient(135deg,${ACCENT},#5b52e8)`,btnShadow:"rgba(201,168,76,.3)"}
      ...
      return <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.82)",zIndex:120,display:"flex",alignItems:"flex-end",backdropFilter:"blur(5px)"}}>
        <div style={{background:CARD,borderRadius:"24px 24px 0 0",padding:"26px 20px 30px",width:"100%",boxSizing:"border-box",boxShadow:"0 -24px 60px rgba(0,0,0,.6)",animation:"slideUp .25s ease"}}>
          ...
          {/* Título */}
          <div style={{textAlign:"center",fontSize:18,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,letterSpacing:"-0.5px",marginBottom:8}}>
            {title}
          </div>
          ...
          {details.length>0&&<div style={{background:cfg.bg,border:`1px solid ${cfg.border}`,borderRadius:14,padding:"12px 16px",marginBottom:22}}>
            {details.map((line,i)=><div key={i} style={{fontSize:12,color:"rgba(255,255,255,.75)",fontFamily:SANS_BODY,lineHeight:1.8,textAlign:"center"}}>{line}</div>)}
          </div>}

          {/* Botão de confirmação */}
          <button onClick={()=>setOrderFeedback(null)}
            style={{width:"100%",padding:"16px",borderRadius:18,border:"none",cursor:"pointer",background:cfg.btnBg,color:"#fff",fontSize:15,fontWeight:800,fontFamily:SANS_BODY,letterSpacing:"0.2px",boxShadow:`0 8px 28px ${cfg.btnShadow}`}}>
    ...
    {scheduleModal&&(()=>{
      ...
      const accentCol=isBuy?ACCENT:RED;
      const borderCol=isBuy?"rgba(201,168,76,.3)":"rgba(244,63,94,.3)";
      ...
      return <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.8)",zIndex:100,display:"flex",alignItems:"flex-end",backdropFilter:"blur(4px)"}}
        ...
        <div onClick={e=>e.stopPropagation()} style={{background:"#111825",borderRadius:"24px 24px 0 0",padding:"22px 18px 30px",width:"100%",boxSizing:"border-box",boxShadow:"0 -30px 80px rgba(0,0,0,.7),0 0 0 1px rgba(255,255,255,.05)",animation:"slideUp .25s ease"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <div style={{width:42,height:42,borderRadius:13,background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>📅</div>
            <div>
              <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>Agendar {isBuy?"Compra":"Venda"}</div>
              <div style={{fontSize:10,color:"#D4B466",fontFamily:SANS_BODY,fontWeight:600}}>Execução na abertura · 11h00 (Brasília)</div>
            </div>
          </div>

          {/* Clube */}
          <div style={{background:isBuy?"rgba(201,168,76,.06)":"rgba(244,63,94,.07)",border:`1px solid ${borderCol}`,borderRadius:14,padding:"12px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{cl.name}</div>
              <div style={{fontSize:9,color:MUTED,fontFamily:MONO}}>{cl.ticker} · {scheduleModal.qty} cota{scheduleModal.qty!==1?"s":""}</div>
            </div>
            <div style={{background:isBuy?"rgba(201,168,76,.12)":"rgba(244,63,94,.15)",border:`1px solid ${borderCol}`,borderRadius:8,padding:"3px 10px",fontSize:10,fontWeight:900,color:accentCol,fontFamily:SANS_BODY}}>
              {isBuy?"COMPRA":"VENDA"}
            </div>
          </div>
          ...
          <div style={{background:"rgba(245,166,35,.08)",border:"1px solid rgba(245,166,35,.22)",borderRadius:11,padding:"8px 12px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <span style={{fontSize:9,color:"#f5a623",fontFamily:SANS_BODY,fontWeight:700}}>💰 PREÇO DE FECHAMENTO ATUAL</span>
          ...
          <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:8}}>TIPO DE PREÇO</div>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {[{id:"close",label:"Preço de fechamento",sub:"Usa o último preço registrado"},{id:"fixed",label:"Preço limite",sub:"Define um valor máximo/mínimo"}].map(opt=>(
              <div key={opt.id} onClick={()=>setPriceType(opt.id)} style={{flex:1,padding:"11px 10px",borderRadius:12,border:`1.5px solid ${priceType===opt.id?accentCol:"rgba(255,255,255,.1)"}`,background:priceType===opt.id?(isBuy?"rgba(201,168,76,.06)":"rgba(244,63,94,.08)"):"rgba(255,255,255,.03)",cursor:"pointer",transition:"all .15s"}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                  ...
                  <span style={{fontSize:10,fontWeight:800,color:priceType===opt.id?"#fff":"rgba(255,255,255,.5)",fontFamily:SANS_BODY}}>{opt.label}</span>
                </div>
                <div style={{fontSize:8,color:priceType===opt.id?"rgba(255,255,255,.5)":"rgba(255,255,255,.25)",fontFamily:SANS_BODY,paddingLeft:20}}>{opt.sub}</div>
              </div>
            ))}
          </div>
          ...
          {priceType==="fixed"&&<div style={{marginBottom:14}}>
            <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:6}}>
            ...
            {fixedInput&&!isNaN(fixedPrice)&&fixedPrice>0&&(()=>{
              ...
              return <div style={{fontSize:9,color:isHigh?"#fb923c":"rgba(255,255,255,.4)",fontFamily:SANS_BODY,marginTop:5,display:"flex",gap:4,alignItems:"center"}}>
          ...
          {totalEstim&&(()=>{
            ...
            return <div style={{background:"rgba(255,255,255,.04)",border:`1px solid ${insuf?"rgba(244,63,94,.3)":"rgba(255,255,255,.08)"}`,borderRadius:13,padding:"12px 14px",marginBottom:14}}>
              {[
                ...
                {l:"Taxa operacional ℹ",v:`FS$${fee.toFixed(2)}`,c:"#f5a623",tag:true},
                ...
                <span style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,display:"flex",alignItems:"center",gap:5}}>
          ...
          {/* Aviso */}
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:18,background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.22)",borderRadius:10,padding:"9px 12px"}}>
            <span style={{fontSize:13,flexShrink:0}}>⚡</span>
            <span style={{fontSize:9,color:"rgba(200,195,255,.8)",fontFamily:SANS_BODY,lineHeight:1.7}}>A ordem será executada automaticamente na <strong style={{color:"#fff"}}>abertura do mercado (11h00)</strong>. Se não houver cotas disponíveis ou o preço limite não for atingido, a ordem será <strong style={{color:"#f43f5e"}}>cancelada automaticamente</strong> e você será notificado.</span>
          </div>

          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>setScheduleModal(null)} style={{flex:1,padding:"14px",borderRadius:14,border:`1px solid ${BORDER}`,background:SURFACE,color:MUTED,fontSize:13,fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer"}}>
              Cancelar
            </button>
            <button onClick={confirmSchedule} disabled={!canConfirm}
              style={{flex:2,padding:"14px",borderRadius:14,border:"none",cursor:canConfirm?"pointer":"default",background:canConfirm?"linear-gradient(135deg,#C9A84C,#8a6820)":"rgba(201,168,76,.08)",color:canConfirm?"#fff":"rgba(255,255,255,.25)",fontSize:14,fontWeight:700,fontFamily:SANS_BODY,boxShadow:canConfirm?"0 6px 24px rgba(201,168,76,.3)":"none",transition:"all .2s"}}>
    ...
    {upgradeModal&&(()=>{
      ...
      const planData=isPlanCraque
        ?{name:"Craque",price:"R$19,90/mês",color:ACCENT,icon:"⭐",gradient:"linear-gradient(135deg,#C9A84C,#8a6820)",features:["Ordem Precificada 🎯","FS$5.000 adicional","5 ordens/dia","IA básica"],cta:"Assinar Craque"}
        :{name:"Lenda",price:"R$39,90/mês",color:GOLD,icon:"👑",gradient:"linear-gradient(135deg,#C9A84C,#a07828)",features:["Short Selling 📉","Stop Loss / Take Profit 🛡","Ordem Precificada 🎯","FS$25.000 adicional","Ilimitado","IA VIP"],cta:"Assinar Lenda"};
      ...
      return <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.82)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(6px)",padding:"0 20px"}}
        ...
        <div onClick={e=>e.stopPropagation()} style={{background:"linear-gradient(160deg,#130f08,#080808)",border:`1.5px solid ${planData.color}55`,borderRadius:24,padding:"26px 22px",width:"100%",boxSizing:"border-box",boxShadow:`0 0 60px ${planData.color}30, 0 30px 80px rgba(0,0,0,.6)`,animation:"slideUp .22s ease",position:"relative",overflow:"hidden"}}>
          {/* Glow bg */}
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
            ...
            <div>
              <div style={{fontSize:9,color:"rgba(255,255,255,.6)",fontWeight:700,letterSpacing:"1.5px",fontFamily:SANS_BODY,marginBottom:3}}>UPGRADE NECESSÁRIO</div>
              <div style={{fontSize:18,fontWeight:800,color:"#fff",fontFamily:SANS_BODY,letterSpacing:"-0.5px",lineHeight:1.1}}>Plano <span style={{background:planData.gradient,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{planData.name}</span></div>
              <div style={{fontSize:11,fontWeight:700,color:planData.color,fontFamily:SANS_BODY,marginTop:2}}>{planData.price}</div>
            </div>
          </div>
          ...
          <div style={{background:`${planData.color}12`,border:`1px solid ${planData.color}35`,borderRadius:14,padding:"11px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <div style={{fontSize:9,color:"rgba(255,255,255,.6)",fontFamily:SANS_BODY,fontWeight:700,letterSpacing:"0.8px",marginBottom:2}}>VOCÊ TENTOU USAR</div>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{upgradeModal.feature}</div>
              <div style={{fontSize:9,color:planData.color,fontFamily:SANS_BODY,marginTop:2}}>Disponível a partir do Plano {planData.name}</div>
            </div>
          </div>

          {/* Features list */}
          <div style={{marginBottom:20}}>
            <div style={{fontSize:8,color:"rgba(255,255,255,.5)",fontWeight:700,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:8}}>INCLUÍDO NO PLANO {planData.name.toUpperCase()}</div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {planData.features.map((f,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontSize:11,color:"rgba(255,255,255,.8)",fontFamily:SANS_BODY}}>{f}</span>
              </div>)}
            </div>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:18}}>
            <div style={{padding:"4px 10px",borderRadius:20,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",fontSize:9,color:"rgba(255,255,255,.6)",fontFamily:SANS_BODY,fontWeight:700}}>{currentPlan}</div>
            <span style={{fontSize:12,color:"rgba(255,255,255,.45)"}}>→</span>
            <div style={{padding:"4px 10px",borderRadius:20,background:planData.gradient,fontSize:9,color:isPlanCraque?BG:"#1a0800",fontFamily:SANS_BODY,fontWeight:900}}>{planData.name} {planData.icon}</div>
          </div>

          {/* CTA */}
          <button onClick={()=>{setUpgradeModal(null);onUpgrade&&onUpgrade(isPlanCraque?"Craque":"Lenda",userPlan,(plan,bil)=>applyUpgrade(plan==="craque"?"Craque":"Lenda",bil||"monthly"));}} style={{width:"100%",padding:"15px",borderRadius:16,border:"none",cursor:"pointer",background:planData.gradient,color:isPlanCraque?BG:"#1a0800",fontSize:14,fontWeight:800,fontFamily:SANS_BODY,boxShadow:`0 8px 28px ${planData.color}45`,marginBottom:10}}>
            {planData.cta} →
          </button>
          <button onClick={()=>setUpgradeModal(null)} style={{width:"100%",padding:"10px",borderRadius:12,border:"1px solid rgba(255,255,255,.1)",background:"transparent",color:"rgba(255,255,255,.55)",fontSize:11,fontWeight:700,fontFamily:SANS_BODY,cursor:"pointer"}}>
            Agora não
          </button>
        </div>
      </div>;
    ...
    {orderConfirm&&(()=>{
      ...
      const accentCol=isBuy?ACCENT:RED;
      const gradBg=isBuy?"linear-gradient(135deg,rgba(201,168,76,.08),rgba(201,168,76,.04))":"linear-gradient(135deg,rgba(244,63,94,.12),rgba(244,63,94,.04))";
      const borderCol=isBuy?"rgba(201,168,76,.3)":"rgba(244,63,94,.3)";
      ...
      return <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.8)",zIndex:100,display:"flex",alignItems:"flex-end",backdropFilter:"blur(10px)"}}
        ...
        <div onClick={e=>e.stopPropagation()} style={{background:"#111825",borderRadius:"24px 24px 0 0",padding:"22px 18px 28px",width:"100%",boxSizing:"border-box",boxShadow:"0 -30px 80px rgba(0,0,0,.7),0 0 0 1px rgba(255,255,255,.05)",animation:"slideUp .25s ease",maxHeight:"90vh",overflowY:"auto"}}>
          <div style={{width:36,height:4,background:BORDER,borderRadius:2,margin:"0 auto 18px",opacity:.6}}/>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <div style={{width:40,height:40,borderRadius:12,background:isBuy?"rgba(201,168,76,.12)":"rgba(244,63,94,.15)",border:`1px solid ${borderCol}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>
              {isBuy?"▲":"▼"}
            </div>
            <div>
              <div style={{fontSize:16,fontWeight:700,color:"#fff",fontFamily:SANS_BODY}}>{orderType==="short"?"Abrir Short":orderType==="short_close"?"Fechar Short":isBuy?"Confirmar Compra":"Confirmar Venda"}</div>
              <div style={{fontSize:11,color:MUTED,fontFamily:SANS_BODY}}>Revise os detalhes antes de confirmar</div>
            </div>
          </div>

          {/* ── TIPO DE ORDEM ── */}
          <div style={{marginBottom:14}}>
            <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:8}}>TIPO DE ORDEM</div>
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {[
                ...
                return <div key={t.id}
                  ...
                  style={{padding:"10px 12px",borderRadius:12,border:`1.5px solid ${locked?"rgba(255,255,255,.08)":disabled?"rgba(255,255,255,.06)":active?cardColor:isShortCard?"rgba(244,63,94,.2)":t.id==="oco"?"rgba(245,166,35,.2)":BORDER}`,background:locked?"rgba(255,255,255,.02)":disabled?"rgba(255,255,255,.02)":active?(isShortCard?"rgba(244,63,94,.08)":t.id==="oco"?"rgba(245,166,35,.08)":isBuy?"rgba(201,168,76,.08)":"rgba(244,63,94,.08)"):(isShortCard?"rgba(244,63,94,.03)":t.id==="oco"?"rgba(245,166,35,.03)":"rgba(255,255,255,.02)"),cursor:locked?"pointer":disabled?"not-allowed":"pointer",transition:"all .15s",opacity:locked?.55:disabled?.45:1,position:"relative"}}>
                  <div style={{display:"flex",alignItems:"center",gap:7}}>
                    <span style={{fontSize:15,flexShrink:0,filter:locked?"grayscale(1) opacity(0.5)":"none"}}>{t.ico}</span>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                        <span style={{fontSize:11,fontWeight:900,color:locked?"rgba(255,255,255,.35)":disabled?"rgba(255,255,255,.3)":active?"#fff":"rgba(255,255,255,.6)",fontFamily:SANS_BODY}}>{t.title}</span>
                        {t.craque&&<span style={{fontSize:7,background:"rgba(201,168,76,.08)",color:ACCENT,borderRadius:4,padding:"1px 5px",fontWeight:800,fontFamily:SANS_BODY}}>⭐ CRAQUE</span>}
                        {t.lenda&&<span style={{fontSize:7,background:isLenda?"rgba(245,166,35,.18)":"rgba(245,166,35,.28)",color:GOLD,borderRadius:4,padding:"1px 5px",fontWeight:800,fontFamily:SANS_BODY}}>👑 LENDA</span>}
                        {t.sellOnly&&isBuy&&<span style={{fontSize:7,color:"rgba(255,255,255,.45)",fontFamily:SANS_BODY}}>· apenas na venda</span>}
                      </div>
                      <div style={{fontSize:8,color:locked?"rgba(255,255,255,.25)":MUTED,fontFamily:SANS_BODY,marginTop:2,lineHeight:1.4}}>{locked?`🔒 Exclusivo Plano ${targetUpgradePlan} — toque para fazer upgrade`:t.sub}</div>
                    </div>
          ...
          {orderType==="limit"&&<div style={{marginBottom:14}}>
            <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:8}}>PREÇO LIMITE</div>
            <div style={{background:SURFACE,border:`1.5px solid ${limitValid?(limitWarn?GOLD:accentCol):BORDER}`,borderRadius:12,padding:"10px 13px",display:"flex",alignItems:"center",gap:10,transition:"border-color .15s"}}>
              <span style={{fontSize:12,color:MUTED,fontFamily:SANS_BODY}}>FS$</span>
              <input
              ...
              {limitValid&&<div style={{textAlign:"right",flexShrink:0}}>
                ...
                <div style={{fontSize:7,color:MUTED,fontFamily:SANS_BODY}}>vs mercado</div>
              </div>}
            </div>
            ...
            {limitWarn&&<div style={{marginTop:6,display:"flex",gap:6,background:"rgba(245,166,35,.07)",border:"1px solid rgba(245,166,35,.2)",borderRadius:8,padding:"6px 9px",alignItems:"center"}}>
              ...
              <span style={{fontSize:8,color:"#d4a017",fontFamily:SANS_BODY,lineHeight:1.5}}>Preço {isBuy?"acima":"abaixo"} de 5% do mercado. A ordem pode demorar para executar.</span>
            </div>}
            <div style={{marginTop:6,fontSize:8,color:MUTED,fontFamily:SANS_BODY,lineHeight:1.5}}>
          ...
          {orderType==="oco"&&(()=>{
            ...
            return <div style={{marginBottom:14}}>
              {/* Modo de input */}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY}}>DEFINIR GATILHOS</div>
                <div style={{display:"flex",background:SURFACE,borderRadius:8,border:`1px solid ${BORDER}`,overflow:"hidden"}}>
                  {[{id:"pct",l:"%"},{id:"price",l:"FS$"}].map(m=>(
                    <button key={m.id} onClick={()=>{setOcoMode(m.id);setOcoSL("");setOcoTP("");}} style={{padding:"3px 10px",border:"none",background:ocoMode===m.id?"rgba(245,166,35,.2)":"transparent",color:ocoMode===m.id?GOLD:MUTED,fontSize:9,fontWeight:800,cursor:"pointer",fontFamily:SANS_BODY}}>{m.l}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                  <span style={{fontSize:10}}>🛑</span>
                  <span style={{fontSize:9,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Stop Loss</span>
                  <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>— vende se o preço cair até</span>
                ...
                <div style={{background:SURFACE,border:`1.5px solid ${slValid?RED:BORDER}`,borderRadius:10,padding:"8px 12px",display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:10,color:MUTED,fontFamily:SANS_BODY,flexShrink:0}}>{ocoMode==="pct"?"−%":"FS$"}</span>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                  <span style={{fontSize:10}}>🎯</span>
                  <span style={{fontSize:9,fontWeight:800,color:"#fff",fontFamily:SANS_BODY}}>Take Profit</span>
                  <span style={{fontSize:8,color:MUTED,fontFamily:SANS_BODY}}>— vende se o preço subir até</span>
                ...
                <div style={{display:"flex",gap:5,marginTop:5}}>
                  {[5,10,20].map(p=>(
                    <button key={p} onClick={()=>setOcoTP(ocoMode==="pct"?String(p):pctToPrice(p).toFixed(2))} style={{flex:1,padding:"3px",borderRadius:6,border:`1px solid rgba(201,168,76,.2)`,background:"rgba(201,168,76,.05)",color:ACCENT,fontSize:8,fontWeight:700,cursor:"pointer",fontFamily:MONO}}>+{p}%<br/><span style={{color:"rgba(255,255,255,.6)",fontSize:7}}>FS${pctToPrice(p).toFixed(2)}</span></button>
                  ))}
                </div>
              </div>
              ...
              <div style={{marginTop:8,display:"flex",gap:6,background:"rgba(245,166,35,.06)",border:"1px solid rgba(245,166,35,.18)",borderRadius:8,padding:"6px 9px",alignItems:"flex-start"}}>
                ...
                <span style={{fontSize:8,color:"#d4a017",fontFamily:SANS_BODY,lineHeight:1.5}}>OCO — One Cancels Other: ao disparar um gatilho, o outro é automaticamente cancelado. Taxa cobrada somente na execução.</span>
              </div>
            </div>;
          })()}
          ...
          {orderType==="short"&&(()=>{
            ...
            return <div style={{marginBottom:14}}>
              {existingShort&&<div style={{background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.3)",borderRadius:12,padding:"10px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:9}}>
                <span style={{fontSize:14}}>📉</span>
                <div>
                  <div style={{fontSize:9,fontWeight:800,color:RED,fontFamily:SANS_BODY}}>POSIÇÃO SHORT ABERTA</div>
                  ...
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS_BODY,marginTop:1}}>P&L atual: <span style={{color:(mktPrice<existingShort.entryPrice)?ACCENT:RED,fontWeight:700}}>{((existingShort.entryPrice-mktPrice)*existingShort.qty>=0?"+":"")}FS${((existingShort.entryPrice-mktPrice)*existingShort.qty).toFixed(2)}</span></div>
                </div>
              </div>}
              <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"1px",fontFamily:SANS_BODY,marginBottom:8}}>QUANTIDADE A VENDER A DESCOBERTO<InfoTip term="Short Selling (Venda a Descoberto)"/></div>
              ...
              <div style={{marginTop:12,background:"rgba(244,63,94,.06)",border:"1px solid rgba(244,63,94,.2)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:8,color:MUTED,fontWeight:800,letterSpacing:"0.8px",fontFamily:SANS_BODY,marginBottom:8}}>RESUMO DO SHORT</div>
                {[
                  ...
                  <span style={{fontSize:9,color:row.warn?RED:MUTED,fontFamily:SANS_BODY}}>{row.l}</span>
              ...
              {!canShort&&<div style={{marginTop:8,display:"flex",gap:6,background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.25)",borderRadius:8,padding:"7px 10px",alignItems:"center"}}>
                <span style={{fontSize:12}}>⚠️</span>
                <span style={{fontSize:8,color:"#f87171",fontFamily:SANS_BODY,lineHeight:1.5}}>Saldo insuficiente para a margem requerida de FS${shortMargin.toFixed(2)}.</span>
              </div>}
              <div style={{marginTop:8,display:"flex",gap:6,background:"rgba(244,63,94,.05)",border:"1px solid rgba(244,63,94,.15)",borderRadius:8,padding:"7px 10px",alignItems:"flex-start"}}>
                <span style={{fontSize:11,flexShrink:0}}>💡</span>
                <span style={{fontSize:8,color:"rgba(255,150,130,.8)",fontFamily:SANS_BODY,lineHeight:1.6}}>Short Selling: você aluga cotas, vende agora e recompra mais barato para lucrar com a queda. Risco ilimitado se o preço subir. Margem bloqueada até fechar posição.</span>
              </div>
            </div>;
          })()}
          ...
          <div style={{background:gradBg,border:`1px solid ${borderCol}`,borderRadius:16,padding:"14px 16px",marginBottom:16}}>
            ...
            {[
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
                <span style={{fontSize:row.bold?11:10,color:row.fee?"rgba(255,255,255,.6)":row.feeNote?"rgba(245,166,35,.7)":MUTED,fontFamily:SANS_BODY,display:"flex",alignItems:"center",gap:5}}>
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:18,background:"rgba(245,166,35,.06)",border:`1px solid ${orderType==="short"?"rgba(244,63,94,.3)":orderType==="oco"?"rgba(245,166,35,.3)":orderType==="limit"?"rgba(201,168,76,.2)":"rgba(245,166,35,.2)"}`,borderRadius:10,padding:"9px 12px"}}>
            <span style={{fontSize:14,flexShrink:0}}>{orderType==="short"?"📉":orderType==="oco"?"🛡":orderType==="limit"?"🎯":"⚠️"}</span>
            <span style={{fontSize:9,color:orderType==="short"?"#f87171":orderType==="oco"?GOLD:orderType==="limit"?"#D4B466":"#d4a017",fontFamily:SANS_BODY,lineHeight:1.6}}>
              {orderType==="short"
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>setOrderConfirm(null)}
              style={{flex:1,padding:"14px",borderRadius:14,border:`1px solid ${BORDER}`,background:SURFACE,color:MUTED,fontSize:13,fontWeight:800,fontFamily:SANS_BODY,cursor:"pointer"}}>
              Cancelar
            </button>
            <button onClick={confirm} disabled={!canConfirm}
              style={{flex:2,padding:"14px",borderRadius:14,border:"none",cursor:canConfirm?"pointer":"not-allowed",background:canConfirm?(orderType==="short"?`linear-gradient(135deg,${RED},#cc1133)`:orderType==="oco"?`linear-gradient(135deg,${GOLD},#d97706)`:isBuy?"linear-gradient(135deg,#C9A84C,#8a6820)":`linear-gradient(135deg,${RED},#bb1133)`):"rgba(255,255,255,.06)",color:canConfirm?"#fff":"rgba(255,255,255,.25)",fontSize:13,fontWeight:800,fontFamily:SANS_BODY,boxShadow:canConfirm?`0 6px 24px ${orderType==="short"?"rgba(244,63,94,.35)":orderType==="oco"?"rgba(245,166,35,.3)":isBuy?"rgba(201,168,76,.3)":"rgba(244,63,94,.3)"}`:"none",transition:"all .2s"}}>
