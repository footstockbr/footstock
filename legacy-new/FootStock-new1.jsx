...
/* ── TOKENS — Black & Gold Premium Palette (FootStock Admin) ── */
const BG="#080808",SURFACE="#0f0e0b",CARD="#141210",BORDER="#2a2010";
const ACCENT="#C9A84C",ACCENT2="#D4B466",RED="#e05555",GOLD="#C9A84C";
const GREEN="#22c55e",ORANGE="#f97316";
const TEXT="#F0EAD6",MUTED="#7a7060";
const GRAD_ACCENT="linear-gradient(135deg,#C9A84C,#8a6820)";
const SANS="'Inter','Helvetica Neue',sans-serif",MONO="'JetBrains Mono','Fira Mono',monospace";
const SANS_DISPLAY="'Playfair Display','Georgia',serif";
...
const SESSIONS=[
  ...
  {id:"closing", label:"Call de Fechamento", short:"LEILÃO", color:"#D4B466", bg:"rgba(212,180,102,.07)", border:"rgba(212,180,102,.25)", start:24*60+45, end:25*60+0},
  {id:"after",   label:"After-Market",       short:"AFTER",  color:"#C9A84C", bg:"rgba(201,168,76,.07)", border:"rgba(201,168,76,.25)",  start:25*60+0,  end:25*60+30},
];
...
function useMockMarket(){
  ...
  return{motorState,paused,setPaused,forceCircuitBreaker,clearCircuitBreaker,setFV};
}
...
/* ── Componentes base ── */
function Badge({children,color=MUTED,bg="rgba(201,168,76,.07)"}){
...
function StatCard({label,value,sub,color="#fff",icon}){
  return <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
      <span style={{fontSize:9,color:"rgba(201,168,76,.6)",fontWeight:700,letterSpacing:"1px",fontFamily:SANS}}>{label}</span>
...
function SectionHeader({title,sub,action}){
  return <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
    <div>
      <div style={{fontSize:16,fontWeight:800,color:"#fff",fontFamily:SANS_DISPLAY}}>{title}</div>
...
function Input({label,value,onChange,type="text",placeholder="",min,max,step}){
  return <div style={{marginBottom:12}}>
    ...
    <input type={type} value={value} onChange={e=>onChange(e.target.value)}
      ...
      style={{width:"100%",background:"rgba(201,168,76,.04)",border:`1px solid rgba(201,168,76,.18)`,borderRadius:10,padding:"9px 13px",
        color:TEXT,fontSize:12,fontFamily:MONO,outline:"none",boxSizing:"border-box",caretColor:ACCENT}}/>
  </div>;
}
...
══════════════════════════════════════════════════════════════════ */
function MotorModule({motorState,paused,setPaused,forceCircuitBreaker,clearCircuitBreaker,setFV,users}){
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <div>
            ...
            <select value={clubForm.div||"A"} onChange={e=>setClubForm(p=>({...p,div:e.target.value}))}
              style={{width:"100%",background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.18)",borderRadius:10,padding:"9px 13px",color:TEXT,fontSize:12,fontFamily:MONO,outline:"none"}}>
...
function UsersModule({currentRole="admin"}){
  ...
  const updateUser=(id,patch)=>setUsers(prev=>prev.map(u=>(u.id===id&&u.id!==SUPER_ADMIN.id)?{...u,...patch}:u));

  const executeAction=()=>{
    if(!confirmAction) return;
    const {type,userId}=confirmAction;
    if(!canAction(currentRole,type)) return; // role-based double-guard
    if(userId===SUPER_ADMIN.id) return;       // Super Admin é imutável — nunca pode ser alterado
  ...
  const roleColor={superadmin:"#ffd700",admin:RED,monitor:ORANGE,editor:ACCENT,moderador:ACCENT2,user:MUTED};
    ...
    {sel?<>
      ...
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:"18px",marginBottom:12}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          {!canAction(currentRole,"promote_monitor")
            ?<div style={{background:"rgba(249,115,22,.07)",border:"1px solid rgba(249,115,22,.2)",borderRadius:10,padding:"8px 12px",fontSize:10,color:ORANGE,fontFamily:SANS}}>
               🔒 Alteração de roles restrita a Administradores e superiores
             </div>
            :<div style={{display:"flex",flexDirection:"column",gap:8}}>
              {/* Promover a Admin — exclusivo do Super Admin */}
              {canAction(currentRole,"promote_admin")&&<>
                <div style={{fontSize:9,color:"rgba(255,215,0,.7)",fontWeight:700,fontFamily:SANS,letterSpacing:"0.5px"}}>⚡ EXCLUSIVO SUPER ADMIN</div>
                <div style={{display:"flex",gap:6}}>
                  <Btn onClick={()=>setConfirmAction({type:"promote_admin",userId:sel.id})} color="#f43f5e" variant="outline" small>👑 Promover a Admin</Btn>
                </div>
    ...
    {confirmAction&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:20,padding:"24px",maxWidth:360,width:"100%"}}>
        ...
        <div style={{fontSize:12,color:MUTED,fontFamily:SANS,marginBottom:20}}>
          ...
          {confirmAction.type==="promote_admin"     &&"⚡ Promover a Admin? Esta ação só pode ser desfeita pelo Super Admin."}
...
function FinanceiroModule(){
  ...
  const BLANK_GW={nome:"",icon:"💳",cor:"#C9A84C",ativo:true,pct:0,
  ...
  const gwFormFields=(
    <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"16px",marginBottom:12}}>
      ...
      )(GW_WEBHOOK_META[gwForm.id]||GW_WEBHOOK_DEFAULT)}
          ...
          return <div key={g.id} style={{background:CARD,border:`1.5px solid ${g.ativo?g.cor+"55":BORDER}`,borderRadius:14,padding:"14px",marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
              ...
              <div style={{flex:1}}>
                ...
                )(GW_WEBHOOK_META[g.id]||GW_WEBHOOK_DEFAULT)}
              </div>
            </div>
...
const LIGAS_MOCK_INIT=[
  {id:"l001",nome:"Copa FootStock Pro",patrocinador:"Mercado Pago",premio:"FS$10.000",inicio:"2026-04-01",fim:"2026-04-30",maxJogadores:50,inscritos:34,status:"ativa",    planoMinimo:"Craque",cor:"#C9A84C",descricao:"Liga mensal oficial com premiação em saldo FS."},
  ...
  {id:"l003",nome:"Liga Lenda Exclusiva",patrocinador:"PagSeguro",  premio:"FS$25.000",inicio:"2026-05-01",fim:"2026-05-31",maxJogadores:20,inscritos:7, status:"agendada", planoMinimo:"Lenda",  cor:"#C9A84C",descricao:"Liga exclusiva para usuários Lenda com grande premiação."},
];
...
const BLANK_BANNER={titulo:"",patrocinador:"",posicao:"home_top",ativo:true,cor:"#C9A84C",cta:"Saiba mais",url:"",inicio:"",fim:""};
const BLANK_LIGA  ={nome:"",patrocinador:"",premio:"",inicio:"",fim:"",maxJogadores:50,planoMinimo:"Craque",cor:"#C9A84C",descricao:""};

function ColorPicker({value,onChange}){
  const presets=["#C9A84C","#D4B466","#22c55e","#C9A84C","#f43f5e","#f97316","#D4B466","#00B1EA","#CC0000","#003087"];
  return <div>
...
function PhoneShell({children,time}){
  return <div style={{minHeight:"100vh",background:"#050505",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px 0"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700;800;900&family=Inter:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;700&display=swap');@keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}@keyframes slideUp{from{opacity:0;transform:translateY(50px)}to{opacity:1;transform:translateY(0)}}@keyframes pls{0%,100%{transform:scale(1)}50%{transform:scale(1.06)}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes goldPulse{0%,100%{box-shadow:0 0 0 0 rgba(201,168,76,0.25)}50%{box-shadow:0 0 0 8px rgba(201,168,76,0)}}_::-webkit-scrollbar{width:0}*{-webkit-font-smoothing:antialiased}`}</style>
...
/* ══════════════════════════════════════════════════════════════════
   SUPER ADMIN NATIVO — Identidade imutável, fora do ADMIN_USERS_DB
   Único com poder de promover outros usuários a Admin.
   Senha armazenada como SHA-256 — nunca em texto plano.
   Hash gerado offline: SHA-256("Rc@0509bf*19")
══════════════════════════════════════════════════════════════════ */
const SUPER_ADMIN_HASH="0b32e83cf0178587e8aac9861e4e516d2ecaa2775f9312171b563424bc123d63";
const SUPER_ADMIN={
  id:"__sa__",
  name:"Super Admin",
  email:"sa@footstock.com",
  role:"superadmin",
  plan:"Lenda",
  since:"Jan 2026",
  // Nunca aparece em listagens de usuários
  // Nunca pode ser editado, suspenso ou removido por nenhuma ação
  immutable:true,
};

// Computa SHA-256 via Web Crypto API (assíncrono, sem dependências)
...
const ADMIN_USERS_DB=[
  ...
  // ── Usuários Clube (role: "clube") ─────────────────────────────────────────
  // Cada clube parceiro tem um usuário próprio com ticker vinculado
  {id:"clube_URU3",name:"Flamengo · Parceiro",   email:"parceiro@flamengo.com.br",  password:"Flamengo",  role:"clube",ticker:"URU3",since:"Jan 2026"},
  {id:"clube_POR4",name:"Palmeiras · Parceiro",  email:"parceiro@palmeiras.com.br", password:"Palmeiras", role:"clube",ticker:"POR4",since:"Jan 2026"},
  {id:"clube_TIM3",name:"Corinthians · Parceiro",email:"parceiro@corinthians.com.br",password:"Timao",   role:"clube",ticker:"TIM3",since:"Jan 2026"},
  {id:"clube_FOG3",name:"Botafogo · Parceiro",   email:"parceiro@botafogo.com.br",  password:"Botafogo",  role:"clube",ticker:"FOG3",since:"Fev 2026"},
  {id:"clube_GAL3",name:"Atlético-MG · Parceiro",email:"parceiro@atletico.com.br",  password:"Galo",      role:"clube",ticker:"GAL3",since:"Fev 2026"},
];
...
function AdminLoginScreen({onLogin}){
  ...
  const iS=(err,extra={})=>({width:"100%",background:"rgba(201,168,76,.05)",border:`1.5px solid ${err?"rgba(224,85,85,.5)":"rgba(201,168,76,.2)"}`,borderRadius:12,padding:"13px 16px",color:"#fff",fontSize:13,fontFamily:SANS,outline:"none",boxSizing:"border-box",caretColor:ACCENT,...extra});
  const lbl=(t)=><div style={{fontSize:9,color:"rgba(201,168,76,.55)",fontWeight:700,letterSpacing:"1px",fontFamily:SANS,marginBottom:5,textTransform:"uppercase"}}>{t}</div>;

  const handleLogin=async()=>{
    if(!email.trim()||!pass){setError("Preencha e-mail e senha.");return;}
    setLoading(true);setError("");
    try{
      // ── Verifica Super Admin primeiro (hash SHA-256) ─────────────────────
      if(email.trim().toLowerCase()===SUPER_ADMIN.email.toLowerCase()){
        const inputHash=await sha256(pass);
        if(inputHash!==SUPER_ADMIN_HASH){
          setLoading(false);setError("E-mail ou senha inválidos.");return;
        }
        setLoading(false);
        const user={...SUPER_ADMIN};
        setLastLoginUser(user);
        if(bioSupported&&!hasBiometricRegistered(user.id)) setShowRegisterBio(true);
        else onLogin(user);
        return;
      }
      // ── Usuários do ADMIN_USERS_DB ────────────────────────────────────────
      setTimeout(()=>{
        ...
        if(!["admin","monitor","editor","moderador","clube"].includes(found.role)){
          setError("Esta conta não tem permissão de acesso ao painel administrativo.");return;
        }
        setLastLoginUser(found);
        if(bioSupported&&!hasBiometricRegistered(found.id)) setShowRegisterBio(true);
        else onLogin(found);
      },600);
    }catch(e){setLoading(false);setError("Erro ao verificar credenciais.");}
  };
  ...
  const handleBiometric=async()=>{
    ...
    try {
      ...
      // Verifica Super Admin ou usuários comuns
      const found=userId===SUPER_ADMIN.id?{...SUPER_ADMIN}:ADMIN_USERS_DB.find(u=>u.id===userId);
  ...
  if(showRegisterBio) return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    <div style={{position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#161008 0%,#0f0d08 55%,#080808 100%)",padding:"28px 20px 26px",flexShrink:0}}>
      ...
      <div style={{fontSize:26,fontWeight:900,color:"#fff",letterSpacing:"-1.2px",fontFamily:SANS,lineHeight:1.05}}>
        Use sua<br/><span style={{background:"linear-gradient(90deg,#C9A84C,#D4B466)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>biometria</span>
      </div>
    </div>
    <div style={{flex:1,overflowY:"auto",padding:"24px 20px 32px"}}>
      ...
      <div style={{background:"rgba(201,168,76,.07)",border:"1px solid rgba(201,168,76,.2)",borderRadius:13,padding:"12px 14px",marginBottom:20}}>
      ...
      <button onClick={handleRegisterBio} disabled={bioLoading} style={{
        ...
        boxShadow:"0 6px 24px rgba(201,168,76,.35)",marginBottom:12,opacity:bioLoading?.75:1
    ...
    <div style={{position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#161008 0%,#0f0d08 55%,#080808 100%)",padding:"28px 20px 26px",flexShrink:0}}>
      ...
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between"}}>
        <div>
          ...
          <div style={{fontSize:26,fontWeight:900,color:"#fff",letterSpacing:"-1.2px",fontFamily:SANS,lineHeight:1.05}}>
            Entre no<br/><span style={{background:"linear-gradient(90deg,#C9A84C,#D4B466)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>painel</span>
          </div>
        </div>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"22px 20px 32px"}}>
      ...
      {bioSupported&&bioRegistered&&<>
        <button onClick={handleBiometric} disabled={bioLoading} style={{
          width:"100%",padding:"15px",borderRadius:14,border:`1.5px solid rgba(201,168,76,.4)`,
          background:"rgba(201,168,76,.1)",color:"#fff",fontSize:14,fontWeight:800,fontFamily:SANS,
      ...
      <button onClick={handleLogin} disabled={loading} style={{
        ...
        background:GRAD_ACCENT,color:"#fff",fontSize:14,fontWeight:800,fontFamily:SANS,
        boxShadow:"0 6px 24px rgba(201,168,76,.35)",opacity:loading?.75:1,marginBottom:20
...
function AdminLockScreen({adminUser,onUnlock,onLogout}){
  ...
  const handleUnlock=async()=>{
    ...
    setLoading(true);
    try{
      // Super Admin: verificação via SHA-256
      if(adminUser.id===SUPER_ADMIN.id){
        const inputHash=await sha256(pass);
        setLoading(false);
        if(inputHash!==SUPER_ADMIN_HASH){setError("Senha incorreta.");return;}
        setError("");setPass("");onUnlock();return;
      }
      // Outros usuários
      setTimeout(()=>{
    ...
    }catch(e){setLoading(false);setError("Erro ao verificar senha.");}
  };
  ...
  return <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"22px 20px 32px"}}>
      ...
      <button onClick={handleUnlock} disabled={loading} style={{
        width:"100%",padding:"15px",borderRadius:14,border:"none",cursor:"pointer",
        background:GRAD_ACCENT,color:"#fff",fontSize:14,fontWeight:800,fontFamily:SANS,
        boxShadow:"0 6px 24px rgba(201,168,76,.35)",opacity:loading?.75:1,marginBottom:14,
      ...
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"16px",marginBottom:12}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <textarea value={form.texto} onChange={e=>setForm(p=>({...p,texto:e.target.value}))} rows={4}
            ...
            style={{width:"100%",background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.18)",borderRadius:10,padding:"9px 13px",color:TEXT,fontSize:12,fontFamily:SANS,outline:"none",resize:"vertical",boxSizing:"border-box",caretColor:ACCENT}}/>
        </div>
        <div style={{marginBottom:16}}>
...
/* ══════════════════════════════════════════════════════════════════
   MÓDULO 9 — PARCERIAS COM CLUBES
   Acesso: admin e superadmin apenas
   Gerencia contratos de parceria, contatos e dados bancários
══════════════════════════════════════════════════════════════════ */

const PARCERIAS_INIT=[
  {
    id:"parc_URU3",ticker:"URU3",status:"ativa",royalties:20,
    inicioContrato:"2026-01-01",renovacaoContrato:"2026-12-31",
    contatoPrincipal:{nome:"Ricardo Mendes",email:"r.mendes@flamengo.com.br",tel:"(21) 99201-4455"},
    contatoSecundario:{nome:"Beatriz Faria",email:"b.faria@flamengo.com.br",tel:"(21) 98822-3310"},
    banco:{cnpj:"71.581.931/0001-60",banco:"Itaú Unibanco",agencia:"0032",conta:"12345-6",tipoConta:"Corrente"},
    pix:{tipo:"CNPJ",chave:"71.581.931/0001-60"},
    obs:"Contrato vigente — renovação automática.",
  },
  {
    id:"parc_POR4",ticker:"POR4",status:"ativa",royalties:18,
    inicioContrato:"2026-01-15",renovacaoContrato:"2026-12-31",
    contatoPrincipal:{nome:"Fernando Augusto",email:"f.augusto@palmeiras.com.br",tel:"(11) 99312-7700"},
    contatoSecundario:{nome:"Carla Siqueira",email:"c.siqueira@palmeiras.com.br",tel:"(11) 97744-2201"},
    banco:{cnpj:"61.219.503/0001-80",banco:"Bradesco",agencia:"1234",conta:"98765-4",tipoConta:"Corrente"},
    pix:{tipo:"CNPJ",chave:"61.219.503/0001-80"},
    obs:"Royalties negociados em 18% por contrato de exclusividade.",
  },
  {
    id:"parc_TIM3",ticker:"TIM3",status:"negociacao",royalties:20,
    inicioContrato:"",renovacaoContrato:"",
    contatoPrincipal:{nome:"Marcelo Drügg",email:"m.drugg@corinthians.com.br",tel:"(11) 98810-5544"},
    contatoSecundario:{nome:"",email:"",tel:""},
    banco:{cnpj:"",banco:"",agencia:"",conta:"",tipoConta:"Corrente"},
    pix:{tipo:"Email",chave:""},
    obs:"Em fase de aprovação jurídica.",
  },
];

const BLANK_PARCERIA={
  id:"",ticker:"",status:"negociacao",royalties:20,
  inicioContrato:"",renovacaoContrato:"",
  contatoPrincipal:{nome:"",email:"",tel:""},
  contatoSecundario:{nome:"",email:"",tel:""},
  banco:{cnpj:"",banco:"",agencia:"",conta:"",tipoConta:"Corrente"},
  pix:{tipo:"CNPJ",chave:""},
  obs:"",
...
const STATUS_PARCERIA={
  ativa:      {color:"#22c55e",label:"Ativa"},
  negociacao: {color:"#f97316",label:"Em Negociação"},
  suspensa:   {color:"#f43f5e",label:"Suspensa"},
  encerrada:  {color:"#7a7060",label:"Encerrada"},
};

function ParceriasModule(){
  const [parcerias,setParcerias]=useState(PARCERIAS_INIT.map(p=>({...p})));
  const [editId,setEditId]=useState(null);
  const [form,setForm]=useState({...BLANK_PARCERIA});
  const [filterStatus,setFilterStatus]=useState("todas");
  const [confirmDel,setConfirmDel]=useState(null);
  const [saved,setSaved]=useState(false);

  const sf=(patch)=>setForm(p=>({...p,...patch}));
  const sfc=(key,patch)=>setForm(p=>({...p,[key]:{...p[key],...patch}}));

  const filtered=filterStatus==="todas"?parcerias:parcerias.filter(p=>p.status===filterStatus);

  const openEdit=(p)=>{setEditId(p.id);setForm({...p,contatoPrincipal:{...p.contatoPrincipal},contatoSecundario:{...p.contatoSecundario},banco:{...p.banco},pix:{...p.pix}});};
  const openNew =()=>{setEditId("new");setForm({...BLANK_PARCERIA,id:`parc_${Date.now()}`});};

  const save=()=>{
    if(editId==="new") setParcerias(p=>[{...form},...p]);
    else setParcerias(p=>p.map(x=>x.id===editId?{...form}:x));
    setSaved(true);
    setTimeout(()=>{setSaved(false);setEditId(null);},1200);
  };

  const del=(id)=>{setParcerias(p=>p.filter(x=>x.id!==id));setConfirmDel(null);};

  const clubeDoTicker=(t)=>CLUBS.find(c=>c.ticker===t);
  const fmt=(n)=>n.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});

  // ── Cabeçalho de seção interna
  const SecHead=({children})=><div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS,marginBottom:8,marginTop:16,textTransform:"uppercase"}}>{children}</div>;

  // ── Campo de input inline
  const F=({label,value,onChange,type="text",placeholder=""})=><div style={{marginBottom:10}}>
    <div style={{fontSize:9,color:MUTED,fontWeight:700,fontFamily:SANS,marginBottom:4,letterSpacing:"0.3px"}}>{label}</div>
    <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
      style={{width:"100%",background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.18)",borderRadius:9,padding:"8px 12px",
        color:TEXT,fontSize:11,fontFamily:MONO,outline:"none",boxSizing:"border-box",caretColor:ACCENT}}/>
  </div>;

  // ── Formulário de edição
  if(editId){
    const club=clubeDoTicker(form.ticker);
    return <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        <span style={{fontSize:13,fontWeight:800,color:"#fff",fontFamily:SANS}}>{editId==="new"?"Nova Parceria":"Editar Parceria"}</span>
        <Btn onClick={()=>setEditId(null)} color={MUTED} variant="outline" small>✕ Cancelar</Btn>
      </div>

      {/* Clube & Contrato */}
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:12}}>
        <SecHead>Clube & Contrato</SecHead>

        {/* Seletor de clube */}
        <div style={{marginBottom:10}}>
          <div style={{fontSize:9,color:MUTED,fontWeight:700,fontFamily:SANS,marginBottom:4}}>CLUBE</div>
          <select value={form.ticker} onChange={e=>sf({ticker:e.target.value})}
            style={{width:"100%",background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.18)",borderRadius:9,padding:"8px 12px",color:TEXT,fontSize:11,fontFamily:MONO,outline:"none"}}>
            <option value="">— Selecione —</option>
            {CLUBS.map(c=><option key={c.ticker} value={c.ticker}>{c.ticker} · {c.realName} (Série {c.div})</option>)}
          </select>
        </div>

        {/* Preview do clube selecionado */}
        {club&&<div style={{display:"flex",alignItems:"center",gap:10,background:SURFACE,borderRadius:10,padding:"10px 12px",marginBottom:10}}>
          <div style={{width:32,height:32,borderRadius:9,background:`linear-gradient(145deg,${club.color},${club.color}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:900,color:club.c2,flexShrink:0}}>{club.ticker.replace(/\d/g,"")}</div>
          <div>
            <div style={{fontSize:11,fontWeight:700,color:"#fff",fontFamily:SANS}}>{club.realName}</div>
            <div style={{fontSize:9,color:MUTED,fontFamily:SANS}}>{club.name} · {club.ticker} · Série {club.div} · Receita: {club.revenueLabel}</div>
          </div>
        </div>}

        {/* Status */}
        <div style={{marginBottom:10}}>
          <div style={{fontSize:9,color:MUTED,fontWeight:700,fontFamily:SANS,marginBottom:6}}>STATUS</div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            {Object.entries(STATUS_PARCERIA).map(([k,v])=><button key={k} onClick={()=>sf({status:k})} style={{
              padding:"5px 12px",borderRadius:8,border:`1px solid ${form.status===k?v.color:BORDER}`,cursor:"pointer",
              background:form.status===k?`${v.color}18`:"transparent",color:form.status===k?v.color:MUTED,
        ...
        {/* Royalties */}
        <div style={{marginBottom:10}}>
          <div style={{fontSize:9,color:MUTED,fontWeight:700,fontFamily:SANS,marginBottom:6}}>
            ROYALTIES: <span style={{color:GOLD,fontFamily:MONO}}>{form.royalties}%</span>
          </div>
          <input type="range" min="1" max="40" step="0.5" value={form.royalties}
            onChange={e=>sf({royalties:parseFloat(e.target.value)})}
            style={{width:"100%",accentColor:GOLD}}/>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:8,color:MUTED,fontFamily:SANS,marginTop:2}}>
            <span>1%</span><span style={{color:GOLD,fontWeight:700}}>Padrão: 20%</span><span>40%</span>
          </div>
        </div>

        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <F label="INÍCIO DO CONTRATO" value={form.inicioContrato} onChange={v=>sf({inicioContrato:v})} type="date"/>
          <F label="RENOVAÇÃO" value={form.renovacaoContrato} onChange={v=>sf({renovacaoContrato:v})} type="date"/>
        </div>
      </div>

      {/* Contato Principal */}
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:12}}>
        <SecHead>👤 Contato Principal</SecHead>
        <F label="NOME" value={form.contatoPrincipal.nome} onChange={v=>sfc("contatoPrincipal",{nome:v})} placeholder="Nome completo"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <F label="E-MAIL" value={form.contatoPrincipal.email} onChange={v=>sfc("contatoPrincipal",{email:v})} placeholder="email@clube.com.br" type="email"/>
          <F label="TELEFONE (DDD)" value={form.contatoPrincipal.tel} onChange={v=>sfc("contatoPrincipal",{tel:v})} placeholder="(11) 99999-0000"/>
        </div>
      </div>

      {/* Contato Secundário */}
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:12}}>
        <SecHead>👤 Contato Secundário</SecHead>
        <F label="NOME" value={form.contatoSecundario.nome} onChange={v=>sfc("contatoSecundario",{nome:v})} placeholder="Nome completo (opcional)"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <F label="E-MAIL" value={form.contatoSecundario.email} onChange={v=>sfc("contatoSecundario",{email:v})} placeholder="email@clube.com.br" type="email"/>
          <F label="TELEFONE (DDD)" value={form.contatoSecundario.tel} onChange={v=>sfc("contatoSecundario",{tel:v})} placeholder="(11) 99999-0000"/>
        </div>
      </div>

      {/* Dados Bancários */}
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:12}}>
        <SecHead>🏦 Dados Bancários</SecHead>
        <F label="CNPJ DO CLUBE" value={form.banco.cnpj} onChange={v=>sfc("banco",{cnpj:v})} placeholder="00.000.000/0001-00"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <F label="BANCO" value={form.banco.banco} onChange={v=>sfc("banco",{banco:v})} placeholder="ex: Itaú Unibanco"/>
          <F label="AGÊNCIA" value={form.banco.agencia} onChange={v=>sfc("banco",{agencia:v})} placeholder="0000"/>
          <F label="CONTA" value={form.banco.conta} onChange={v=>sfc("banco",{conta:v})} placeholder="00000-0"/>
          <div style={{marginBottom:10}}>
            <div style={{fontSize:9,color:MUTED,fontWeight:700,fontFamily:SANS,marginBottom:4}}>TIPO</div>
            <div style={{display:"flex",gap:5}}>
              {["Corrente","Poupança"].map(t=><button key={t} onClick={()=>sfc("banco",{tipoConta:t})} style={{
                flex:1,padding:"7px",borderRadius:8,border:`1px solid ${form.banco.tipoConta===t?ACCENT:BORDER}`,cursor:"pointer",
                background:form.banco.tipoConta===t?"rgba(201,168,76,.12)":"transparent",
                color:form.banco.tipoConta===t?ACCENT:MUTED,fontSize:10,fontWeight:700,fontFamily:SANS
              }}>{t}</button>)}
            </div>
          </div>
        </div>
        ...
        {/* PIX */}
        <div style={{background:SURFACE,borderRadius:10,padding:"12px",marginTop:4}}>
          <div style={{fontSize:9,color:MUTED,fontWeight:700,fontFamily:SANS,marginBottom:8}}>CHAVE PIX</div>
          <div style={{display:"flex",gap:5,marginBottom:8}}>
            {["CNPJ","Email","Telefone","Aleatória"].map(t=><button key={t} onClick={()=>sfc("pix",{tipo:t})} style={{
              flex:1,padding:"5px 4px",borderRadius:7,border:`1px solid ${form.pix.tipo===t?GOLD:BORDER}`,cursor:"pointer",
              background:form.pix.tipo===t?"rgba(201,168,76,.12)":"transparent",
              color:form.pix.tipo===t?GOLD:MUTED,fontSize:8,fontWeight:700,fontFamily:SANS
            }}>{t}</button>)}
          </div>
          <F label={`CHAVE (${form.pix.tipo})`} value={form.pix.chave} onChange={v=>sfc("pix",{chave:v})} placeholder={
            form.pix.tipo==="CNPJ"?"00.000.000/0001-00":
            form.pix.tipo==="Email"?"email@clube.com.br":
            form.pix.tipo==="Telefone"?"(11) 99999-0000":"Chave aleatória UUID"
          }/>
        </div>
      </div>
      ...
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:16}}>
        <SecHead>📝 Observações</SecHead>
        <textarea value={form.obs} onChange={e=>sf({obs:e.target.value})} rows={3}
          placeholder="Notas internas sobre esta parceria..."
          style={{width:"100%",background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:10,padding:"9px 12px",
            color:TEXT,fontSize:11,fontFamily:SANS,outline:"none",resize:"vertical",boxSizing:"border-box",caretColor:ACCENT}}/>
      </div>

      <Btn onClick={save} color={saved?GREEN:ACCENT} disabled={!form.ticker}>
        {saved?"✓ Salvo!":"💾 Salvar Parceria"}
      </Btn>
    </div>;
  }
  ...
  return <div>
    <SectionHeader
      title="Parcerias de Clubes"
      sub={`${parcerias.filter(p=>p.status==="ativa").length} ativas · ${parcerias.length} total`}
      action={<Btn onClick={openNew} color={ACCENT}>+ Nova Parceria</Btn>}
    />

    {/* Filtro de status */}
    <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
      {[["todas","Todas"],["ativa","Ativas"],["negociacao","Negociação"],["suspensa","Suspensas"],["encerrada","Encerradas"]].map(([k,l])=>(
        <button key={k} onClick={()=>setFilterStatus(k)} style={{
          padding:"4px 11px",borderRadius:8,border:"none",cursor:"pointer",
          background:filterStatus===k?ACCENT:SURFACE,color:filterStatus===k?BG:MUTED,fontSize:9,fontWeight:800,fontFamily:SANS
        }}>{l}</button>
      ))}
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
      {[
        {l:"ATIVAS",    v:parcerias.filter(p=>p.status==="ativa").length,      c:GREEN},
        {l:"NEGOCIAÇÃO",v:parcerias.filter(p=>p.status==="negociacao").length,  c:ORANGE},
        {l:"TOTAL",     v:parcerias.length,                                     c:"#fff"},
      ].map(s=><div key={s.l} style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:12,padding:"12px",textAlign:"center"}}>
        <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:4}}>{s.l}</div>
        <div style={{fontSize:22,fontWeight:800,color:s.c,fontFamily:MONO}}>{s.v}</div>
      </div>)}
    ...
    {/* Cards de parceria */}
    {filtered.length===0
      ?<div style={{textAlign:"center",padding:"40px 0",color:MUTED,fontSize:12,fontFamily:SANS}}>Nenhuma parceria neste filtro</div>
      :filtered.map(p=>{
        const club=clubeDoTicker(p.ticker);
        const st=STATUS_PARCERIA[p.status]||{color:MUTED,label:p.status};
        const fansData=CLUB_FANS_DB[p.ticker];
        const mesAtual=fansData?.historico?.[fansData.historico.length-1];
        const receitaEstimada=fansData?(fansData.byPlan.Craque*19.90+fansData.byPlan.Lenda*49.90):0;
        const royaltiesEstimados=receitaEstimada*(p.royalties/100);

        return <div key={p.id} style={{background:CARD,border:`1.5px solid ${club?club.color+"33":BORDER}`,borderRadius:14,padding:"14px",marginBottom:12}}>
          {/* Header do card */}
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
            {club&&<div style={{width:40,height:40,borderRadius:12,background:`linear-gradient(145deg,${club.color},${club.color}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:club.c2,flexShrink:0}}>
              {club.ticker.replace(/\d/g,"")}
            </div>}
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>
                <span style={{fontSize:12,fontWeight:800,color:"#fff",fontFamily:SANS}}>{club?.realName||p.ticker}</span>
                <Badge children={st.label} color={st.color} bg={`${st.color}18`}/>
                <Badge children={`${p.royalties}% royalties`} color={GOLD} bg="rgba(201,168,76,.1)"/>
              </div>
              <div style={{fontSize:9,color:MUTED,fontFamily:SANS}}>{p.ticker} · Série {club?.div||"—"}</div>
            </div>
          </div>

          {/* Financeiro estimado */}
          {fansData&&p.status==="ativa"&&<div style={{background:SURFACE,borderRadius:10,padding:"10px 12px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS}}>ROYALTIES EST. / MÊS</div>
              <div style={{fontSize:16,fontWeight:800,color:GOLD,fontFamily:MONO}}>R${fmt(royaltiesEstimados)}</div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS}}>RECEITA BASE</div>
              <div style={{fontSize:13,fontWeight:700,color:GREEN,fontFamily:MONO}}>R${fmt(receitaEstimada)}</div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:8,color:MUTED,fontFamily:SANS}}>FÃS NO APP</div>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:MONO}}>{fansData.totalFans.toLocaleString("pt-BR")}</div>
            </div>
          </div>}

          {/* Contato */}
          <div style={{marginBottom:10}}>
            <div style={{fontSize:8,color:MUTED,fontFamily:SANS,fontWeight:700,marginBottom:5}}>CONTATO PRINCIPAL</div>
            {p.contatoPrincipal.nome
              ?<div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{width:28,height:28,borderRadius:8,background:"rgba(201,168,76,.1)",border:"1px solid rgba(201,168,76,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0}}>👤</div>
                <div>
                  <div style={{fontSize:11,fontWeight:700,color:"#fff",fontFamily:SANS}}>{p.contatoPrincipal.nome}</div>
                  <div style={{fontSize:9,color:MUTED,fontFamily:SANS}}>{p.contatoPrincipal.email} · {p.contatoPrincipal.tel}</div>
                </div>
              </div>
              :<div style={{fontSize:10,color:MUTED,fontFamily:SANS}}>— não informado</div>
            }
          </div>

          {/* Dados bancários resumidos */}
          {p.banco.banco&&<div style={{background:"rgba(34,197,94,.05)",border:"1px solid rgba(34,197,94,.15)",borderRadius:9,padding:"8px 10px",marginBottom:10}}>
            <div style={{fontSize:8,color:MUTED,fontFamily:SANS,fontWeight:700,marginBottom:4}}>🏦 DADOS BANCÁRIOS</div>
            <div style={{fontSize:9,color:"rgba(255,255,255,.7)",fontFamily:MONO}}>
              {p.banco.banco} · Ag. {p.banco.agencia} · Cc. {p.banco.conta} ({p.banco.tipoConta})
            </div>
            {p.pix.chave&&<div style={{fontSize:9,color:ACCENT2,fontFamily:MONO,marginTop:3}}>
              PIX ({p.pix.tipo}): {p.pix.chave}
            </div>}
          </div>}

          {/* Período do contrato */}
          {(p.inicioContrato||p.renovacaoContrato)&&<div style={{fontSize:9,color:MUTED,fontFamily:SANS,marginBottom:10}}>
            📅 {p.inicioContrato||"—"} → {p.renovacaoContrato||"—"}
          </div>}

          {p.obs&&<div style={{fontSize:9,color:MUTED,fontFamily:SANS,fontStyle:"italic",marginBottom:10,lineHeight:1.5}}>"{p.obs}"</div>}

          {/* Ações */}
          <div style={{display:"flex",gap:6}}>
            <Btn onClick={()=>openEdit(p)} color={ACCENT} variant="outline" small>✎ Editar</Btn>
            <Btn onClick={()=>setConfirmDel(p.id)} color={RED} variant="outline" small>🗑 Excluir</Btn>
          </div>
        </div>;
      })
    }
    ...
    {confirmDel&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
      <div style={{background:CARD,border:"1px solid rgba(244,63,94,.4)",borderRadius:20,padding:"24px",maxWidth:340,width:"100%"}}>
        <div style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SANS,marginBottom:8}}>Excluir parceria?</div>
        <div style={{fontSize:12,color:MUTED,fontFamily:SANS,marginBottom:20}}>Esta ação é irreversível. Todos os dados desta parceria serão removidos.</div>
        <div style={{display:"flex",gap:8}}>
          <Btn onClick={()=>setConfirmDel(null)} color={MUTED} variant="outline">Cancelar</Btn>
          <Btn onClick={()=>del(confirmDel)} color={RED}>Excluir</Btn>
        </div>
      </div>
    </div>}
  </div>;
...
/* ══════════════════════════════════════════════════════════════════
   DADOS MOCK — Fãs por clube (time preferido no app principal)
   Em produção viria da API: GET /clubs/:ticker/fans
══════════════════════════════════════════════════════════════════ */
const CLUB_FANS_DB={
  URU3:{
    totalFans:1840,
    byPlan:{Jogador:980,Craque:610,Lenda:250},
    engagement:{tempoMedio:"9min 12s",recorrencia:72,acessosMes:4820,acessosPorUsuario:2.6},
    historico:[
      {mes:"Out/25",receita:18920,fans:1540},
      {mes:"Nov/25",receita:21340,fans:1620},
      {mes:"Dez/25",receita:23870,fans:1700},
      {mes:"Jan/26",receita:26100,fans:1740},
      {mes:"Fev/26",receita:28450,fans:1790},
      {mes:"Mar/26",receita:30840,fans:1840},
    ],
  },
  POR4:{
    totalFans:1320,
    byPlan:{Jogador:680,Craque:450,Lenda:190},
    engagement:{tempoMedio:"8min 44s",recorrencia:68,acessosMes:3410,acessosPorUsuario:2.6},
    historico:[
      {mes:"Out/25",receita:13100,fans:1100},
      {mes:"Nov/25",receita:14800,fans:1150},
      {mes:"Dez/25",receita:17200,fans:1220},
      {mes:"Jan/26",receita:18900,fans:1270},
      {mes:"Fev/26",receita:20100,fans:1300},
      {mes:"Mar/26",receita:21720,fans:1320},
    ],
  },
  TIM3:{
    totalFans:940,
    byPlan:{Jogador:560,Craque:290,Lenda:90},
    engagement:{tempoMedio:"7min 05s",recorrencia:52,acessosMes:2100,acessosPorUsuario:2.2},
    historico:[
      {mes:"Out/25",receita:9200,fans:820},
      {mes:"Nov/25",receita:9800,fans:855},
      {mes:"Dez/25",receita:10400,fans:890},
      {mes:"Jan/26",receita:10900,fans:910},
      {mes:"Fev/26",receita:11200,fans:928},
      {mes:"Mar/26",receita:11590,fans:940},
    ],
  },
  FOG3:{
    totalFans:780,
    byPlan:{Jogador:420,Craque:270,Lenda:90},
    engagement:{tempoMedio:"8min 30s",recorrencia:64,acessosMes:1900,acessosPorUsuario:2.4},
    historico:[
      {mes:"Out/25",receita:7100,fans:630},
      {mes:"Nov/25",receita:7800,fans:670},
      {mes:"Dez/25",receita:8400,fans:710},
      {mes:"Jan/26",receita:8900,fans:740},
      {mes:"Fev/26",receita:9300,fans:762},
      {mes:"Mar/26",receita:9660,fans:780},
    ],
  },
  GAL3:{
    totalFans:610,
    byPlan:{Jogador:340,Craque:210,Lenda:60},
    engagement:{tempoMedio:"7min 50s",recorrencia:58,acessosMes:1450,acessosPorUsuario:2.4},
    historico:[
      {mes:"Out/25",receita:5400,fans:510},
      {mes:"Nov/25",receita:5900,fans:540},
      {mes:"Dez/25",receita:6300,fans:565},
      {mes:"Jan/26",receita:6700,fans:585},
      {mes:"Fev/26",receita:7100,fans:600},
      {mes:"Mar/26",receita:7390,fans:610},
    ],
  },
};

/* ══════════════════════════════════════════════════════════════════
   MÓDULO 9 — DASHBOARD DO CLUBE PARCEIRO
   Acesso: role "clube" apenas — cada clube vê somente seus dados
══════════════════════════════════════════════════════════════════ */
function ClubeDashboardModule({adminUser}){
  const ticker=adminUser?.ticker||"";
  const club=CLUBS.find(c=>c.ticker===ticker);
  const data=CLUB_FANS_DB[ticker];
  ...
  if(!club||!data) return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"60vh",gap:12,textAlign:"center"}}>
      ...
      <div style={{fontSize:14,fontWeight:700,color:"#fff",fontFamily:SANS}}>Clube não encontrado</div>
      <div style={{fontSize:11,color:MUTED,fontFamily:SANS}}>Entre em contato com o suporte FootStock.</div>
    </div>
  );

  const mesAtual=data.historico[data.historico.length-1];
  const mesAnterior=data.historico[data.historico.length-2];
  const receitaAtual=mesAtual.receita;
  const royaltiesAtual=receitaAtual*0.20;
  const crescimento=mesAnterior?((receitaAtual-mesAnterior.receita)/mesAnterior.receita*100):0;

  const planPrices={Craque:19.90,Lenda:49.90,Jogador:0};
  const receitaCalc=data.byPlan.Craque*planPrices.Craque+data.byPlan.Lenda*planPrices.Lenda;

  const fmt=(n)=>n.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});

  return <div>
    {/* Cabeçalho do clube */}
    <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:"16px",marginBottom:14,position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,borderRadius:"50%",background:`radial-gradient(circle,${club.color}22 0%,transparent 70%)`,pointerEvents:"none"}}/>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:48,height:48,borderRadius:14,background:`linear-gradient(145deg,${club.color},${club.color}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:900,color:club.c2,flexShrink:0,boxShadow:`0 4px 16px ${club.color}44`}}>
          {club.ticker.replace(/\d/g,"")}
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:15,fontWeight:800,color:"#fff",fontFamily:SANS,lineHeight:1}}>{club.realName}</div>
          <div style={{fontSize:9,color:MUTED,fontFamily:SANS,marginTop:3}}>{club.name} · {club.ticker} · Série {club.div}</div>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:2}}>PARCEIRO DESDE</div>
          <div style={{fontSize:11,fontWeight:700,color:ACCENT,fontFamily:SANS}}>{adminUser?.since||"2026"}</div>
        </div>
      </div>
    </div>

    {/* KPIs — fãs */}
    <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS,marginBottom:8}}>TORCIDA NO FOOTSTOCK</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px"}}>
        <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:4}}>TOTAL DE FÃS</div>
        <div style={{fontSize:26,fontWeight:800,color:"#fff",fontFamily:MONO,lineHeight:1}}>{data.totalFans.toLocaleString("pt-BR")}</div>
        <div style={{fontSize:9,color:GREEN,fontFamily:SANS,marginTop:4}}>↑ {mesAtual.fans-mesAnterior.fans} este mês</div>
      </div>
      <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px"}}>
        <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:4}}>PLANOS PAGOS</div>
        <div style={{fontSize:26,fontWeight:800,color:ACCENT,fontFamily:MONO,lineHeight:1}}>{(data.byPlan.Craque+data.byPlan.Lenda).toLocaleString("pt-BR")}</div>
        <div style={{fontSize:9,color:MUTED,fontFamily:SANS,marginTop:4}}>Craque + Lenda</div>
      </div>
    </div>

    {/* Distribuição por plano */}
    <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:14}}>
      <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS,marginBottom:12}}>DISTRIBUIÇÃO POR PLANO</div>
      {[
        {plan:"Lenda",  color:GOLD,   icon:"👑", price:"R$49,90/mês"},
        {plan:"Craque", color:ACCENT, icon:"⭐", price:"R$19,90/mês"},
        {plan:"Jogador",color:MUTED,  icon:"⚡", price:"Gratuito"},
      ].map(({plan,color,icon,price})=>{
        const count=data.byPlan[plan]||0;
        const pct=data.totalFans>0?Math.round(count/data.totalFans*100):0;
        return <div key={plan} style={{marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:11}}>{icon}</span>
              <span style={{fontSize:11,fontWeight:700,color,fontFamily:SANS}}>{plan}</span>
              <span style={{fontSize:9,color:MUTED,fontFamily:SANS}}>{price}</span>
            </div>
            <span style={{fontSize:11,fontWeight:800,color,fontFamily:MONO}}>{count.toLocaleString("pt-BR")} <span style={{fontSize:9,color:MUTED}}>({pct}%)</span></span>
          </div>
    ...
    {/* Engajamento */}
    <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS,marginBottom:8}}>ENGAJAMENTO DA TORCIDA</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
      {[
        {l:"TEMPO MÉDIO/SESSÃO", v:data.engagement.tempoMedio,         c:"#fff"},
        {l:"TAXA RECORRÊNCIA",   v:`${data.engagement.recorrencia}%`,  c:ACCENT},
        {l:"ACESSOS/MÊS",       v:data.engagement.acessosMes.toLocaleString("pt-BR"), c:"#fff"},
        {l:"ACESSOS/FÃ",        v:`${data.engagement.acessosPorUsuario}×`, c:ACCENT2},
      ].map(s=><div key={s.l} style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:12,padding:"12px"}}>
        <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:4}}>{s.l}</div>
        <div style={{fontSize:18,fontWeight:800,color:s.c,fontFamily:MONO,lineHeight:1}}>{s.v}</div>
      </div>)}
    </div>
    ...
    <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS,marginBottom:8}}>FINANCEIRO — MÊS ATUAL ({mesAtual.mes})</div>
    <div style={{background:CARD,border:`1px solid rgba(34,197,94,.25)`,borderRadius:14,padding:"16px",marginBottom:14}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <div>
          <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:4}}>RECEITA TOTAL</div>
          <div style={{fontSize:22,fontWeight:800,color:GREEN,fontFamily:MONO,lineHeight:1}}>R${fmt(receitaCalc)}</div>
          <div style={{fontSize:9,color:MUTED,fontFamily:SANS,marginTop:3}}>assinaturas pagas</div>
        </div>
        <div>
          <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginBottom:4}}>CRESCIMENTO</div>
          <div style={{fontSize:22,fontWeight:800,color:crescimento>=0?GREEN:RED,fontFamily:MONO,lineHeight:1}}>{crescimento>=0?"↑":"↓"}{Math.abs(crescimento).toFixed(1)}%</div>
          <div style={{fontSize:9,color:MUTED,fontFamily:SANS,marginTop:3}}>vs. mês anterior</div>
        </div>
      </div>
      {/* Royalties */}
      <div style={{background:"rgba(201,168,76,.08)",border:"1px solid rgba(201,168,76,.25)",borderRadius:12,padding:"14px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <div>
            <div style={{fontSize:9,color:MUTED,fontFamily:SANS,fontWeight:700,letterSpacing:"0.5px"}}>💰 ROYALTIES A RECEBER</div>
            <div style={{fontSize:9,color:MUTED,fontFamily:SANS,marginTop:2}}>20% sobre receita total de assinaturas</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:22,fontWeight:900,color:GOLD,fontFamily:MONO,lineHeight:1}}>R${fmt(receitaCalc*0.20)}</div>
            <div style={{fontSize:8,color:MUTED,fontFamily:SANS,marginTop:2}}>previsão {mesAtual.mes}</div>
          </div>
        </div>
        <div style={{height:1,background:BORDER,margin:"10px 0"}}/>
        <div style={{display:"flex",justifyContent:"space-between"}}>
          <span style={{fontSize:10,color:MUTED,fontFamily:SANS}}>Craque ({data.byPlan.Craque} fãs × R$19,90)</span>
          <span style={{fontSize:10,fontWeight:700,color:ACCENT,fontFamily:MONO}}>R${fmt(data.byPlan.Craque*19.90*0.20)}</span>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
          <span style={{fontSize:10,color:MUTED,fontFamily:SANS}}>Lenda ({data.byPlan.Lenda} fãs × R$49,90)</span>
          <span style={{fontSize:10,fontWeight:700,color:GOLD,fontFamily:MONO}}>R${fmt(data.byPlan.Lenda*49.90*0.20)}</span>
        </div>
      </div>
    </div>

    {/* Histórico mensal — cards colapsáveis */}
    <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:"14px 16px",marginBottom:20}}>
      <button onClick={()=>setHistExpanded(v=>!v)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{fontSize:9,color:MUTED,fontWeight:700,letterSpacing:"0.8px",fontFamily:SANS}}>HISTÓRICO MENSAL</div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:9,color:ACCENT,fontFamily:SANS,fontWeight:700}}>{histExpanded?"ocultar":"ver todos"}</span>
          <span style={{fontSize:14,color:ACCENT,transition:"transform .2s",display:"inline-block",transform:histExpanded?"rotate(180deg)":"rotate(0deg)"}}>⌄</span>
        </div>
      </button>

      {/* Mês atual sempre visível */}
      <div style={{marginTop:12,background:"rgba(201,168,76,.07)",border:"1px solid rgba(201,168,76,.2)",borderRadius:12,padding:"12px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:GREEN}}/>
            <span style={{fontSize:11,fontWeight:800,color:"#fff",fontFamily:SANS}}>{mesAtual.mes}</span>
            <Badge children="Atual" color={GREEN} bg="rgba(34,197,94,.12)"/>
          </div>
          <span style={{fontSize:11,fontWeight:800,color:GOLD,fontFamily:MONO}}>R${fmt(receitaCalc*0.20)}</span>
        </div>
        <div style={{display:"flex",justifyContent:"space-between"}}>
          <span style={{fontSize:9,color:MUTED,fontFamily:SANS}}>{mesAtual.fans.toLocaleString("pt-BR")} fãs · R${fmt(receitaCalc)} receita</span>
          <span style={{fontSize:9,color:GREEN,fontFamily:SANS}}>royalties estimados</span>
        </div>
      </div>

      {/* Histórico colapsável */}
      {histExpanded&&<div style={{marginTop:8,display:"flex",flexDirection:"column",gap:6}}>
        {[...data.historico].reverse().slice(1).map((h,i)=>{
          const recH=h.fans*(data.byPlan.Craque/data.totalFans)*19.90+(h.fans*(data.byPlan.Lenda/data.totalFans))*49.90;
          const royH=recH*0.20;
          return <div key={h.mes} style={{background:SURFACE,borderRadius:10,padding:"11px 13px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,.7)",fontFamily:SANS}}>{h.mes}</span>
              <span style={{fontSize:11,fontWeight:800,color:ACCENT2,fontFamily:MONO}}>R${fmt(royH)}</span>
            </div>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <span style={{fontSize:9,color:MUTED,fontFamily:SANS}}>{h.fans.toLocaleString("pt-BR")} fãs</span>
              <span style={{fontSize:9,color:MUTED,fontFamily:SANS}}>R${fmt(recH)} receita</span>
            </div>
          </div>;
        })}
      </div>}
...
const ROLE_TABS={
  superadmin: new Set(["dashboard","motor","usuarios","financeiro","engajamento","moderacao","noticias","patrocinadores","parcerias"]),
  admin:      new Set(["dashboard","motor","usuarios","financeiro","engajamento","moderacao","noticias","patrocinadores","parcerias"]),
  ...
  clube:      new Set(["clube_dashboard"]),
};
// Ações de usuário permitidas por role
const ROLE_USER_ACTIONS={
  // Superadmin: único que pode promover a admin
  superadmin: new Set(["promote_admin","promote_monitor","promote_editor","promote_moderador","demote","suspend","unsuspend","reset","promote_lenda","promote_craque"]),
  // Admin: tudo exceto promover a admin
  admin:      new Set(["promote_monitor","promote_editor","promote_moderador","demote","suspend","unsuspend","reset","promote_lenda","promote_craque"]),
  monitor:    new Set([]),
...
const ROLE_META={
  superadmin:{color:"#ffd700",icon:"⚡",label:"SUPER ADMIN"},
  admin:     {color:"#f43f5e",icon:"👑",label:"ADMIN"},
  monitor:   {color:"#f97316",icon:"🔭",label:"MONITOR"},
  editor:    {color:"#C9A84C",icon:"✏️", label:"EDITOR"},
  moderador: {color:"#D4B466",icon:"🛡",label:"MODERADOR"},
  clube:     {color:"#22c55e",icon:"🏟",label:"CLUBE PARCEIRO"},
};
...
const ROLE_DEFAULT_TAB={superadmin:"dashboard",admin:"dashboard",monitor:"dashboard",editor:"dashboard",moderador:"moderacao",clube:"clube_dashboard"};

const pillStyle=a=>({flex:1,padding:"6px 2px",borderRadius:8,border:"none",cursor:"pointer",
  fontSize:9,fontWeight:a?700:500,background:a?"rgba(201,168,76,.15)":"transparent",
...
export default function FootStockAdmin(){
  ...
  const {motorState,paused,setPaused,forceCircuitBreaker,clearCircuitBreaker,setFV}=useMockMarket();
  ...
  // ── Clube parceiro: interface dedicada, sem nav principal ──────────────────
  if(currentRole==="clube") return <PhoneShell time={liveTime}>
    <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
      <div style={{padding:"6px 14px 0",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:7}}>
            <div style={{width:32,height:32,borderRadius:9,background:"linear-gradient(145deg,#1a1510,#0e0c09)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 0 1px rgba(201,168,76,.3),0 4px 20px rgba(201,168,76,.15)"}}>
            ...
            <div>
              <div style={{fontSize:14,fontWeight:700,color:"#fff",letterSpacing:"-0.3px",lineHeight:1,fontFamily:SANS,display:"flex",alignItems:"baseline",gap:4}}>
                FootStock<span style={{fontSize:8,fontWeight:900,color:GOLD,letterSpacing:"2px",fontFamily:SANS}}>PARCEIRO</span>
              </div>
              <div style={{fontSize:7,color:GREEN,fontWeight:700,letterSpacing:"2px",marginTop:1,fontFamily:SANS,opacity:.85}}>🏟 CLUBE PARCEIRO</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div style={{background:"rgba(34,197,94,.12)",border:"1px solid rgba(34,197,94,.3)",borderRadius:20,padding:"3px 9px",fontSize:7,fontWeight:900,color:GREEN,fontFamily:SANS}}>
              🏟 {adminUser?.ticker}
            </div>
            <button onClick={()=>setAdminUser(null)} style={{background:"rgba(244,63,94,.08)",border:"1px solid rgba(244,63,94,.2)",color:RED,borderRadius:8,padding:"4px 9px",fontSize:9,cursor:"pointer",fontFamily:SANS,fontWeight:700}}>Sair</button>
          </div>
        </div>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"10px 14px"}}>
        <ClubeDashboardModule adminUser={adminUser}/>
      </div>
    </div>
  </PhoneShell>;
  ...
  const NAV_ROW=[
    ...
    {id:"parcerias",     icon:"🏟",label:"Parcerias", blocked:!canAccess(currentRole,"parcerias")},
  ];
  ...
  return <PhoneShell time={liveTime}>
    <div style={{flex:1,display:"flex",flexDirection:"column",background:BG,overflow:"hidden"}}>
      ...
      <div style={{padding:"6px 14px 0",position:"relative",zIndex:20,flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:1,background:"rgba(201,168,76,.04)",borderRadius:10,padding:"3px",marginBottom:2,border:"1px solid rgba(201,168,76,.12)"}}>
          {NAV_ROW.map(n=>{
            ...
            return <button key={n.id} onClick={()=>{if(!n.blocked)setTab(n.id);}}
              ...
              style={{...pillStyle(active),cursor:n.blocked?"not-allowed":"pointer",opacity:n.blocked?.3:1}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px 14px",position:"relative",zIndex:1}}>
        ...
        {tab==="motor"          &&(canAccess(currentRole,"motor")           ?<MotorModule motorState={motorState} paused={paused} setPaused={setPaused} forceCircuitBreaker={forceCircuitBreaker} clearCircuitBreaker={clearCircuitBreaker} setFV={setFV} users={users}/>:<BlockedSection label="Motor de Mercado"/>)}
        ...
        {tab==="parcerias"      &&(canAccess(currentRole,"parcerias")       ?<ParceriasModule/>:<BlockedSection label="Parcerias de Clubes"/>)}
      </div>
    </div>
  </PhoneShell>;
}
