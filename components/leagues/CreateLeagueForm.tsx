...
import { UploadEmblem } from './UploadEmblem'

type LeagueType = 'PUBLICA' | 'AMIGOS' | 'PRO'
type LeagueDivision = 'BRONZE' | 'PRATA' | 'OURO' | 'ABERTA'
type LeagueDuration = '1S' | '1M' | 'TEMPORADA'
    ...
    >
      ...
      <div>
        ...
        <span
          ...
          className={cn(
            'block mt-1 text-xs',
            (watch('name')?.length ?? 0) > 40 ? 'text-red-400' : 'text-gray-500'
          )}
          aria-live="polite"
        >
      ...
      {/* Emblem upload (feature flag) */}
      {process.env.NEXT_PUBLIC_ENABLE_EMBLEM_UPLOAD === 'true' && (
        <UploadEmblem onSelect={() => null} />
      )}
      ...
      {plan === 'JOGADOR' && (
        <p className="text-xs text-gray-500 flex items-center gap-1.5">
          ...
          Ligas de amigos exigem plano Craque. Ligas PRO são exclusivas do plano Lenda.
        </p>
      )}
