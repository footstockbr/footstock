...
export function LeagueCard({ league, isMember, currentUserId: _currentUserId, onJoinSuccess }: Props) {
  ...
  const division = league.division ?? 'ABERTA'
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap mt-1">
            <span className={cn('text-xs px-2 py-0.5 rounded-full font-medium', DIVISION_COLORS[division])}>
              {division}
            </span>
            <span className={cn(
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500 mb-3">
        ...
        <span className="flex items-center gap-1">
          ...
          {league.duration ? (DURATION_LABELS[league.duration] ?? league.duration) : '—'}
        </span>
        {league.endsAt && (
