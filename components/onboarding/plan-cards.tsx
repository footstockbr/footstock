...
import { Btn as Button, Badge } from "@/components/ui";
...
export function PlanCards({ onSelect, onSkip, currentPlan }: PlanCardsProps) {
  ...
  const handlePlanClick = (plan: PlanType) => {
    if (onSelect) {
      onSelect(plan);
    } else if (plan === PLAN_TYPE.JOGADOR) {
      onSkip();
    } else {
      router.push(`${ROUTES.PLANOS}?plan=${plan}&source=onboarding`);
    }
  };
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-[#EAECEF]">
              ⚽ {PLAN_LABELS[PLAN_TYPE.JOGADOR]}
            </span>
            {currentPlan === PLAN_TYPE.JOGADOR && (
        ...
        <ul className="flex flex-col gap-1">
          {PLAN_FEATURES_DISPLAY[PLAN_TYPE.JOGADOR].map((f) => (
            <li
              key={f}
        ...
        <Button
          ...
          fullWidth
          size="sm"
          onClick={() => handlePlanClick(PLAN_TYPE.JOGADOR)}
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-[#F0B90B]">
              ⭐ {PLAN_LABELS[PLAN_TYPE.CRAQUE]}
            </span>
            {currentPlan === PLAN_TYPE.CRAQUE && (
              <Badge variant="default">Seu plano atual</Badge>
            )}
          </div>
        ...
        <ul className="flex flex-col gap-1">
          {PLAN_FEATURES_DISPLAY[PLAN_TYPE.CRAQUE].map((f) => (
            <li
              key={f}
              className="flex items-center gap-2 text-xs text-[#929AA5]"
            >
        ...
        <Button
          ...
          fullWidth
          size="sm"
          onClick={() => handlePlanClick(PLAN_TYPE.CRAQUE)}
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-[#F0B90B]">
              👑 {PLAN_LABELS[PLAN_TYPE.LENDA]}
            </span>
            {currentPlan === PLAN_TYPE.LENDA && (
              <Badge variant="default">Seu plano atual</Badge>
            )}
          </div>
          <div className="text-right">
        ...
        <ul className="flex flex-col gap-1">
          {PLAN_FEATURES_DISPLAY[PLAN_TYPE.LENDA].map((f) => (
            <li
              key={f}
              className="flex items-center gap-2 text-xs text-[#929AA5]"
            >
        ...
        <Button
          ...
          fullWidth
          size="sm"
          onClick={() => handlePlanClick(PLAN_TYPE.LENDA)}
