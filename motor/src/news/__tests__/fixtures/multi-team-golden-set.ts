...
/**
 * `recorded` e reservado para capturas legadas/locais cuja origem HTTP real
 * nao foi comprovada. Somente `real-http` pode participar do gate H8 de
 * producao; `simulated` inclui explicitamente `agent-sim`.
 */
export type GoldenProvenance = 'simulated' | 'recorded' | 'real-http'

export interface GoldenSetMeta {
  schema: 'h8-golden-set-meta/v1'
  provenance: GoldenProvenance
  acquisition: 'agent-sim' | 'local-recorder' | 'provider-http'
...
export function isProductionH8Evidence(
  meta: GoldenSetMeta,
  cases: readonly GoldenCase[],
): boolean {
  return (
    meta.provenance === 'real-http' &&
    meta.acquisition === 'provider-http' &&
    meta.productionH8Eligible === true &&
    meta.recordedAt !== null &&
    cases.length >= 30 &&
    cases.every((gold) => gold.provenance === 'real-http')
  )
}
