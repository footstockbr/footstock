...
export const CreateOrderSchema = z
  .object({
    ...
    type: z.enum(
      ...
      { errorMap: () => ({ message: 'Tipo de ordem inválido. Use MARKET, LIMIT, OCO ou SCHEDULED.' }) }
    ),

    side: z.enum([ORDER_SIDE.BUY, ORDER_SIDE.SELL] as [string, ...string[]], {
      errorMap: () => ({ message: 'Lado da operação inválido. Use BUY ou SELL.' }),
    }),

    quantity: z
      .number({ invalid_type_error: 'quantity deve ser um número inteiro positivo.' })
