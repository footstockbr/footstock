...
// Cache Redis segregado por plano (ai:cache:{ticker}:{plan} TTL 15min)
// ============================================================================
  ...
  async fetchContext(ticker: string, userId: string): Promise<AnalysisContext> {
    ...
    return {
      ...
      userPosition: userPositionRaw
        ? { qty: userPositionRaw.quantity, avgPrice: Number(userPositionRaw.avgPrice) }
        : null,
      recentNews: recentNewsRaw.map(n => ({
        title: n.title,
        sentiment: SENTIMENT_SCORE[n.sentiment as string] ?? 0,
      })),
    }
  }
