...
export class ShortService {
  ...
  async closeShort(
    ...
    const closeOperationValue = position.quantity * currentPrice
    const closeFee = calculateFee(closeOperationValue)

    // P&L: positivo quando preço caiu (short ganhou), menos juros e taxa
    const pnl = (avgPrice - currentPrice) * position.quantity - interestAccrued - closeFee
  ...
  async accrueInterest(positionId: string): Promise<number> {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await tx.transaction.create({
        data: {
          userId: position.userId, assetId: position.assetId,
          type: 'MARKET', financialType: 'SHORT_INTEREST', side: 'SELL',
          quantity: position.quantity, price: Number(position.avgPrice), fee: 0,
          totalAmount: dailyInterest, fsAmount: -dailyInterest,
