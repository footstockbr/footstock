...
async function collectUserData(userId: string) {
  const [profile, orders, positions, transactions, subscriptions, consents] = await Promise.all([
    ...
    prisma.position.findMany({
      where: { userId },
      select: { id: true, assetId: true, quantity: true, avgPrice: true, totalInvested: true },
    }),
    prisma.transaction.findMany({
      where: { userId },
      select: { id: true, type: true, side: true, quantity: true, price: true, fee: true, createdAt: true },
    }),
