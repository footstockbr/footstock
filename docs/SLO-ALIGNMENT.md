...
|---------|---------|--------|--------|----------------|
| API Next.js | Disponibilidade | 99.5% | 30 dias rolling | Railway Metrics / Sentry |
...
| Postgres (Railway) | Query p95 | < 100ms | Rolling 5min | `pg_stat_statements` |
...
| Postgres p95 > 200ms por 5min | Warning | Investigar queries lentas com `pg_stat_statements` |
