...
- Name: e2e-flows.spec.ts >> Smoke Tests — API Health Check >> GET /api/v1/health retorna status ok
- Location: tests/e2e/e2e-flows.spec.ts:388:7
...
```ts
  289 | 
  290 |   test('/glossario carrega termos', async ({ page }) => {
  291 |     await page.goto('/glossario')
  292 |     await page.waitForLoadState('networkidle')
  293 |     await expectNoServerError(page)
  294 |   })
  295 | 
  296 |   test('/noticias carrega feed de notícias', async ({ page }) => {
  297 |     await page.goto('/noticias')
