...
export type AdminResource =
  ...
  | 'news:read'
  | 'news:write'
  | 'news:archive'   // RESOLVED: recurso de arquivo para EDITOR (sem permissão de delete permanente)
  | 'news:delete'
  | 'leagues:read'
  | 'leagues:moderate'
  | 'forum:read'
...
const PERMISSIONS: Record<AdminRole, AdminResource[]> = {
  ...
  EDITOR: [
    // RESOLVED: EDITOR pode arquivar notícias mas NÃO pode excluí-las permanentemente (FDD RF-PA-006)
    'news:read',
    'news:write',
    'news:archive',
    'admin:dashboard',
