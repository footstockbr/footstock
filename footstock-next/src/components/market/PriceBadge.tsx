...
// T-022/T9: Badge que indica delay de cotação por plano ou estado de buffering.
// Retorna null quando isDelayed=false e isBuffering=false — não ocupa espaço para LENDA.
...
export function PriceBadge({
  ...
  if (isBuffering) {
    const label = 'Cotação em aquecimento'
    if (variant === 'inline') {
      return (
        <span
          className={cn(
          ...
          aria-label={label}
          data-testid="price-badge-buffering-inline"
        >
          {label}
        </span>
      )
    ...
    return (
      <span
        ...
        aria-label={label}
        data-testid="price-badge-buffering"
      >
        <Loader2 className={cn('animate-spin', size === 'sm' ? 'w-2.5 h-2.5' : 'w-3 h-3')} aria-hidden="true" />
        {label}
      </span>
    )
  }

  if (!isDelayed || !delayMinutes || delayMinutes <= 0) return null
