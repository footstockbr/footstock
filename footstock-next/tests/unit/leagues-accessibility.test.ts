...
describe('LeagueTabs — accessibility contract', () => {
  ...
  it('tab panel has role=tabpanel with aria-labelledby', () => {
    const tabId = 'tab-memberships'
    const panelId = 'tabpanel-memberships'
    expect(panelId).toBe(`tabpanel-${tabId.replace('tab-', '')}`)
  })

  it('memberships tab exists with accessible label', () => {
    const tabs = [
    ...
    expect(tabs.find(t => t.id === 'memberships')?.label).toBe('Minhas')
    expect(tabs.find(t => t.id === 'criadas')?.label).toBe('Criadas por mim')
  })
})
